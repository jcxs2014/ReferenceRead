#!/usr/bin/env python3
"""
quality_matrix.py — 扫描 23 篇论文的 literature_analysis 目录，
...
ROOT = Path(__file__).resolve().parent.parent  # scripts/ → 仓库根（不含 papers/ 子目录）
...
CHECKLIST = [
    ("metadata",      r"## 2\.",                                    "元信息"),
    ("structure",    r"## 3\.",                                    "结构"),
    ("chapter",     r"## (?:4\.|X\.\d)",                            "章节(4/X.n)"),
    ("figure",       r"## Figure \d+",                              "Figure"),
    ("table",        r"## Table \d+",                               "Table"),
    ("formula",      r"## 7\.",                                    "公式"),
    ("numerical",    r"## 8\.",                                    "数值信息"),
    ("experimental", r"## 9\.",                                     "实验/数据"),
]
...
def score_chapter_sections(text: str) -> int:
    """X.1-X.8 章节子节覆盖数量（0-8）。"""
    return len(re.findall(r"## X\.\d", text))
