...
作者开宗明义：银河系宇宙线所需的功率，与银河系中超新星爆发所提供的能量非常吻合，约 **$3\times10^{42}$ erg/s**。这一数字基于"每 30 年一次超新星，每次抛出壳层的动能约 $2\times10^{51}$ erg"的估算。
...
代入参数：B = 3 $\mu$Gauss，$u_{1}$ ≈ $10^{9}$ cm/s，$\tau_{\rm A}$ ≈ 1000 yr：
...
- **明确陡化点**：**$~5\times10^{15}$ eV**（这就是膝部）[FACT]
...
- 这些点源也是 TeV–PeV $\gamma$ 光子的潜在源——$\gamma$ 光子来自源附近加速粒子与物质碰撞产生的 $\pi^{0}$ 衰变。
...
- 第二项 1/$\tau_{\rm R}$ · J：反质子从**银河系逃逸**
- 第三项 1/$\tau_{\rm I}$ · [...]：反质子在星际气体中的**衰减**（湮灭 + 产生反核子出射的次生反应；高能时后者主导）
...
- 一部分宇宙线是"老组分"（old component），驻留时间 $\tau_{\rm R}$ → ∞
- 老组分产生的反质子通量用 $\tau_{\rm R}$ → $\tau_{\rm I}$ 代替
- 此时 p̄/p 比**最终趋于常数**（因为 $\tau_{\rm I}$ = const）
- 但只在**非常高能量**才发生——当"年轻组分"的 $\tau_{\rm R}$ 下降得足够小，变得可忽略之后
...
- 若用 leaky-box 模型的 $\tau_{\rm R}$(E) 作为输入，次级/初级比会比观测**更陡**；通过调整 $\delta$（Eq. 3 中的 $\delta$），使 $\tau_{\rm R}$(E) 下降更慢来修正（Ref. 51）
...
| (6) | E_max ≈ 3 $u_{1}$ Z e B ($\tau_{\rm u}$ $\tau_{\rm A}$)/(20 c) | 最小散射长度下的加速上限 |
| (7) | E_max/Z ≈ $10^{14}$ eV (B=3$\mu$G, $u_{1}$=$10^{9}$ cm/s, $\tau_{\rm A}$=$10^{3}$ yr) | 代入 ISM 参数后的 E_max |
