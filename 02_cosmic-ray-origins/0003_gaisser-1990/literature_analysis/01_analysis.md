...
作者开宗明义：银河系宇宙线所需的功率，与银河系中超新星爆发所提供的能量非常吻合，约 **3 × 10⁴² erg/s**。这一数字基于"每 30 年一次超新星，每次抛出壳层的动能约 2 × 10⁵¹ erg"的估算。

**宇宙线维持稳态谱所需的功率**：

$$Q_{CR} = \frac{\rho_E \, V_S}{\tau_R} \sim 5 \times 10^{40} \text{ erg} \quad (1)$$

（[CRITIQUE] 此处文本中的数值印刷为 5×10⁴⁰，但前文声称功率匹配 3×10⁴²，数量级上存在差异。结合下文推导，作者实际使用的 ρ_E ≈ 1 eV/cm³、V_S ≈ 5×10⁶⁶ cm³、τ_R ≈ 5×10⁶ yr 得到的结果应当为 ~3×10⁴¹ erg/s，与超新星供给 3×10⁴² erg/s 相当接近——这是本文核心论据之一。）

**LIS 能量密度**：

$$\rho_E = 4\pi E^2 \frac{dN}{dE}\bigg|_{\ln E} = \frac{d\rho_\epsilon}{d\ln E} \quad (2)$$

其中 E 是每核子动能，βc 是粒子速度，dN/dE 是核子的微分谱。

**参数取值（作者假设）**：
- ρ_E ≈ 1 eV/cm³ [FACT]
- 源区体积 V_S ≈ 5 × 10⁶⁶ cm³（取银河系盘面）[FACT]
- 在盘中的驻留时间 τ_R ≈ 5 × 10⁶ yr [FACT]

[FACT] 关键脚注：τ_R 是"在盘中的驻留时间"，**不**是宇宙线的"年龄"（后者由如 ¹⁰Be 等放射性同位素相对丰度给出）。
...
### 3.2.1 图的目的
把不同能量区间对总宇宙线能量密度的贡献可视化。作者选择绘制 ∝ dρ_E/d ln E 的量，使得对数坐标图上的物理面积正比于各能量区间的贡献。

### 3.2.2 坐标轴
- **横轴**：动能 E₀，单位 GeV，对数坐标，范围约 0.1 GeV ~ 1000 GeV
- **纵轴**：ρ_e（eV/cm³），线性坐标，范围 0 ~ 0.16
...
- 二者相加 ≈ **1.1 eV/cm³**，与作者前文"ρ_E ≈ 1 eV/cm³"一致 [FACT]
...
- [INTERPRETATION] 这也解释了为什么膝部（10¹⁵ eV 附近）的观测如此困难：那里通量已经很低。
...
对 R < 4 GV，λ_esc = const = 10.88 g/cm²。
...
- [INTERPRETATION] δ ≈ 0.6、γ ≈ 2.7、源谱 E⁻²·¹，三者之间通过 Eq. (4) 形成自洽链，把加速理论、传播模型、观测谱三个部分串联起来。

### 3.3.3 关键参数小结

| 参数 | 数值 | 说明 |
|---|---|---|
| δ | ≈ 0.6 | rigidity-dependent 逃逸指数 |
| γ | ≈ 2.7 | 观测微分谱指数 (10 GeV – 数 TeV) |
| 源谱指数 | ≈ 2.1 | 由 γ - δ 推出，与 Fermi 一阶加速的 E⁻² 接近 |
| λ_esc(4 GV) | ≈ 6 g/cm² | 4 GV 处逃逸长度（来自 Gupta-Webber） |
...
主成分谱在 **10¹⁵ – 10¹⁶ eV** 能量区间发生陡化（spectral steepening），称为 **"knee"**。这是本文的核心问题之一。
...
代入参数：B = 3 μGauss，u₁ ≈ 10⁹ cm/s，τ_A ≈ 1000 yr：
...
[FACT] **Fichtel & Linsley (Ref. 16)**：在 ~10¹⁸ eV 以上**新源占主导**（低能组分在加速失败或逃逸增加后消失）；新源**主要加速质子**。证据来自空气簇射实验观测到的**大波动**（重核簇射波动较小）。

[FACT] **Maryland group (Ref. 17)**：即使是较低能量就存在**两个分量**：质子/氦有较陡谱，重核有较平谱；因此 10¹⁴ eV 及膝部以上组分**已经以重核为主**。这与 Fichtel-Linsley 的假说相反。
...
- **Jokipii & Morrill (Ref. 21)**：银河系星风的终止激波可加速到 **10¹⁹ – 10²⁰ eV**。

**点源（point sources）**：
- 吸积 X 射线双星
- 新的超新星
- **毫秒双星脉冲星**：PSR 1957+20，周期 1.6 ms (Ref. 25)
- 这些点源也是 TeV–PeV γ 光子的潜在源——γ 光子来自源附近加速粒子与物质碰撞产生的 π⁰ 衰变。

[FACT] 作者引用大量综述：Ref. 22 (Weekes, VRH γ-ray astronomy)、Ref. 23 (Nagle-Gaisser-Protheroe)、Ref. 24 (Bonnet-Bidaud & Chardin, Cygnus X-3 review)。

### 3.4.8 本章结论 [FACT]

- **直接在膝部区测量各主要组分的能谱**对理解起源、加速、传播至关重要。
- 低能直接测量（Ref. 12, 27, 28）已经显示出有趣的能量依赖性，需要**跟到高能量**。
- 高于 10¹⁶ eV 的通量约 **1 粒子 m⁻² sr⁻¹ 每年**，需要**数百 m²·年的大口径探测器**。
...
- [CRITIQUE] 简单模型**难以同时满足观测到的 ³He/⁴He 比值**——氦截面足够小，使得 ³He/⁴He 会高于观测（Ref. 33）。
...
- 对比：碎裂反应如 ¹⁶O + H → ¹⁰B + … (10)，**高于 1 GeV 截面几乎恒定**，低于 100 MeV 有共振结构（Ref. 42）。
...
- 第二项 1/τ_R · J：反质子从**银河系逃逸**
- 第三项 1/τ_I · [...]：反质子在星际气体中的**衰减**（湮灭 + 产生反核子出射的次生反应；高能时后者主导）
...
- 因此若反质子在盘内由本地质子谱产生，**p̄/p 比在高能随 τ_R(E) 下降而下降**（τ_R 在反质子的能量处求值，而不是在产生它的更高能量处）[FACT]
...
- 一部分宇宙线是"老组分"（old component），驻留时间 τ_R → ∞
- 老组分产生的反质子通量用 τ_R → τ_I 代替
- 此时 p̄/p 比**最终趋于常数**（因为 τ_I = const）
- 但只在**非常高能量**才发生——当"年轻组分"的 τ_R 下降得足够小，变得可忽略之后
...
| 编号 | 公式 | 含义 |
|---|---|---|
| (1) | Q_CR = ρ_E V_S / τ_R ~ 5×10⁴⁰ erg | 稳态宇宙线功率（文本印刷如此） |
| (2) | ρ_E = 4π E² dN/dE = dρ_ε/d ln E | LIS 能量密度 |
| (3) | λ_esc = λ_esc(4 GV)(R/4 GV)^0.6 (R > 4 GV); 10.88 (R < 4 GV) | rigidity-dependent 逃逸长度 |
| (4) | J(E) ∝ E^(-δ) × Q(E), δ ≈ 0.6 | 观测谱-源谱关系 |
| (5) | J(E) ∝ E^(-γ), γ ≈ 2.7 (10 GeV – 数 TeV) | 观测微分谱 |
| (6) | E_max ≈ 3 u₁ Z e B (τ_u τ_A)/(20 c) | 最小散射长度下的加速上限 |
| (7) | E_max/Z ≈ 10¹⁴ eV (B=3μG, u₁=10⁹ cm/s, τ_A=10³ yr) | 代入 ISM 参数后的 E_max |
| (8) | E₀ = w × N_e^(max) | 空气簇射初级能量估算 |
| (9) | p + H → p + p + p + p̄ | 反质子最小产生过程，阈值 5.63 GeV |
| (10) | ¹⁶O + H → ¹⁰B + … | 碎裂反应，>1 GeV 截面近似恒定 |
| (11) | 反质子平衡方程（源-逃逸-衰减-再加速） | 反质子通量的完整动力学方程 |
| (12) | Q_p̄(E) = 2αρF ∫ dN_p J_p(E_p) dE_p | 反质子源项 |
| (13) | j⁽⁰⁾(E) = τ_R(E) Q_p̄(E) | leaky-box 平衡解 |
...
| 每次超新星壳层动能 | 2 × 10⁵¹ erg | 脚注 |
| 银河系宇宙线供给功率 | ~3 × 10⁴² erg/s | 脚注 |
| 稳态宇宙线功率 | ~5 × 10⁴⁰ erg（或推论 ~3 × 10⁴¹ erg/s） | Eq. (1) |
| LIS 能量密度 ρ_E | ~1 eV/cm³ | 本文 |
| 源区体积 V_S | 5 × 10⁶⁶ cm³ | 本文 |
| 在盘中的驻留时间 τ_R | 5 × 10⁶ yr | 本文 |
| 逃逸指数 δ | ≈ 0.6 | Eq. (4) |
| 观测微分谱 γ | ≈ 2.7 | Eq. (5) |
| 源谱指数 | ≈ 2.1 | 由 γ - δ 推出 |
...
| >10¹⁶ eV 通量 | ~1 m⁻² sr⁻¹ yr⁻¹ | 本文 |
| PSR 1957+20 周期 | 1.6 ms | Ref. 25 |
| 再加速所需 δ 调整 | τ_R(E) 下降更慢 | Ref. 51 |
...
| 21 | Jokipii & Morrill, 1987 | 银河星风终止激波加速到 10¹⁹–10²⁰ eV |
...
- **[Information Not Explicitly Provided]** 超新星供给功率 3×10⁴² erg/s 与 Eq. (1) 中 5×10⁴⁰ erg 的**数量级差异未解释**——可能是 Eq. (1) 印刷错误或单位不同（例如漏了 s⁻¹）。
