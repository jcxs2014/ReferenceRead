...
**[FACT]** X 射线边缘厚度量级 **~$10^{-2}$ pc** → 要求局部 B ~ **100–1000 $\mu$G**（对比 ISM 的 1–6 $\mu$G）→ 放大因子 ~10–100。

**[FACT]** Blasi §4.2 给出关键量级：
- 100 eV X 光子同步辐射要求电子能量 E_e ≈ 8 (E_$\gamma$/100 eV)^{1/2} $B_{100}$^{-1/2} TeV；
...
| 机制 | 位置 | 依赖 | 对加速的影响 |
|---|---|---|---|
| **Shock corrugation / Richtmyer-Meshkov**（Giacalone & Jokipii 2007；Sano 2012） | 下游 | 密度不均匀 $\delta$$\rho$/$\rho$ ~ 1 | 上游散射不变；只对**垂直激波**几何加速有帮助 |
| **加速粒子 streaming 不稳定**（Blasi 主流观点） | **上游** | CR 电流 | 上游 $\delta$B 增大 → 加速时间缩短 → PeVatron 可能 |
...
**[FACT]** Amato §7 独立推导相同结果（式 (29)、(30)），指出**相速度 Re($\omega$)/k ≈ 2% V_sh**——这是散射中心速度的关键修正，见下。
...
**[CRITIQUE]** Bell 模式虽能解释 X 射线边缘的 ~100 $\mu$G 磁场，但**不能直接解决 PeVatron 散射问题**（尺度不匹配）。
...
- 各种机制各有缺陷：
  - 共振 Alfvén 波（强流区）：放大被限制在 $\delta$B ~ $B_{0}$；
