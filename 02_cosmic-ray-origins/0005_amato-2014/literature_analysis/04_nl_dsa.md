...
3. **放大磁场的动力学反作用**：100–1000 $\mu$G 场的磁压虽只是 $\rho$v$^{2}$_s 的 $10^{-2}$–$10^{-3}$，但可**超过**上游热压 → 影响压缩比 → 进一步耦合回分布函数。
...
- 上游无穷远处流体速度 $u_{0}$；
- 加速粒子压力 P_c(z) 随距离向激波增大 → 流体减速 → 速度从 $u_{0}$ 逐渐降到 $u_{1}$（亚激波上游）；
- 在**亚激波**（subshock）处发生阶跃压缩 $u_{1}$→$u_{2}$；
- 总压缩比 R_tot = $u_{0}$/$u_{2}$ 可显著大于 4；亚激波压缩 R_sub = $u_{1}$/$u_{2}$ < 4。

**[FACT]** 图 7（Blasi Fig.7）：CR 修改的激波结构示意图，标注了 $u_{0}$、$u_{1}$、$u_{2}$、R_sub、R_tot。
...
- 低能粒子扩散短、只看到亚激波 R_sub < 4 → 谱比 p$^{-4}$ 更陡；
- 高能粒子扩散到前置区深处、看到 R_tot > 4 → 谱比 p$^{-4}$ 更硬。

**[FACT]** Blasi Fig.8（Blasi et al. 2005）：$M_{0}$ = 10, 50, 100 三个 Mach 数下的粒子分布函数（热+非热）× p$^{4}$ 图。p_max = $10^{5}$ GeV/c，$\xi$ = 3.5。Mach 越高，凹性越强，下游热峰越左移（温度越低）。
...
2. **下游温度低于 Rankine-Hugoniot 预期**（在 RCW86 H$\alpha$ 观测中首次得到支持）；
3. **凹形粒子谱**（少数 SNR 射电谱显示硬化）；
4. **放大磁场**（Chandra X 射线窄边缘 → ~100 $\mu$G）。

**[CRITIQUE]** 预测 #3（凹形谱，高能端比 E$^{-2}$ 硬）与观测到的 $\gamma$ 射线谱（见第 7 章）存在**系统性张力**——几乎所有已测 SNR 的推断粒子谱比 E$^{-2}$ 更陡。