...
2. **B/C 比 + 银河系几何 + CR 通量 → 单个 SN 需提供的加速效率 $\xi_{\rm CR}$**；
...
**[CRITIQUE]** 从 B/C 反推 $\delta$ 时，**$\delta$ = 0.3–0.6 的不确定性**（约 2 倍）反映了 B/C 高能测量的误差（Blasi Fig.2）。这直接影响对**注入谱斜率 $\gamma_{\rm inj}$** 的推断（Amato §5 进一步讨论）。
