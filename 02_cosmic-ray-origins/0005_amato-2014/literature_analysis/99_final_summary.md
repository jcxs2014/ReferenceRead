...
>
> 上一章：[[02_cosmic-ray-origins/0005_amato-2014/literature_analysis/10_references_conclusions.md|10_references_conclusions.md]]

## 11.1 一句话总结

两篇综述（Amato 2014；Blasi 2013，**并非 Padovani & Protheroe 2013**）系统梳理了"SNR 范式"——银河系宇宙线起源于超新星遗迹扩散激波加速（DSA）——从能量学基座、test-particle DSA 到非线性理论（NLDSA + 磁场放大 MFA），并与 2013 年前的最新观测（$\pi^{0}$ 鼓包、Tycho $\gamma$ 射线、H$\alpha$ 宽线、PAMELA/AMS-02 直接测量）逐一比对，指出**证据强有力但仍是间接的**，**单个 SNR 加速到膝区**和**理论硬谱 vs 观测陡谱**两个核心问题仍未解决。

## 11.2 科学问题

- **宇宙线的起源**：银河系 CR 是否主要由 SNR 加速？
- **加速机制**：DSA 是否足以解释膝区？
- **磁场放大**：如何放大 $\delta$B/$B_{0}$ ~ 10–100 倍？
...
- **观测**：多波段（射电、X 射线、$\gamma$ 射线、光学 Balmer 线）；直接宇宙线测量（PAMELA、AMS-02、CREAM、KASCADE-Grande）
- **传播**：Leaky Box → GALPROP/DRAGON → 离散源 + 各向异性约束

## 11.4 最重要结果

1. **加速效率 $\xi_{\rm CR}$ ~ 5–10%**（能量学必需）；
2. **谱指数**：test-particle 强激波 $\alpha$ = 3r/(r-1) → 4；NLDSA → 凹谱；
3. **磁场放大**：$\delta$B/$B_{0}$ ~ 10–100（X 射线窄边缘证据）；
4. **PeVatron 条件**：需 $\delta$B/$B_{0}$ ≫ 1（否则 E_max ~ GeV）；
5. **Bell 非共振模式**：生长快但尺度 < r_L,0；
6. **$\pi^{0}$ 鼓包**（Ackermann 2013）：**强子起源直接证据**（IC443、W44）；
7. **Tycho 推断 E_max ~ 500 TeV**——最接近膝但差一个量级；
8. **Balmer 线**（SNR 0509-67.5）：若 $\beta_{\rm down}$ ≪ 1 → $\xi_{\rm CR}$ ~ 10–20%；
9. **注入谱斜率** $\gamma_{\rm inj}$ ~ 2.3–2.4（结合 B/C + 各向异性）——**比 NLDSA 预测陡**；
...
5. **$\xi_{\rm CR}$ 的 Balmer 线推断**依赖 V_sh、$\beta_{\rm down}$、湍流加热等不确定参数；
6. **正电子分数上升**物理起源未定（暗物质 vs 脉冲星）；
7. **银河系/河外 CR 过渡**（~$10^{17}$–$10^{18}$ eV）观测结论矛盾。

## 11.7 应记住的关键点（15 条）

1. **CR 能量学**：$\xi_{\rm CR}$ ~ 10% → 非线性理论必需；
2. **test-particle 谱**：$\alpha$ = 3r/(r-1)，强激波 $\alpha$ → 4 → E$^{-2}$；
3. **加速时间** $\tau_{\rm acc}$ ∝ D/($u_{1}$-$u_{2}$) → 需缩短 D；
4. **PeVatron 必要条件**：$\delta$B/$B_{0}$ ≫ 1；
5. **无碰撞激波**：粒子库仑碰撞不足以热化 → 靠集体电磁不稳定性；
6. **电子-质子不平衡**：T_e ~ (m_e/m_p) T_p，即使经过 SNR 年龄；
7. **NLDSA 三大标志**：凹谱 + 前置区 + 温度降低；
8. **MFA 强流 vs 弱流**：强流区共振 Alfvén 波增长被**限制在 $\delta$B ~ $B_{0}$**（过去被高估）；
9. **Bell 模式**：非共振、生长极快、但尺度 < r_L,0（不能直接散射 PeV 粒子）；
10. **丝状不稳定性**：逃逸粒子驱动 → 可能放大到逃逸粒子回旋半径尺度；
11. **Sedov-Taylor 相逃逸谱**：N_esc ∝ p$^{-4}$（与 test-particle 谱**巧合**但物理不同）；
12. **$\pi^{0}$ 鼓包**（~70 MeV）：强子起源的"smoking gun"（Fermi-LAT 2013）；
13. **W44/IC443** 强子，**Tycho** 可能强子（E_max ~ 500 TeV），**RXJ1713.7-3946** 倾向轻子；
14. **H$\alpha$ Balmer 线**：窄宽线 + 宽窄线 = CR 加速；需要 V_sh、$\beta_{\rm down}$ 完备测量；
15. **注入谱斜率** $\gamma_{\rm inj}$ ~ 2.3–2.4（B/C + 各向异性约束）——**比 E$^{-2}$ 陡**，是最大未解张力。
...
- Balmer 线多分量测量 — 定量 $\xi_{\rm CR}$。

### 11.9.2 可直接使用的公式
- 谱斜率 $\alpha$ = 3r/(r-1)（test-particle）；
- 加速时间 $\tau_{\rm acc}$ = 3/($u_{1}$-$u_{2}$) · [$D_{1}$/$u_{1}$ + $D_{2}$/$u_{2}$]；
- Bohm 扩散 D_B = r_L c/3；
- 逃逸谱 N_esc ∝ p$^{-4}$（Sedov-Taylor）；
...
- [x] Escape & Spectra — 覆盖
- [x] Superbubble 假说 — 覆盖
- [x] $\gamma$ 射线观测 — 覆盖（RXJ1713.7-3946、Tycho、W44、IC443、W28、W51C、Cygnus）
- [x] Balmer 线 / H$\alpha$ — 覆盖（含 SNR 0509-67.5、RCW86、SN1006）
- [x] 公式 — 全部保留并解释
- [x] 数值 — 全部关键数值保留
