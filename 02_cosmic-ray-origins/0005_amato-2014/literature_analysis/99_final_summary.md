...
3. **磁场放大**：δB/$B_{0}$ ~ 10–100（X 射线窄边缘证据）；
4. **PeVatron 条件**：需 δB/$B_{0}$ ≫ 1（否则 E_max ~ GeV）；
...
3. **加速时间** τ_acc ∝ D/($u_{1}$-$u_{2}$) → 需缩短 D；
4. **PeVatron 必要条件**：δB/$B_{0}$ ≫ 1；
5. **无碰撞激波**：粒子库仑碰撞不足以热化 → 靠集体电磁不稳定性；
6. **电子-质子不平衡**：T_e ~ (m_e/m_p) T_p，即使经过 SNR 年龄；
7. **NLDSA 三大标志**：凹谱 + 前置区 + 温度降低；
8. **MFA 强流 vs 弱流**：强流区共振 Alfvén 波增长被**限制在 δB ~ $B_{0}$**（过去被高估）；
...
- 加速时间 τ_acc = 3/($u_{1}$-$u_{2}$) · [$D_{1}$/$u_{1}$ + $D_{2}$/$u_{2}$]；
- Bohm 扩散 D_B = r_L c/3；
