# 10. 参考文献、结论与进一步阅读

> 本章属于：The origin of galactic cosmic rays (Amato 2014 & Blasi 2013)
>
> 上一章：[[02_cosmic-ray-origins/0005_amato-2014/literature_analysis/09_figures_tables.md|09_figures_tables.md]]
>
> 下一章：[[02_cosmic-ray-origins/0005_amato-2014/literature_analysis/99_final_summary.md|99_final_summary.md]]

## 10.1 关键参考文献（对两篇论证起支柱作用）

### 历史奠基

| 引用 | 作用 | 是否必读 |
|---|---|---|
| Hess (1912) | 宇宙线发现 | 历史 |
| Baade & Zwicky (1934) | SN-CR 联系的最初能量学论证 | 必读 |
| Fermi (1949, 1954) | Fermi II 随机加速原始概念 | 必读 |
| Rossi (1964) | 早期综述（1960s 视角） | 参考 |

### DSA 理论基础

| 引用 | 作用 |
|---|---|
| Krymskii (1977) | DSA 首次提出 |
| Axford, Leer, Skadron (1977) | DSA 独立提出 |
| Bell (1978a, 1978b) | DSA + 波激发 |
| Blandford & Ostriker (1978) | DSA 独立提出 |
| Skilling (1975a) | 扩散-对流方程 |
| Blandford & Eichler (1987, Phys Rep 154:1) | **DSA 最经典的教科书综述** |
| Drury (1983) | 加速时间严格推导 |

### NLDSA 与 MFA

| 引用 | 作用 |
|---|---|
| Malkov & Drury (2001) | **NLDSA 最完整的综述** |
| Berezhko & Völk (1997, 2000) | 数值 NLDSA（有限差分） |
| Blasi (2002) | 半解析 NLDSA |
| Amato & Blasi (2005, 2006, 2009) | 半解析 NLDSA + MFA |
| Caprioli et al. (2008, 2009b, 2010a, 2010b, 2012) | MFA 动力学反作用、逃逸谱 |
| Bell (2004, 2005) | **Bell 非共振模式** |
| Riquelme & Spitkovsky (2009) | Bell 模式 PIC |
| Reville & Bell (2012) | 丝状不稳定性 |
| Caprioli & Spitkovsky (2013) | 丝状 hybrid 模拟 |
| Bell et al. (2013) | 逃逸 + 丝状 → 200 TeV |
| Treumann (2009) | 无碰撞激波综述 |
| Schure et al. (2012) | MFA 综述 |

### 观测

| 引用 | 作用 |
|---|---|
| Aharonian et al. (2004, 2006, 2007) | RXJ1713.7-3946 HESS |
| Giordano et al. (2012) | Tycho Fermi-LAT |
| Acciari et al. (2011) | Tycho VERITAS |
| **Ackermann et al. (2013, Science 339:807)** | **IC443/W44 $\pi^{0}$ 鼓包——里程碑** |
| Ackermann et al. (2011, Science 334:1103) | Cygnus superbubble |
| Morlino & Caprioli (2012) | Tycho 多频建模 |
| Berezhko et al. (2013) | Tycho 强子起源 + 密度不均匀 |
| Helder et al. (2010, 2011) | SNR 0509-67.5 H$\alpha$ |
| Helder et al. (2009, 2013) | RCW86 H$\alpha$ |
| Nikolić et al. (2013) | SN1006 H$\alpha$ |
| Ghavamian et al. (2013) | Balmer 线综述 |
| Heng (2010) | Balmer 线作为 CR 量热器理论 |
| Morlino et al. (2012, 2013c) | 部分电离介质 NLDSA |
| Blasi et al. (2012) | 部分电离介质测试粒子 |

### 宇宙线测量

| 引用 | 作用 |
|---|---|
| Adriani et al. (2011) | PAMELA 质子/氦硬化 |
| Aguilar et al. (2013) | AMS-02 正电子分数 |
| Adriani et al. (2009) | PAMELA 正电子分数 |
| Apel et al. (2013) | KASCADE-Grande 轻核踝状结构 |
| Aartsen et al. (2013) | ICETOP 组成测量 |
| Gaisser et al. (2013) | 膝区模型综述 |

## 10.2 Blasi §8 结论（关键要点）

**[FACT]** Blasi 结尾强调：
1. **CR 起源 = 加速 + 逃逸 + 传播** 三重卷积，每层非线性；
2. **SNR 范式**有足够间接证据：$\gamma$ 射线强子示踪、MFA 磁边、H$\alpha$ 异常；
3. **膝区问题**仍开放——单个 SNR 加速到膝尚无直接证据；
4. **MFA 微观机制**尚不完整——特别是 PeV 粒子散射的**尺度**问题；
5. **谱斜率张力**——NLDSA 硬谱 vs 观测陡谱 vs 各向异性约束；
6. **未来关键实验**：CTA（高分辨率 $\gamma$ 射线）、AMS-02/PAMELA 后续数据、Balmer 线高精度测量。

## 10.3 Amato §10 结论（关键要点）

**[FACT]** Amato 结论：
1. SNR 范式"间接证据不断累积，但 direct proof 仍缺"；
2. MFA + NLDSA 的核心框架仍成立，但需**修订**（散射中心速度、强流区增长率）；
3. **新数据**（PAMELA 硬化、AMS-02 正电子分数）指向新物理；
4. 正电子分数上升：暗物质 vs 脉冲星，**尚无定论**；
5. Balmer 线是"CR 量热器"——**最有望的定量 $\xi_{\rm CR}$ 测量工具**，但仍需完备观测。

## 10.4 两篇的合流观点

**[FACT]** 两篇一致认同：
- SNR 是**最可能**的银河系 CR 来源；
- DSA + MFA 是**核心机制**；
- $\xi_{\rm CR}$ ~ 10% 是**能量学必需**；
- 单一 SNR 加速到膝**未被直接证明**；
- **理论预测的硬谱与观测的陡谱**存在系统性张力，是当前最大未解问题；
- 未来 CTA、高分辨率 Balmer 线、Galactic $\gamma$ 射线空间分布是**关键突破口**。

## 10.5 建议进一步阅读

| 主题 | 文献 |
|---|---|
| DSA 完整综述 | Blandford & Eichler 1987 (Phys Rep 154:1) |
| NLDSA 完整综述 | Malkov & Drury 2001 (Reports on Progress in Physics 64) |
| MFA 综述 | Schure, Bell, O'C Drury, Biermann 2012 |
| SNR $\gamma$ 射线综述 | Aharonian 2013 (Astropart Phys 43:71) |
| Balmer 线综述 | Ghavamian, Ray, van Adelsberg 2013 |
| CR 传播综述 | Strong, Moskalenko, Ptuskin 2007 (ARAA 45:57) |
| 膝区综述 | Gaisser, Stanev, Seckel 2016; Hörandel 2006 |
| 正电子/反质子综述 | Serpico 2012 |
| CR 数据库 | Maurin et al. 2013 (Cosmic Ray Database) |
| 银河系 CR 传播工具 | GALPROP (Strong et al.), DRAGON (Gaggero et al.) |