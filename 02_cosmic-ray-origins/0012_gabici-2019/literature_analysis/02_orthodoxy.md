...
**γ 射线证据**（银河系起源的直接验证）：
- 银河系气体盘在 GeV 域发出弥散 γ 射线 → π⁰ 衰变 → CR 与气体相互作用
...
**驻留时间** $\tau_{\rm esc}$ 的提取：通过 B/C 次级/初级比值，结合 spallation 截面，推出 grammage $\Lambda \approx 10$ g/cm² → $\tau_{\rm esc} \approx$ few Myr
...
**¹⁰Be 放射性同位素验证**：
- ¹⁰Be 半衰期 $\tau(¹⁰Be) \approx 1.4$ Myr
- 观测到的 ¹⁰Be/Be 比值相对产生比值抑制 $\sim \tau(¹⁰Be)/\tau_{\rm esc}$
...
**能量依赖**：$D(R) = D_0 (R/R_0)^\delta$，最佳拟合 $\delta = 0.3$–$0.6$，$D_0 \approx 10^{28}$–$10^{29}$ cm²/s
...
**预测**：若 SNR 将 $\sim 10\%$ 动能转为加速粒子（$E^{-2}$ 谱），则可几乎模型无关地预测 SNR 的 γ 射线光度（仅依赖 $n_{\rm ISM} \approx 0.1$–$1$ cm⁻³）。
...
> "pion bump"（π⁰ 衰变谱在 $E_\gamma = m_{\pi^0}/2 = 67.5$ MeV 处的峰）常被错误引用为 SNR 中强子 CR 的"证据"。
