...
## 4.13 SNR 演化阶段与 CR 修改激波的非线性结构（从 fulltext 补充）

### 4.13.1 SNR 演化的四个阶段

[FACT] B&E §4 隐含地使用了 SNR 的演化阶段，但对其与 DSA 的关系讨论不足。一个完整的 SNR 演化包括：① **自由膨胀阶段**（Free expansion，$t < 300$ yr）：激波速度 $u_{\rm sh} \sim 10^4$ km/s 不变，CR 加速效率低；② **Sedov-Taylor 阶段**（$300 < t < 10^4$ yr）：激波半径 $R \propto t^{2/5}$，$u_{\rm sh} \propto t^{-3/5}$，DSA 加速效率提高；③ ** radiative 阶段**（$t > 10^4$ yr）：激波冷却主导，激波结构变为 radiative 激波，DSA 加速基本停止；④ **分子云碰撞阶段**：SNR 与分子云碰撞产生二次加速（reacceleration），可以提升低能 CR 的能量。对 SNR DSA 加速，Sedov-Taylor 阶段是最重要的——这个阶段的激波参数（$u_{\rm sh}$、$B$、$t$）最适合 PeV CR 加速。[FACT]

[INTERPRETATION] SNR 演化阶段对 DSA 的影响：① **能量依赖的加速时间**：不同能量的 CR 在 SNR 的不同演化阶段被加速——低能 CR（GeV）在早期就被加速，高能 CR（PeV）需要在 Sedov-Taylor 阶段才能达到 $E_{\rm max}$；② **$E_{\rm max}$ 的时间演化**：$E_{\rm max}(t)$ 随 SNR 年龄增加（$u_{\rm sh}$ 下降），达到某个最大值后开始下降；③ **CR 逃逸的时间依赖**：SNR 演化后期，CR 的逃逸概率增加，可能在分子云中产生二次加速。B&E 的处理假设了稳态激波，没有充分考虑 SNR 演化的时间依赖——这影响了他们对 $E_{\rm max}$ 估计的准确性。[INTERPRETATION]

[CRITIQUE] B&E 对 SNR 演化的处理有几个问题：① **他们假设了稳态激波**：但真实 SNR 的激波参数随时间变化，稳态假设只适用于 Sedov-Taylor 阶段的一个小区间；② **他们忽略了自由膨胀阶段**：这个阶段的激波速度最高（$u_{\rm sh} \sim 10^4$ km/s），对 PeV CR 加速最重要；③ **他们没有讨论 radiative 阶段**：这个阶段的 DSA 加速已经停止，但对 CR 在银河系中的传播仍有贡献。这些简化使 B&E 的 $E_{\rm max}$ 估计对某些 SNR（尤其是年轻的）可能偏低，而对另一些（尤其是年老的）可能偏高。[CRITIQUE]

### 4.13.2 CR 修改激波的两流体描述

[FACT] 在 NL-DSA 中，CR 压力 $P_{\rm CR}$ 对激波结构有显著修改：① **上游区域**：CR 压力使上游流体速度降低（$u < u_{\rm free}$），在激波前形成 CR 前兆区（precursor），前兆区长度 $L_{\rm prec} \sim P_{\rm CR}/(\rho u^2)$；② **激波面**：在 CR 前兆区末端，有一个相对较窄的耗散区（dissipation region），耗散区的压缩比可以超过 Rankine-Hugoniot 条件给出的值（$\tilde{r} > r$）；③ **下游区域**：CR 压力使下游流体温度降低（与没有 CR 压力的情况相比），因为 CR 分走了一部分动能。B&E §6 的两流体模型描述了这个结构，但没有给出前兆区精细结构的完整描述。[FACT]

[INTERPRETATION] CR 修改激波的物理图景：① **自相似行为**：当 CR 压力主导时（$P_{\rm CR} \gg P_{\rm thermal}$），激波结构变得自相似——这与普通流体激波的 Rankine-Hugoniot 跳跃条件不同；② **加速效率反馈**：CR 压力增大使激波压缩比增大，$\Delta E/E$ 增大，加速率提高——这形成正反馈，可能导致加速效率快速增长；③ **饱和机制**：当 CR 压力增大到一定程度，波-粒子相互作用的非线性效应（Bell 不稳定性、辐射损失）开始抑制 CR 加速，形成负反馈。B&E 的两流体模型部分描述了这个正反馈，但饱和机制的处理相对简化。[INTERPRETATION]

[CRITIQUE] B&E 的两流体模型的局限性：① **空间均匀性假设**：他们假设 CR 压力在 precursor 区是均匀的，但实际上 $P_{\rm CR}(x)$ 随位置变化；② **忽略热力学反馈**：CR 对热流体的影响（温度降低）反过来影响 CR 的加速（因为加速率依赖于 $u_{\rm sh}$），这个反馈在 B&E 的处理中过于简化；③ **忽略了离子注入效应**：在真实 SNR 中，离子（尤其是质子）被加速并注入 CR 群体的过程（ion injection）在两流体模型中没有被详细处理。今天的 PIC 模拟（Amato 2014, Brodw 2020）对这些精细结构有更准确的描述，B&E 的两流体模型在今天看来是过于简化的起点。[CRITIQUE]

### 4.13.3 粒子注入与离子加速的阈值

[FACT] B&E §4 提到了粒子注入问题（particle injection），但没有给出详细推导。离子注入阈值由以下条件决定：① **能量阈值**：离子需要被热碰撞加热到一定能量（$E_{\rm th} \sim 1-10$ keV 对质子），才能进入 DSA 加速；② **镜子比阈值**：在激波面上，磁镜比 $\Delta B/B$ 决定了有多少离子能够被散射回上游（而不是穿透激波下游）；③ **电荷态**：对多电荷离子（$Z > 1$），电离态影响 gyroradius 和共振条件，导致注入阈值不同（重核的注入阈值通常低于质子）。[FACT]

[INTERPRETATION] 注入问题对 DSA 的重要性：① **CR 丰度解释**：CR 中重核相对于 H 的丰度（与源丰度不同）与离子注入阈值有关——如果重核的注入比质子更容易，这可能解释某些观测到的 CR 丰度异常；② **加速效率**：注入率 $\eta_{\rm inj}$ 决定了有多少离子被加速，直接影响 CR 的加速效率 $\eta_{\rm CR} \propto \eta_{\rm inj}$；③ **非线性耦合**：注入过程与 CR 压力的增长是耦合的——初始注入的 CR 产生 CR 压力，CR 压力改变激波结构，激波结构又影响后续的注入率。这个耦合在 NL-DSA 中必须自洽处理，但 B&E 的处理相对简化。[INTERPRETATION]

[CRITIQUE] B&E 对注入问题的处理有几点不足：① **他们假设了热离子**：没有讨论离子如何从热群体（thermal pool）被"挑选"进入 DSA 加速；② **没有讨论电离态效应**：对重核，电离态（$Z$）影响 gyroradius 和共振条件，进而影响注入阈值——这在 B&E 的处理中被忽略了；③ **注入阈值的不确定性**：实际 SNR 中的注入阈值依赖于很多微观过程（库仑碰撞、波-粒子相互作用），这些过程在两流体模型中没有详细描述。今天的研究（Monte Carlo 模拟、PIC 模拟）对注入过程有更细致的处理，但注入问题仍然是 NL-DSA 中的一个主要不确定性来源。B&E 对注入问题的简化处理影响了他们对加速效率 $\eta_{\rm CR}$ 的精确预言能力。[CRITIQUE]
