...
## 6.19 NL-DSA 与辐射自洽模型（从 fulltext 补充）

### 6.19.1 为什么 DSA 预言需要与辐射模型耦合

[FACT] DSA 理论预言（如 $q$、$E_{\rm max}$、$\epsilon_{\rm CR}$）必须与辐射模型耦合才能与观测对比：① **辐射机制决定观测信号**：DSA 加速的 CR 通过同步辐射（电子）、逆康普顿散射（电子）和核子-核子碰撞（核子）产生可观测辐射；② **辐射过程反向约束 DSA 参数**：通过拟合观测的辐射能谱和空间分布，可以约束 DSA 参数（如 $E_{\rm max}$、$\epsilon_{\rm CR}$、$B$ 等）；③ **自洽模型的要求**：真实的 DSA + 辐射模型必须是自洽的——DSA 参数决定辐射，辐射过程（如同步冷却）又反过来影响 DSA（如冷却导致电子谱截断）。B&E 的处理将 DSA 和辐射分开讨论，没有给出自洽耦合的方法。[FACT]

[INTERPRETATION] DSA-辐射耦合的物理逻辑：① **冷却对电子谱的影响**：高能电子在同步辐射中损失能量，导致电子谱在 $E_e \gtrsim 10$ TeV 时偏离 DSA 源谱——冷却截断的形状（指数 vs super-exponential）携带了 SNR 年龄和磁场的信息；② **γ射线的核子-轻子混合**：真实 SNR 的 γ射线辐射同时包含核子过程（π$^0$ 衰变）和轻子过程（IC + 同步）——观测到的 γ射线谱是两者的混合，需要模型拟合来分离；③ **多波段一致性的要求**：好的 DSA-辐射模型应该在所有观测波段（射电、X射线、γ射线）同时一致，而不仅仅是某一个波段。B&E 的 DSA 框架是辐射模型的理论输入，而今天的多波段观测要求 DSA 和辐射模型必须作为统一整体来拟合。[INTERPRETATION]

[CRITIQUE] B&E 没有讨论 DSA-辐射耦合的实际困难：① **参数简并**：不同的 DSA 参数组合可以产生相同的辐射谱——打破简并需要多波段数据或额外的诊断（如空间分布）；② **系统误差的传递**：DSA 参数的不确定性（如 $E_{\rm max}$ 的误差）通过辐射模型传递给最终结论——这种误差传递在 B&E 的处理中被完全忽略；③ **时间演化的复杂性**：SNR 演化过程中，DSA 参数和辐射特征都在变化——静态 DSA 模型（如 B&E 的框架）无法描述这个演化。今天的 DSA-辐射拟合使用贝叶斯方法或嵌套采样来量化参数不确定性，但这些方法在 B&E 1987 年还不存在——他们的定性框架是这些定量方法的前身，但两者之间有巨大的方法论鸿沟。[CRITIQUE]

### 6.19.2 NL-DSA 的辐射特征：与观测对比的关键预言

[FACT] NL-DSA 与 test-particle DSA 在辐射预言上有几个关键差异：① **谱形差异**：NL-DSA 预言的 CR 源谱在 $E \lesssim E_{\rm max}$ 区间变平（$q_{\rm NL} < q_{\rm TP}$），这个变平在 γ射线谱中也应该被观察到；② **空间分布差异**：NL-DSA 中 CR 压力在前兆区的分布不均匀，导致 γ射线辐射的空间分布也不是均匀的——观测的空间分布可以区分 NL-DSA 和 test-particle DSA；③ **时间演化差异**：NL-DSA 的 $E_{\rm max}$ 随 SNR 演化增长（$E_{\rm max} \propto t^{1/2}$），这意味着 γ射线谱的截断能量应该随 SNR 年龄演化。[FACT]

[INTERPRETATION] 这些 NL-DSA 辐射特征与当前观测的对比：① **HESS 对年轻 SNR 的观测**：HESS 观测到某些年轻 SNR（如 RX J1713.7-3946）的 TeV γ射线谱明显偏离 test-particle DSA 预言——这支持了 NL-DSA 的框架；② **老年 SNR 的γ射线**：老年 SNR 的 TeV γ射线辐射通常较弱，与 NL-DSA 的高效率预言不完全一致——可能反映了 NL-DSA 在老年阶段的饱和效应；③ **LHAASO 的 PeVatron 候选**：LHAASO 探测到的一些 PeV 级 γ射线源暗示 $E_{\rm max}$ 可以超过 PeV——这与 NL-DSA 的 $E_{\rm max}$ 增长机制一致。B&E 的 NL-DSA 讨论是 1987 年的理论预言，这些预言在今天正在被观测验证——但验证仍然不完整，某些观测与理论预言存在显著偏差。[INTERPRETATION]

[CRITIQUE] NL-DSA 辐射预言面临几个关键困难：① **NL-DSA 的数值不确定性**：NL-DSA 的具体预言依赖于许多未知的微观参数（如散射率、注入率），不同数值模型给出不同的辐射预言；② **观测的系统误差**：γ射线观测的系统误差（能量标定、背景建模）可能掩盖 NL-DSA 的特征；③ **简并问题**：NL-DSA 和其他模型（如 test-particle DSA + 特殊传播效应）可以产生相似的辐射谱，区分它们需要更多诊断。B&E 的 NL-DSA 框架是今天数值模型的理论基础，但具体预言和观测的对比仍然在发展中——B&E 的定性框架是正确的，但定量预言的不确定性仍然很大。[CRITIQUE]