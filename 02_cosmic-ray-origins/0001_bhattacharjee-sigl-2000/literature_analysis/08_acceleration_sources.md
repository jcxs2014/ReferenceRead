...
```
E_c ≡ E_{q=3} ~ $10^{17}$ · Z · (R/kpc) · (B/μG)  eV
```
- 假设 B 平行激波法线。

**倾斜磁场额外漂移加速 (公式 50)**：
```
E_max = Ze $u_{1}$ B R ~ $10^{18}$ · Z $u_{1}$ · (R/kpc) · (B/μG)  eV
```
...
2. EGRET diffuse γ-ray 能量密度 ~ E⁻² 质子注入谱到 $10^{20}$ eV 所需能量 → 支持河外质子加速。
...
- [218]：比较 UHECR 通量 (E ~ $10^{19}$ eV) → 对 diffuse ν 通量给出更强上界。
...
1. **加速时间 < 膨胀时间** (公式 53)：
```
B >~ (E/erg) ≃ 3×$10^{4}$ · (E/$10^{20}$ eV) · (r_d/$10^{13}$ cm)⁻¹  G
```
2. **π 产生损失 < 加速** (公式 54)：
```
B >~ 20 · (L_γ/$10^{51}$ erg/s) · (r_d/$10^{13}$ cm)⁻² · (γ/300)⁻²  G
```
3. **同步损失 < 加速** (公式 55)：
```
B <~ 3×$10^{5}$ · (γ/300)² · (E/$10^{20}$ eV)⁻²
```

**同时满足**：需 r_d > $10^{12}$ (γ/300)⁻² (E/$10^{20}$ eV)³ cm（公式 56），以及 **γ > 40 (E/$10^{20}$ eV)^(3/4) (t_GRB/s)⁻^(1/4)**。

### 8.5.4 次级产物 [FACT]

- **ν 通量**：~$10^{14}$ eV，数十事件/km³ 中微子望远镜 [365,370]；> $10^{19}$ eV 可被 AIRWATCH/MASS 探测 [366]。
- **MACRO 上界**：每个 GRB 0.87×$10^{-9}$ cm⁻² μ 通量 [372]。
- **同步辐射信号**：~1% 总 burst 能量在 10 MeV（GLAST 可探测）；数百 GeV（IACT 可探测）；~TeV afterglow [367–369]。
- 若 GeV cascade 解释 diffuse γ-ray → 每个 GRB 释放 ~$10^{56}$ erg，γ > 500。
...
| 量 | 值 |
|---|---|
| DSAM E_c (benchmark, 公式 49) | $10^{17}$ Z (R/kpc)(B/μG) eV |
| 漂移加速 E_max (公式 50) | $10^{18}$ Z $u_{1}$ (R/kpc)(B/μG) eV |
| Hillas 判据 (公式 51) | B(μG)·R(kpc) > 2·(E/$10^{18}$ eV)/(Zβ) |
| AGN 核心 E_c | ~$10^{19}$ eV（但损失使可逃出 < $10^{16}$ eV） |
| FR-II hot spot E_max | 可达 ~$10^{21}$ eV（若 B 足够） |
| GRB 条件 γ > | 40 (E/$10^{20}$ eV)^(3/4) (t/s)⁻^(1/4) |
