...
- 预言谱与观测谱 E > ~$10^{19}$ eV 一致（质子注入 E$^{-2.3}$±⁰·⁵ [288]，Fermi 加速）。
- **主要问题**：GZK 距离内 (< 50 Mpc) 宇宙学 GRB 率 ~**每世纪一次** → 观测 UHECR 时间窗内概率极低，除非磁场导致 > 百年时间延迟。

### 8.5.2 磁场下限 (公式 52) [FACT]

```
B >~ $10^{-10}$ · (E/$10^{20}$ eV) · (d/30 Mpc)$^{-1}$ · (l_c/1 Mpc)$^{-}$^(1/2)  G
```
...
1. **加速时间 < 膨胀时间** (公式 53)：
```
B >~ (E/erg) ≃ $3\times10^{4}$ · (E/$10^{20}$ eV) · (r_d/$10^{13}$ cm)$^{-1}$  G
```
2. **$\pi$ 产生损失 < 加速** (公式 54)：
```
B >~ 20 · (L_$\gamma$/$10^{51}$ erg/s) · (r_d/$10^{13}$ cm)$^{-2}$ · ($\gamma$/300)$^{-2}$  G
```
3. **同步损失 < 加速** (公式 55)：
```
B <~ $3\times10^{5}$ · ($\gamma$/300)$^{2}$ · (E/$10^{20}$ eV)$^{-2}$
```

**同时满足**：需 r_d > $10^{12}$ ($\gamma$/300)$^{-2}$ (E/$10^{20}$ eV)$^{3}$ cm（公式 56），以及 **$\gamma$ > 40 (E/$10^{20}$ eV)^(3/4) (t_GRB/s)$^{-}$^(1/4)**。
