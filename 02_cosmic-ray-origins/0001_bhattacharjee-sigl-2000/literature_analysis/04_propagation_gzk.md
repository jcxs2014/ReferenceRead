...
```
l(E)⁻¹ = ∫d$\epsilon$ · n_b($\epsilon$) · ∫₋₁⁺¹ d$\mu$ · (1 − $\mu$$\beta$$\beta$_b)/2 · $\sigma$(s)
```
- n_b($\epsilon$)：背景粒子单位能量数密度
- $\beta$_b = (1 − m_b²/$\epsilon$²)^(1/2)
- $\beta$ = (1 − m²/E²)^(1/2)
- $\mu$：入射动量夹角余弦
- $\sigma$(s)：总截面
- **质心能量平方** (公式 8)：
```
s = m_b² + m² + 2$\epsilon$E(1 − $\mu$$\beta$$\beta$_b)
```
...
```
E_lab,thr^$\gamma$ ≡ m_$\pi$ + m_$\pi$²/(2m_N) ≃ 160 MeV
```

对应质子阈值（对背景光子 $\epsilon$，公式 13）：
```
E_th = m_$\pi$(m_N + m_$\pi$/2)/$\epsilon$ ≃ 6.8 × $10^{16}$ · ($\epsilon$ / eV)⁻¹  eV
```
- CMB 典型 $\epsilon$ ~ $10^{-3}$ eV → **GZK cutoff** 出现在"数十 EeV"处（即 ~5×$10^{19}$ eV），此时质子相互作用长度降至 **~6 Mpc**。

### 4.3.2 截面特征 [FACT, Fig. 8]

- 阈值附近：显著的 $\Delta$(1232) **单 $\pi$ 共振**。
- 高能极限：$\sigma$ 随 s 对数增长。
- 第一个共振之后：由**多重 $\pi$ 产生** N $\gamma$_b → N(n$\pi$), n > 1 主导。

### 4.3.3 其他损失机制

**质子-对产生 (PPP, p $\gamma$_b → p e⁺ e⁻)** [FACT, 公式 14]
```
E_th = m_e(m_N + m_e)/$\epsilon$ ≃ 4.8 × $10^{14}$ · ($\epsilon$ / eV)⁻¹  eV
```
- CMB 中 PPP 发生于 E ~ 5×$10^{17}$ eV。
...
**中子 $\beta$-衰变 (n → p e⁻ $\nu$̄_e)** [FACT, 公式 15]
```
R_n = $\tau$_n · E/m_N ≃ 0.9 · (E / $10^{20}$ eV)  Mpc
```
- $\tau$_n = 888.6 ± 3.5 s
...
- 最新 MC 模拟 [167–169]：降低 IRB 后，CMB 成为主导 → **E ~ 2×$10^{20}$ eV 处衰减长度 ~10 Mpc**。
...
- **Pair Production (PP)**：$\gamma$ $\gamma$_b → e⁺ e⁻
- **Inverse Compton Scattering (ICS)**：e e⁺ + $\gamma$_b → 更高能 $\gamma$

阈值（公式 16）：
```
E_th = m_e²/$\epsilon$ ≃ 2.6 × $10^{11}$ · ($\epsilon$ / eV)⁻¹  eV
```

高能极限截面（公式 17）：
```
$\sigma$_PP ≃ 2 $\sigma$_ICS ≃ (3/2) $\sigma$_T · (m_e²/s) · ln(s/(2m_e²))   (s ≫ m_e²)
```
...
**双对产生 (DPP, $\gamma$ $\gamma$_b → e⁺e⁻e⁺e⁻)** — 公式 18：
```
$\sigma$_DPP ≃ 172$\alpha$⁴/(36$\pi$m_e²) ≃ 6.45 $\mu$barn   (s ≫ m_e²)
```
- DPP 主导 PP 的能区：**E > $10^{21}$–$10^{23}$ eV**（强 URB 时取较高值）。

**三对产生 (TPP, e $\gamma$_b → e e⁺e⁻)** — 公式 19, 20：
```
$\sigma$_TPP ≃ (3$\alpha$/(8$\pi$)) $\sigma$_T · (28/9 ln(s/m_e²) − 218/27)
$\eta$ ≃ 1.768 · (s/m_e²)^(−3/4)   (s ≫ m_e²)
```
- 尽管 $\sigma$_TPP ~ $\sigma$_ICS 在 E ~ $10^{17}$ eV 就已可比，但因 $\eta$ ≲ $10^{-3}$，能量衰减直到 **~$10^{22}$ eV** 才重要。
...
- 双 Compton 散射（$\alpha$³ 阶，UHE 下 <10%）
- $\gamma$ $\gamma$_b → $\gamma$ $\gamma$（z > ~100 时才重要）
- Bethe-Heitler 对产生
...
| GZK 质子阈值 | ~6.8×$10^{19}$ eV |
| GZK 质子相互作用长度 | ~6 Mpc |
| PPP 质子阈值 | ~5×$10^{17}$ eV |
| 中子衰变范围 | 0.9 Mpc (E=$10^{20}$ eV) |
| 核光致分裂长度 (2×$10^{20}$ eV) | ~10 Mpc (低 IRB) |
| UHE $\gamma$ PP 阈值 | 2.6×$10^{11}$ ($\epsilon$/eV)⁻¹ eV |
...
- [FACT] 2017 年 Pierre Auger 合作组首次确认 E > 5×$10^{19}$ eV 处**能谱陡化**（与 GZK 预期一致）[Nature 551, 56–62 (2017)]。
