...
对物质主导平坦宇宙 (Ω₀ = 1)：
```
j(E) = (3/(8π$t_{0}$)) · ∫₀^{z_i,max} dz_i · (1+z_i)⁻^(11/2) · (dE_i(E,z_i)/dE) · Φ(E, z_i)
```
- $t_{0}$：宇宙年龄
...
```
E_th = m_π(m_N + m_π/2)/ε ≃ 6.8 × $10^{16}$ · (ε / eV)⁻¹  eV
```
- CMB 典型 ε ~ $10^{-3}$ eV → **GZK cutoff** 出现在"数十 EeV"处（即 ~5×$10^{19}$ eV），此时质子相互作用长度降至 **~6 Mpc**。
...
```
E_th = m_e(m_N + m_e)/ε ≃ 4.8 × $10^{14}$ · (ε / eV)⁻¹  eV
```
...
```
R_n = τ_n · E/m_N ≃ 0.9 · (E / $10^{20}$ eV)  Mpc
```
- τ_n = 888.6 ± 3.5 s
...
```
- 尽管 σ_TPP ~ σ_ICS 在 E ~ $10^{17}$ eV 就已可比，但因 η ≲ $10^{-3}$，能量衰减直到 **~$10^{22}$ eV** 才重要。
...
- Bethe-Heitler 对产生
- γ B → e⁺e⁻（银河系强度 ~$10^{-6}$ G 需 E > ~$10^{24}$ eV；EGMF 下更高）
