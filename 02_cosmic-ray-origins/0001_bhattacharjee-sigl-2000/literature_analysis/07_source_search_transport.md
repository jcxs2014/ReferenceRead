...
```
3×$10^{3}$ (Δθ/1°)² (d/10 Mpc) yr ≲ T_S ≃ τ_E ≲ $10^{4}$ – $10^{7}$ (E/100 EeV)⁻² yr
```
...
c²k² ≃ E² + χ E³/$E_{0}$
```
- 对应光子群速度 ∂E/∂k = c[1 − χ E/$E_{0}$ + O(E²/$E_{0}$²)]
- χ = ±1；$E_{0}$ = 量子引力尺度
- 阈值 (公式 42)：
```
ε ≃ E/4 · [m_e²/($E_{1}$$E_{2}$ + θ₁θ₂)] + χ E²/(4$E_{0}$)
```

**两种情形**：
- **χ < 0 (c_γ > c_p)**：E > E_c 时光子可自发衰变 → 河外光子传播受阻 → 观测 > 20 TeV 河外光子 [328,329] 约束 **$E_{0}$ >~ M_Pl** 或 (c²_i − c²) > ~ −2×$10^{-17}$。
- **χ > 0**：E > E_c 时 ε 增长 → 光子传播不受阻 → 可观测 > 100 TeV 来自 > 100 Mpc 的河外光子。

**时间色散 (公式)**：
```
Δt ≃ (d/c)(E/$E_{0}$) ≃ 1 (d/100 Mpc)(E/TeV)($E_{0}$/M_Pl)⁻¹ s
```
- Mrk 421 > 2 TeV γ 在 300 s 内到达 → $E_{0}$ > 4×$10^{16}$ GeV [331]。
- HEGRA 若观测到 > 200 TeV GRB γ 在 200 s 内 → $E_{0}$ ≃ M_Pl。

### 7.3.3 快子中微子 [FACT]

- Kostelecký [333]：ν_e 为 tachyon。
- 核内质子可衰变 p → n + e⁺ + ν_e，阈值 Eth = m(A,Z)[m(A,Z±1)+m_e−m(A,Z)]/|m_νe|
- 自由质子：Eth ≃ 1.7×$10^{15}$/(|m_νe|/eV) eV。
