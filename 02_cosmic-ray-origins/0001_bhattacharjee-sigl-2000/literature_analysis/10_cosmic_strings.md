...
- **核心结论**：GUT 尺度弦 (~10¹⁶ GeV) 在多数过程中 X 产额不足；但若为 ~10¹³–10¹⁴ GeV 的"轻"弦，可解释 EHECR 且不违反级联 γ 限制。
...
2. **摩擦主导期**（T > (Gμ)^(1/2) η）：
   - 弯曲弦段达终端速度 ∝ 1/r。
   - 弦被拉直、长度缩短 → ξ_s 增加 → ρ_s 下降。
3. **相对论期**：摩擦可忽略。
   - 情形(a) **Scaling 解**：ξ_s/t = const → ρ_s,scaling ∝ 1/t²
   - 情形(b) ξ_s 增长慢于 t → 弦过早主导宇宙（被排除）。

### 10.2.2 Scaling 能量损失 [FACT]

```
ρ̇_s,total = −2(Ṙ/R) ρ_s + ρ̇_s,loss         (71)
```
- 辐射主导：ρ̇_s,loss = −ρ_s/t
- 物质主导：ρ̇_s,loss = −(2/3) ρ_s/t

### 10.2.3 环形成与小尺度结构 [FACT]

- **早期数值模拟**：大 (horizon-size) 环形成。
- **高分辨率模拟**：大量小尺度结构（kinks） → 环尺寸远小于 horizon。
- **稳定尺度 ζ**：gravitational radiation 反作用稳定化 → ζ ~ ΓGμt（Γ ~ 100）。

### 10.2.4 Scaling 弦的 loop distribution (公式 72–76) [FACT]

- Scaling 解：ρ_s = μ/(x t)², x ∈ [0.3–0.7]（近期模拟 x ~ 0.3）。
- 环长度：L_b = K ζ(t_b) = K Γ Gμ t_b （公式 72）
- 环诞生率（物质主导，公式 73）：
```
dn_b/dt = (2/(3 x²)) (Γ Gμ)⁻¹ K⁻¹ t⁻⁴
```
- 环长度分布（公式 76）：
```
dn/dL (L, t) = [2/(3 x²)] (K+1)/K · 1/(t² (L + Γ Gμ t)²), L ≤ KΓGμt
```

### 10.2.5 现代 loop 数量与尺度 (Gμ = 10⁻⁶ 时) [FACT]

| 量 | 值 |
|---|---|
| 最丰环长度 | ~200 (Gμ/10⁻⁶)(Ω₀h²)^(−1/2) kpc |
| 数密度 | ~4.6×10⁻⁶ (Gμ/10⁻⁶)⁻¹(Ω₀h²)^(3/2) Mpc⁻³ |
| 典型间隔 | ~60 (Gμ/10⁻⁶)^(1/3)(Ω₀h²)^(−1/2) Mpc |

## 10.3 §6.4.2 Intercommuting 过程

### 10.3.1 机制 [FACT]

- 两条弦段交叉 → 重叠长度 ~w ~ η⁻¹ → Higgs 相未定义 → 拓扑去除事件。
- 释放能量 ~μw ~ η → 每个 intercommuting 释放 ~1 个 X 粒子。
...
ℓ_c ~ (L w)^(1/2)
```
比旧估计小 (w/L)^(1/6) → 对 GUT 尺度字符串 (~10¹⁶ GeV)，w/L 为天文数字小量 → **ℓ_c 被严重压低**。

### 10.4.3 X 粒子数 [FACT]

- 单次 cusp 事件：~(ηζ)^(2/3)（长弦）或 ~(ηL)^(2/3)（闭环）。
- 对 GUT 尺度：η ~ 10¹⁶ GeV，但 L/w ~ 极大 → cusp 效率低。
...
- 或反复自相交分裂为越来越小的子环 → debris of tiny loops（~η⁻¹ 尺度）→ X 粒子。
- **碎裂时间**：τ_debris ~ L ≪ τ_grav ~ (ΓGμ)⁻¹L

### 10.5.2 速率 [FACT, 公式 84]

假设 f_X 的环能量转化为 X：
```
ṅ_X(t) = f_X · μ/m_X · (2/(3 x²)) t⁻³          (84)
```
...
f_X · η₁₆^(3/2) ≃ 2.8×10⁻⁵            (85)
```
（η₁₆ = η/10¹⁶ GeV, l(E_γ=300 EeV) = 50 Mpc）

### 10.5.4 级联 γ 约束 (公式 86–88) [FACT]

- 注入 EHECR 能量的 EM 级联：
```
ω_cas ≃ (1/2) m_X ṅ_X t₀            (86)
ω_cas ≤ 2×10⁻⁶ eV cm⁻³              (87, [185])
```
- 导出（公式 88）：
```
f_X · η₁₆² ≤ 9.6×10⁻⁶              (88)
...
联立 (85)+(88)：
```
9.2×10¹² GeV <~ η <~ 1.2×10¹⁵ GeV
f_X ~ 2.8×10⁻⁵ η₁₆^(−3/2)
f_X ∈ [6.7×10⁻⁴, 1]
```
- **m_X ≫ 10¹⁵ GeV 时 GUT-scale 弦难以同时满足**（违反 γ 背景约束）。
- **η ~ 10¹³ GeV 的"轻"弦**是自然候选。

### 10.5.6 数值模拟支持 [FACT]

- **Siemens-Kibble [431]**：环自相交概率随谐波数**指数增长**。
- Kinks 是高谐波配置 → 有 kink 的环高概率自相交 → f_X ~ few×10⁻⁴ 是**合理的**。
...
- Bhattacharjee-Sigl [432]：f_X ~ 1 时（全部环碎裂为 X），若 η ≲ 3.1×10¹³ GeV 且 f_KE ≲ few%，可解释 EHECR 且不违反 γ 约束。

### 10.5.7 轻弦的额外优势 [FACT]

- Loop 数密度 ∝ (Gμ)⁻¹ → η = 10¹³ GeV 时（Gμ ~ 10⁻¹²），与 η = 10¹⁶ GeV 相比 loop 数密度**大 10⁶ 倍**。
- GZK 半径 (50 Mpc) 内：
  - GUT 弦 (~10¹⁶ GeV)：仅 ~2.4(Ω₀h²)^(3/2) 个 loop。
  - 轻弦 (~10¹³ GeV)：~ 2.4×10⁶ 个 loop。
- **结论**：轻弦 (η ~ few×10¹³ GeV) 通过快速多重碎裂过程可**自然产生足够多 X 粒子**。
...
- **若 [417] 正确**：ṅ_X = (84) with f_X = 1 → 与 (68) 对比 → η ≲ 10¹³ GeV（否则会超量产生 EHECR）。
  - → **GUT-scale 弦被排除**。
...
- **临界电流**：
  ...
  - **上限**：J_c ≤ J_max ≃ e η（两种情形）。
- 弦外**X 粒子质量**：m_F = g η（g ~ Yukawa 耦合），可达 GUT 尺度 ~10¹⁶ GeV。

### 10.7.2 OTW-SCS 演化场景 [FACT]

- **Ostriker-Thompson-Witten [421]**：原初磁场随宇宙膨胀 → 通过弦环的磁通变化 → 弦环上感应电流。
- 环振荡 → 电磁辐射 + 引力辐射损失 → 环缩小 → 电流 J ∝ L⁻¹ 增加 → 达到 J_c = J_s。
- 饱和环释放 X 粒子。

### 10.7.3 X 粒子发射率 [FACT, 公式 89–90]

**饱和环上费米子总数**：N_F = (m_F/π) L

**电磁辐射主导** (Gμ ≲ 10⁻⁸ 或 g > 10(Gμ)^(1/2))：
```
Ṅ_F = (4/π²) α_em γ_em g³ η            (89)
γ_em ≃ 100
```
...
**条件**（产生 EHECR）：m_F = gη ≥ 10¹² GeV → η ≥ 10¹² GeV → Gμ ≥ 10⁻¹⁴，且 10⁻⁷(Gμ)⁻^(1/2) ≤ g ≤ 1。
...
| 量 | 值 |
|---|---|
| GUT 弦 μ | ~ (10¹⁶ GeV)² ~ 10⁻⁶ M_Pl² |
| Scaling 解 x | 0.3–0.7（近期 ~0.3） |
| 小尺度结构 ζ | ~ ΓGμt, Γ ~ 100 |
| 现代 loop 长度 | ~200 (Gμ/10⁻⁶) kpc |
| Intercommuting X 数/事件 | ~1 |
| Cusp 长度 ℓ_c (旧) | ~ ζ^(2/3) w^(1/3) |
| Cusp 长度 ℓ_c (新 [425]) | ~ (Lw)^(1/2) |
| Loop 碎裂 EHECR 条件 | f_X η₁₆^(3/2) ≃ 2.8×10⁻⁵ |
| γ 背景约束 | f_X η₁₆² ≤ 9.6×10⁻⁶ |
| 允许 η 窗口 | 9.2×10¹² – 1.2×10¹⁵ GeV |
| Direct emission η 上限 | ≲ 10¹³ GeV |
| SCS X 发射率 (EM 主导) | (4/π²) α_em γ_em g³ η |
| SCS X 发射率 (引力主导) | (g/π)(ΓGμ)η |
...
- [FACT] **CMB anisotropy**（WMAP/Planck 数据）：对宇宙弦网络的约束比 1999 年时强得多 → Gμ < 10⁻⁷ 量级（Planck 2015）→ 与本文 §6.4.6 讨论的 η ≲ 10¹³ GeV 一致（Gμ <~ 10⁻¹²）。
