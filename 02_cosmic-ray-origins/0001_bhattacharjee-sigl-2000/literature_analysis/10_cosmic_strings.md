...
- **核心结论**：GUT 尺度弦 (~$10^{16}$ GeV) 在多数过程中 X 产额不足；但若为 ~$10^{13}$–$10^{14}$ GeV 的"轻"弦，可解释 EHECR 且不违反级联 γ 限制。
...
### 10.2.5 现代 loop 数量与尺度 (Gμ = $10^{-6}$ 时) [FACT]

| 量 | 值 |
|---|---|
| 最丰环长度 | ~200 (Gμ/$10^{-6}$)(Ω₀h²)^(−1/2) kpc |
| 数密度 | ~4.6×$10^{-6}$ (Gμ/$10^{-6}$)⁻¹(Ω₀h²)^(3/2) Mpc⁻³ |
| 典型间隔 | ~60 (Gμ/$10^{-6}$)^(1/3)(Ω₀h²)^(−1/2) Mpc |
...
- 对 GUT 尺度：η ~ $10^{16}$ GeV，但 L/w ~ 极大 → cusp 效率低。
...
```
f_X · η₁₆^(3/2) ≃ 2.8×$10^{-5}$            (85)
```
（η₁₆ = η/$10^{16}$ GeV, l(E_γ=300 EeV) = 50 Mpc）

### 10.5.4 级联 γ 约束 (公式 86–88) [FACT]

- 注入 EHECR 能量的 EM 级联：
```
ω_cas ≃ (1/2) m_X ṅ_X $t_{0}$            (86)
ω_cas ≤ 2×$10^{-6}$ eV cm⁻³              (87, [185])
```
- 导出（公式 88）：
```
f_X · η₁₆² ≤ 9.6×$10^{-6}$              (88)
...
联立 (85)+(88)：
```
9.2×$10^{12}$ GeV <~ η <~ 1.2×$10^{15}$ GeV
f_X ~ 2.8×$10^{-5}$ η₁₆^(−3/2)
f_X ∈ [6.7×$10^{-4}$, 1]
```
- **m_X ≫ $10^{15}$ GeV 时 GUT-scale 弦难以同时满足**（违反 γ 背景约束）。
- **η ~ $10^{13}$ GeV 的"轻"弦**是自然候选。
...
- Bhattacharjee-Sigl [432]：f_X ~ 1 时（全部环碎裂为 X），若 η ≲ 3.1×$10^{13}$ GeV 且 f_KE ≲ few%，可解释 EHECR 且不违反 γ 约束。

### 10.5.7 轻弦的额外优势 [FACT]

- Loop 数密度 ∝ (Gμ)⁻¹ → η = $10^{13}$ GeV 时（Gμ ~ $10^{-12}$），与 η = $10^{16}$ GeV 相比 loop 数密度**大 $10^{6}$ 倍**。
- GZK 半径 (50 Mpc) 内：
  - GUT 弦 (~$10^{16}$ GeV)：仅 ~2.4(Ω₀h²)^(3/2) 个 loop。
  - 轻弦 (~$10^{13}$ GeV)：~ 2.4×$10^{6}$ 个 loop。
- **结论**：轻弦 (η ~ few×$10^{13}$ GeV) 通过快速多重碎裂过程可**自然产生足够多 X 粒子**。
...
- **若 [417] 正确**：ṅ_X = (84) with f_X = 1 → 与 (68) 对比 → η ≲ $10^{13}$ GeV（否则会超量产生 EHECR）。
  - → **GUT-scale 弦被排除**。
...
- 弦外**X 粒子质量**：m_F = g η（g ~ Yukawa 耦合），可达 GUT 尺度 ~$10^{16}$ GeV。
...
**条件**（产生 EHECR）：m_F = gη ≥ $10^{12}$ GeV → η ≥ $10^{12}$ GeV → Gμ ≥ $10^{-14}$，且 $10^{-7}$(Gμ)⁻^(1/2) ≤ g ≤ 1。
...
| 量 | 值 |
|---|---|
| GUT 弦 μ | ~ ($10^{16}$ GeV)² ~ $10^{-6}$ M_Pl² |
| Scaling 解 x | 0.3–0.7（近期 ~0.3） |
| 小尺度结构 ζ | ~ ΓGμt, Γ ~ 100 |
| 现代 loop 长度 | ~200 (Gμ/$10^{-6}$) kpc |
| Intercommuting X 数/事件 | ~1 |
| Cusp 长度 ℓ_c (旧) | ~ ζ^(2/3) w^(1/3) |
| Cusp 长度 ℓ_c (新 [425]) | ~ (Lw)^(1/2) |
| Loop 碎裂 EHECR 条件 | f_X η₁₆^(3/2) ≃ 2.8×$10^{-5}$ |
| γ 背景约束 | f_X η₁₆² ≤ 9.6×$10^{-6}$ |
| 允许 η 窗口 | 9.2×$10^{12}$ – 1.2×$10^{15}$ GeV |
| Direct emission η 上限 | ≲ $10^{13}$ GeV |
...
- [FACT] **CMB anisotropy**（WMAP/Planck 数据）：对宇宙弦网络的约束比 1999 年时强得多 → Gμ < $10^{-7}$ 量级（Planck 2015）→ 与本文 §6.4.6 讨论的 η ≲ $10^{13}$ GeV 一致（Gμ <~ $10^{-12}$）。
