...
```
dE/dt = −(4/3) $\sigma$_T (B²/8$\pi$) (q m_e/m)^4 (E/m_e)²
```
- UHE 质子：**可忽略**。
- UHE 电子：同步损失在转移能量
```
...
```
$\tau$(E,d) ≃ d·$\theta$²/4 ≃ 1.5×$10^{3}$ q² · (E/$10^{20}$ eV)⁻² · (d/10 Mpc)² · (l_c/1 Mpc) · (B/$10^{-9}$ G)²  yr
```
...
**修正版**（用 $\Omega$_b h² ≃ 0.02 替代 closure density，未结构宇宙）：
```
B ≲ 3×$10^{-7}$ ($\Omega$_b h²/0.02)⁻¹ (h/0.65) (l_c/Mpc)⁻^(1/2)  G
```
→ 更强偏转。
...
| 数据 | 关联对象 | 结果 |
|---|---|---|
| Haverah Park + AGASA + VR + Yakutsk | Supergalactic Plane | **~3$\sigma$** 正相关（E > 4×$10^{19}$ eV）[79] |
...
| $\tau$ (随机场) | 1.5×$10^{3}$ q² (E/$10^{20}$ eV)⁻² (d/10 Mpc)² (l_c/1 Mpc) (B/$10^{-9}$ G)² yr |
