...
```
E_syn ≃ 6.8×$10^{13}$ (E/$10^{21}$ eV)² (B/$10^{-9}$ G)  eV
```
- 经典极限下有效（E_syn ≪ E）。
- 约束来源：当 E_syn 落入已有 diffuse γ 观测窗口时：
  - EGRET ~ 1 GeV [185]
  - HEGRA 上界 50–100 TeV [257]
  - CASA-MIA 6×$10^{14}$ – 6×$10^{16}$ eV [258]
- **具体例子**：强 discrete UHE γ 源（如拓扑缺陷，近单能注入谱，EGMF ~$10^{-9}$ G）→ 在某些 E > ~$10^{16}$ eV 处预言 γ 通量 > 带电 CR 通量 → **已被排除** [259]。
...
```
θ(E,d) ≃ d/r_g ≃ 0.52° · q · (E/$10^{20}$ eV)⁻¹ · (d/1 Mpc) · (B_⊥/$10^{-9}$ G)
```
...
```
τ(E,d) ≃ d·θ²/4 ≃ 1.5×$10^{3}$ q² · (E/$10^{20}$ eV)⁻² · (d/10 Mpc)² · (l_c/1 Mpc) · (B/$10^{-9}$ G)²  yr
```

**Bursting source 效应** [FACT]：由于 τ ∝ E⁻²，观测窗口内的瞬时谱与长时间平均谱不同，在
```
τ($E_{0}$, d) ≃ (观测时刻 − 零延迟到达时刻)
```
处出现**谱峰 $E_{0}$**；高能粒子已过，低能粒子未至。

**谱宽度**：
- d < 相互作用长度 且 dθ ≪ l_c → 谱宽 ≪ $E_{0}$。
- 其他情形 → 谱宽 ~ $E_{0}$。
...
```
B ≲ 3×$10^{-7}$ (Ω_b h²/0.02)⁻¹ (h/0.65) (l_c/Mpc)⁻^(1/2)  G
```
→ 更强偏转。
...
**Fly's Eye 300 EeV 事件**（3.2×$10^{20}$ eV）[26]：
| 候选源 | 距离 | 到达方向偏离 |
|---|---|---|
| Cen A | ~3 Mpc | ~136° |
...
- 中微子假设问题：σ_νN 在 $10^{20}$ eV 大气相互作用概率 ~$10^{-5}$。
...
| 数据 | 关联对象 | 结果 |
|---|---|---|
| Haverah Park + AGASA + VR + Yakutsk | Supergalactic Plane | **~3σ** 正相关（E > 4×$10^{19}$ eV）[79] |
...
| E_syn 典型值 | 6.8×$10^{13}$ (E/$10^{21}$ eV)² (B/$10^{-9}$ G) eV |
| θ (均匀场) | 0.52° q (E/$10^{20}$ eV)⁻¹ (d/1 Mpc) (B/$10^{-9}$ G) |
| θ (随机场) | 0.8° q (E/$10^{20}$ eV)⁻¹ (d/10 Mpc)^(1/2) (l_c/1 Mpc)^(1/2) (B/$10^{-9}$ G) |
| τ (随机场) | 1.5×$10^{3}$ q² (E/$10^{20}$ eV)⁻² (d/10 Mpc)² (l_c/1 Mpc) (B/$10^{-9}$ G)² yr |
