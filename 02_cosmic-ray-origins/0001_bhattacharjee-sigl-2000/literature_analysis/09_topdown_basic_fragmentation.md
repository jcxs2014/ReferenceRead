...
>
> 上一章：[[02_cosmic-ray-origins/0001_bhattacharjee-sigl-2000/literature_analysis/08_acceleration_sources.md|08_acceleration_sources.md]]
>
> 下一章：[[02_cosmic-ray-origins/0001_bhattacharjee-sigl-2000/literature_analysis/10_cosmic_strings.md|10_cosmic_strings.md]]
...
- **$\alpha$ = 2 是分界**：
  - 软谱 ($\alpha$ > 2)：粒子数与总能量均由低能端主导。
  - **硬谱 (1 < $\alpha$ < 2)**：总能量由少数极高能粒子携带 → 更"自然"产生 EHECR。
- Top-down 一般 $\alpha$ ~ 1.3–1.5（硬）→ 预测 GZK 截断后有**"recovery"** [200]。
- 硬谱也可能自然产生 "gap" [403]。
...
- Top-down：$\nu$, $\gamma$ 是**初级**产物（直接从 $\pi$ 衰变）。
- Bottom-up：$\nu$, $\gamma$ 是**次级**（来自 GZK 相互作用产生的 $\pi$）。

## 9.5 §6.2.3 Benchmark Calculation (公式 68–69)

### 9.5.1 单 X 粒子光子注入谱 (公式 66) [FACT]

假设 X → q ℓ（2-body），dN_$\gamma$/dE_$\gamma$ ∝ E_$\gamma$^(−$\alpha$)，0 < $\alpha$ < 2：
```
dN_$\gamma$/dE_$\gamma$ = 0.6 m_X / (2 − $\alpha$) · (f_$\pi$/0.9) · (2E_$\gamma$/m_X)^(−$\alpha$)
```

### 9.5.2 光子通量 (公式 67)

```
j_$\gamma$(E_$\gamma$) ≃ (1/(4$\pi$ l(E_$\gamma$))) · ṅ_X · dN_$\gamma$/dE_$\gamma$
```
...
- 实验室验证：$^{3}{\rm He}$ 超流相变中的 vortex-filament 形成（毫开尔文温度）→ Kibble-Zurek 机制的确认 [409]。
...
| 1990s | [200] 一般参数化 [178] $\gamma$/CR 标志 |
