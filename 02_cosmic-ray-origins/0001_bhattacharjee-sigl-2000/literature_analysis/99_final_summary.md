...
- **能谱**：膝（$~3\times10^{15}$ eV）→ 踝（$~3\times10^{18}$ eV）→ GZK 区（> $10^{19.9}$ eV）。
...
- **EGMF 效应**：偏转 $\tau_{\rm E}$ ∝ E$^{-2}$（rectilinear），E$^{-1}$（Bohm），E$^{-1}$/$^{3}$（Kolmogorov 扩散）。
- **AGASA 约束**：B ≲ $2\times10^{-11}$ (l_c/Mpc)^(−1/2)(d/30 Mpc)$^{-1}$ G（式 38）。
...
**关键公式**：
- DSAM E_max：E_c ~ $10^{17}$ Z (R/kpc)(B/$\mu$G) eV（式 49）。
- X 粒子衰变率基准：ṅ_X ~ $10^{35}$ Mpc$^{-3}$ yr$^{-1}$（m_X = $10^{16}$ GeV，式 68）。
- 能量注入率基准：Q_0 ~ $10^{-21}$ eV cm$^{-3}$ s$^{-1}$（式 69）。
...
| 约束 | 限制 |
|---|---|
| **弥漫 $\gamma$-ray** (EGRET) | p = 0 完全排除；p = 1 需 q > 1.7（m_X = $10^{16}$ GeV）；$Q_{0}$ < $10^{-22}$ eV cm$^{-3}$ s$^{-1}$ |
| **BBN / 4He 光致分解** | ($^{3}{\rm He}$+D)/H < $5\times10^{-5}$（与 $\gamma$ 约束独立）|
| **CMB 畸变** | 排除 p = 0 |
| **弥漫 $\nu$ 通量** | SLBY98 ~ 0.15 yr$^{-1}$ (1 km$^{3}$ @ > $10^{19}$ eV)，模型无关上限 (式 105) |

### 2.6 Top-down 场景的核心预言

1. **EHECR 由基本粒子组成**（核子 + 光子 + 中微子），**无重核**。
2. **硬谱**（$\alpha$ ~ 1.3–1.5）→ GZK 截断后有 **"recovery"**（或 "gap"）。
3. **$\gamma$/CR > 1** 在足够高 EHECR 能区（但 EGMF 或高 URB 可压低）。
4. **弥漫 $\nu$ 通量** ~ 0.1–1 event/yr @ 1 km$^{3}$。
...
- **MSRP 作为暗物质**（$\Omega_{\rm X}$ h$^{2}$ ~ 1）→ 现代暗物质搜索（LUX, XENON, PandaX, LZ）+ Auger 相关数据 → **超重 MSRP 主导场景受到严重限制**。
- **纯光子 EHECR** → Auger 组成测量显示 EHECR 为**核子/重核混合**（非光子主导）→ 对纯 TD 光子主导场景不利。

### 3.3 仍开放的问题

- **EGMF 强度**：本文讨论的 B ≲ $10^{-9}$ G → 现代 Faraday rotation 约束在 ~$10^{-9}$ G 量级（基本一致）。
- **Z-burst 机制**：取决于 eV 质量 $\nu$ 的宇宙数密度 → 若 $\nu$ 是 hot DM → 现代宇宙学（结构形成）**对 eV 质量 $\nu$ 极敏感** → Z-burst 场景受到限制。
- **TeV-scale Higgs X**（§6.11）：Fermi-LAT 数据显示 10–100 GeV 弥漫 $\gamma$ 可能有额外成分 → 部分符合 [418] 预言，但非独特。
...
- **数值基准**：ṅ_X ~ $10^{35}$ Mpc$^{-3}$ yr$^{-1}$ 与 Q_0 ~ $10^{-21}$ eV cm$^{-3}$ s$^{-1}$ 至今仍被引用作为 benchmark。
