...
- [FACT] "Hα optical emission from Balmer dominated SNR shocks is a powerful indicator of the conditions around the shock (Chevalier and Raymond, 1978; Chevalier et al, 1980)."
...
- [FACT] **窄线**来自下游直接穿越的中性 H（宽度反映上游温度，10⁴ K 时 ~21 km/s）。
...
- [FACT] **Fig. 14 计算示例**（V_sh=4000 km/s，n=0.1 cm⁻³，p_max=50 TeV/c）：
  - 无加速时：宽线/窄线宽正常。
  - 有加速（ξ_inj=3.5）：宽线变窄，窄线变宽。
  - 湍流加热 η_TH 越大，中间线越明显（几百 km/s 宽）。
...
- [FACT] Helder 使用 van Adelsberg et al. (2008) 计算推断加速效率——但 Morlino et al. (2013a) 指出其对快激波的**中性氢分布函数假设**可能严重高估 ξ_CR。
- [FACT] SW 边缘实际速度可能 ~4000 km/s（形态学判断）——比 Helder 用的 5000 km/s 低。
- [FACT] **修正后**（Morlino et al. 2013b，Fig. 15）：若 V_sh=4000 km/s、h_N=10%、β_down≪1（快激波时电子-离子未平衡），测得 FWHM 支持 **ξ_CR ~ 10–20%**。
- [FACT] 参数 β_down = T_e/T_p 是关键不确定性：β_down=1（完全平衡）时即使 V_sh=4000 km/s，测得 FWHM 仍与无加速兼容；β_down ≪ 1 时才支持高效加速。

### RCW86

- [FACT] Helder et al. (2009) 报告宽 Balmer 线 FWHM = 1100 ± 63 km/s，V_sh = 6000 ± 2800 km/s，推断 **ξ_CR ~ 80%**——异常高。
- [FACT] Helder et al. (2013) **基本撤回**先前结果：细化区域研究后 V_sh 更低；仅有若干区域有**边际证据**支持粒子加速。
- [FACT] RCW86 形态复杂、距离不确定 → ξ_CR 估算难度高。
...
- Balmer 窄线宽：Δv_narrow ~ √(2kT_upstream/m_H) ~ 21 km/s (T_up = 10⁴ K)
- Balmer 宽线宽：Δv_broad ~ √(2kT_downstream/m_H)
- 中性回流激波预热距离：~几倍 λ_cx + λ_ion
- 中间线宽度：~100–300 km/s
...
| SNR 0509 支持 ξ_CR | ~10–20%（β_down ≪ 1 时）|
| RCW86（初测）FWHM | 1100 ± 63 km/s |
| RCW86（初测）V_sh | 6000 ± 2800 km/s |
| RCW86（初测）ξ_CR | ~80%（后被撤回）|
| Sollerman 等观测窄线宽 | 30–50 km/s → T_前置 ~25,000–50,000 K |

## 7.5 图表分析

参见 `09_figures_tables.md`（Figure 13 部分电离介质谱斜率、Figure 14 Balmer 线形状、Figure 15 SNR 0509 FWHM-ξ_CR 关系）。
...
- [CRITIQUE] 但作者也承认**β_down 的不确定性**是最大障碍：若电子-离子平衡（β_down=1），SNR 0509 的 FWHM 与无加速兼容；只有 β_down ≪ 1 时才支持 10–20% 加速效率。这一不确定性**无法通过 Balmer 观测单独解决**，需要 X 射线热辐射的独立测量。
...
- **潜在不一致性**：SNR 0509 的 SW 边缘速度在不同文献中不一致（Helder 用 5000 km/s；Morlino 用 4000 km/s），直接影响 ξ_CR 推断。
