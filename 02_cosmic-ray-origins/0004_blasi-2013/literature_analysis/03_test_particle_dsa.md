...
- [FACT] Coulomb 热化时间分层：电子自热化 τ_eq,ee 最快，质子自热化 τ_eq,pp 最慢；对典型 SNR 参数 τ_eq,ee ≈ 1200·(n/1 cm⁻³)⁻¹·(T_e/$10^{8}$ K)³/² 年，τ_eq,pp ≈ 2.3×$10^{6}$·(n/1)⁻¹·(T_p/$10^{8}$ K)³/² 年。
...
- [FACT] 粒子在均匀 $B_{0}$ + Alfvén 波背景下的运动：Larmor 频率 Ω = $qB_{0}$/(mcγ)，v_z = vµ，回旋半径 r_L = v/Ω。
- [FACT] 投掷角扩散系数（Q-LT）：ν = ⟨ΔθΔθ/Δt⟩ = (π/4)(kP(k)/($B_{0}$²/8π))·Ω（Eq. 29/30）。
- [FACT] 空间扩散系数：D(p) ≈ (1/3) r_L v / F，其中 F = kP(k)/($B_{0}$²/8π)（Eq. 32）。
- [FACT] CR 在星系被约束 ~$10^{7}$ yr → D ~ $10^{29}$ cm²/s → 共振尺度需 δB/B ~ 6 × $10^{-4}$。
...
- [FACT] 注入项（δ 函数近似）：Q(p, x) = η $n_{1}$ $u_{1}$ / (4π p²_inj) · δ(p−p_inj)·δ(z)（Eq. 35）。
- [FACT] 关键结果（Eq. 40, 41）：
...
- [FACT] 关键推论：r_L(p_max) = 1 pc · (E/$10^{15}$ eV) · B⁻¹_µ；由于 c/V_s ~ 100 而 r_L/R_SNR ~ 0.1，**必须 F(k_min) ≫ 1**，即 δB/$B_{0}$ ≫ 1。
...
| 10 | τ_eq = 3$m_{1}$$m_{2}$k³/²_B / [8(2π)^½ n q⁴ lnΛ] · ($T_{1}$/$m_{1}$+$T_{2}$/$m_{2}$)^3/² | Coulomb 热化时间（Spitzer 1962） |
...
| 47 | τ_acc = 3/($u_{1}$−$u_{2}$) ∫ dp'/p' [$D_{1}$/$u_{1}$ + $D_{2}$/$u_{2}$] | 加速时间 |
...
- [INTERPRETATION] §3.4 的核心论证是**"SNR 要达到 PeV 必须依赖磁场放大"**——这一步为下一章（§4）非线性理论埋下伏笔：因为 test-particle 理论里 F≪1 的假设在 p_max 处失效，δB≪$B_{0}$ 的准线性理论在最强散射点必然崩溃。
