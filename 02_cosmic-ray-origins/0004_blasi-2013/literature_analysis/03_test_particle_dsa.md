...
- **DSA 传输方程**（Skilling 1975a）的推导，得出 test-particle 谱指数 $\alpha$ = 3r/(r−1)，强激波极限 $\alpha$→4，即微分能量谱 n(ϵ)∝ϵ⁻²。
...
- [FACT] 非相对论条件 v ≪ c 等价于 Alfvenic Mach 数 M_A ≪ 1.3 × $10^{5}$ · n¹/² cm⁻³ · B⁻¹ µG（Eq. 9）。
- [FACT] Coulomb 热化时间分层：电子自热化 $\tau$_eq,ee 最快，质子自热化 $\tau$_eq,pp 最慢；对典型 SNR 参数 $\tau$_eq,ee ≈ 1200·(n/1 cm⁻³)⁻¹·(T_e/$10^{8}$ K)³/² 年，$\tau$_eq,pp ≈ 2.3×$10^{6}$·(n/1)⁻¹·(T_p/$10^{8}$ K)³/² 年。
...
- [FACT] Fermi 二阶加速：平均能量增益 ⟨$\Delta$E/E⟩ = (4/3)(V/c)²，标度是 (V/c)² → "second order"。
- [FACT] ISM 中 Alfvén 速度 v_A = B/√(4$\pi$$\rho$) = 2 B_µ · n_i,cm⁻³ km/s，太小 → 二阶加速不重要。
- [FACT] 粒子在均匀 $B_{0}$ + Alfvén 波背景下的运动：Larmor 频率 $\Omega$ = $qB_{0}$/(mc$\gamma$)，v_z = vµ，回旋半径 r_L = v/$\Omega$。
- [FACT] 投掷角扩散系数（Q-LT）：$\nu$ = ⟨$\Delta$$\theta$$\Delta$$\theta$/$\Delta$t⟩ = ($\pi$/4)(kP(k)/($B_{0}$²/8$\pi$))·$\Omega$（Eq. 29/30）。
- [FACT] 空间扩散系数：D(p) ≈ (1/3) r_L v / F，其中 F = kP(k)/($B_{0}$²/8$\pi$)（Eq. 32）。
- [FACT] CR 在星系被约束 ~$10^{7}$ yr → D ~ $10^{29}$ cm²/s → 共振尺度需 $\delta$B/B ~ 6 × $10^{-4}$。

### §3.3 DSA 传输方程

- [FACT] 激波压缩比 r = $u_{1}$/$u_{2}$ = (4 M²_s)/(M²_s + 3) → 强激波 r→4（Eq. 33）。
...
- [FACT] 注入项（$\delta$ 函数近似）：Q(p, x) = $\eta$ $n_{1}$ $u_{1}$ / (4$\pi$ p²_inj) · $\delta$(p−p_inj)·$\delta$(z)（Eq. 35）。
- [FACT] 关键结果（Eq. 40, 41）：
...
- [FACT] 时间约束（Eq. 48, 49）：$\tau$_acc(p_max) ≤ $\tau$_SNR → 得到条件
$$F(k_{\rm min}) \approx \frac{1}{3}\frac{c}{V_s}\frac{r_L(p_{\rm max})}{R_{\rm SNR}}$$
- [FACT] 关键推论：r_L(p_max) = 1 pc · (E/$10^{15}$ eV) · B⁻¹_µ；由于 c/V_s ~ 100 而 r_L/R_SNR ~ 0.1，**必须 F(k_min) ≫ 1**，即 $\delta$B/$B_{0}$ ≫ 1。
...
- [FACT] 三种最大能量定义：
  1. **时间约束** $\tau$_acc(p_max) ≤ $\tau$_SNR（§3.4 主线）。
...
| 编号 | 公式 | 含义 |
|------|------|------|
| 4 | V_ej = 10000 $E_{51}$^½ · M_ej,⊙⁻¹/² km/s | 抛射物初速 |
| 5 | c_s ≈ 11 (T/$10^{4}$ K)^½ km/s | ISM 声速 |
| 6 | M_s ≈ 900 $E_{51}$^½ M_ej,⊙⁻¹/² (T/$10^{4}$ K)⁻¹/² | 抛射物 Mach 数 |
| 7 | R_ST ≈ 2 M_ej,⊙^1/3 (n_ISM/1 cm⁻³)⁻¹/³ pc | Sedov-Taylor 半径 |
| 8 | T_ST ≈ 200 M_ej,⊙^5/6 $E_{51}$⁻¹/² (n_ISM/1)⁻¹/³ yr | Sedov 起始时间 |
| 9 | M_A ≪ 1.3×$10^{5}$ n¹/² B⁻¹ | 非相对论激波条件 |
| 10 | $\tau$_eq = 3$m_{1}$$m_{2}$k³/²_B / [8(2$\pi$)^½ n q⁴ ln$\Lambda$] · ($T_{1}$/$m_{1}$+$T_{2}$/$m_{2}$)^3/² | Coulomb 热化时间（Spitzer 1962） |
| 17–20 | Rankine-Hugoniot 跃变条件（强激波 r=4, $T_{2}$/$T_{1}$ = 5M²/16） | 质量-动量-能量守恒 |
| 21 | $kT_{2}$ = 3/16 m_p $u_{1}$² | 强激波后温度 |
...
| 47 | $\tau$_acc = 3/($u_{1}$−$u_{2}$) ∫ dp'/p' [$D_{1}$/$u_{1}$ + $D_{2}$/$u_{2}$] | 加速时间 |
| 48 | F(k_min) ≈ (1/3)(c/V_s)(r_L/R_SNR) | PeVatron 条件 |
| 50 | r_L(p_max) = 1 pc · (E/$10^{15}$ eV) · B⁻¹_µ | 回旋半径参考值 |
...
| $\tau$_eq,ee | ~$10^{3}$ yr |
| $\tau$_eq,pp | ~$10^{6}$ yr |
| kT_p | 5.6×$10^{8}$ (V_sh/5000 km/s)² K |
| D(1 GeV) | ~$10^{29}$ cm²/s |
| $\delta$B/B 约束（$10^{7}$ yr 困住） | ~6×$10^{-4}$ |
...
- [FACT] $\alpha$=4 意味着动量谱 f(p)∝p⁻⁴，能量谱 dN/dE∝E⁻²（相对论）——这是 SNR 范式的"理论招牌"。
...
- [CRITIQUE] Test-particle 假设要求 $\xi$_CR ≪ 1，而 §2 推导出的 $\xi$_CR ~10% 已接近上限——为 §4 非线性理论的出现提供动因。
