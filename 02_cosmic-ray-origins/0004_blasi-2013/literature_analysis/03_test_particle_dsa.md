...
- [FACT] 非相对论条件 v ≪ c 等价于 Alfvenic Mach 数 M_A ≪ $1.3\times10^{5}$ · n¹/² cm⁻³ · B⁻¹ µG（Eq. 9）。
- [FACT] Coulomb 热化时间分层：电子自热化 $\tau_{\rm eq}$,ee 最快，质子自热化 $\tau_{\rm eq}$,pp 最慢；对典型 SNR 参数 $\tau_{\rm eq}$,ee ≈ 1200·(n/1 cm⁻³)⁻¹·(T_e/$10^{8}$ K)³/² 年，$\tau_{\rm eq}$,pp ≈ $2.3\times10^{6}$·(n/1)⁻¹·(T_p/$10^{8}$ K)³/² 年。
...
- [FACT] CR 在星系被约束 ~$10^{7}$ yr → D ~ $10^{29}$ cm²/s → 共振尺度需 $\delta$B/B ~ $6\times10^{-4}$。
...
| 编号 | 公式 | 含义 |
|------|------|------|
| 4 | V_ej = 10000 $E_{51}^{\rm ½}$ · M_ej,⊙⁻¹/² km/s | 抛射物初速 |
| 5 | c_s ≈ 11 (T/$10^{4}$ K)^½ km/s | ISM 声速 |
| 6 | M_s ≈ 900 $E_{51}^{\rm ½}$ M_ej,⊙⁻¹/² (T/$10^{4}$ K)⁻¹/² | 抛射物 Mach 数 |
| 7 | R_ST ≈ 2 M_ej,⊙^1/3 (n_ISM/1 cm⁻³)⁻¹/³ pc | Sedov-Taylor 半径 |
| 8 | T_ST ≈ 200 M_ej,⊙^5/6 $E_{51}^{\rm -1}$/² (n_ISM/1)⁻¹/³ yr | Sedov 起始时间 |
| 9 | M_A ≪ $1.3\times10^{5}$ n¹/² B⁻¹ | 非相对论激波条件 |
| 10 | $\tau_{\rm eq}$ = 3$m_{1}$$m_{2}$k³/²_B / [8(2$\pi$)^½ n q⁴ ln$\Lambda$] · ($T_{1}$/$m_{1}$+$T_{2}$/$m_{2}$)^3/² | Coulomb 热化时间（Spitzer 1962） |
...
| kT_p | $5.6\times10^{8}$ (V_sh/5000 km/s)² K |
| D(1 GeV) | ~$10^{29}$ cm²/s |
