...
- 放大磁场的动力学反作用（$\Lambda$_B, W）使谱更趋幂律。

## 4.2 原文内容（要点摘录）

### §4 引言：三条需要 NLDSA 的理由

- [FACT] 加速粒子对激波的动量反馈：$\xi$_CR ~ 10% → CR 压力改变压缩比 r，谱变为刚度依赖。
...
- [FACT] CR 能量方程（Eq. 64）：∂E_c/∂t + ∇·($\gamma$_c/($\gamma$_c−1)·uP_c) = ∇·(D̄∇E_c) + u∇P_c，其中 D̄ 是能量加权平均扩散系数。
- [FACT] 准稳态下子激波压缩比 R_sub 用 $M_{1}$ 表达（Eq. 68）；总压缩比 R_tot = $u_{0}$/$u_{1}$（Eq. 69）用 R_sub、R_0 表达。
- [FACT] CR 诱导 precursor 的深度：$\xi$_CR(z) ≈ P_c(z)/($\rho$₀$u_{0}$²) ≈ 1 − u(z)/$u_{0}$（Eq. 70）。
...
- [FACT] Bohm 极限下加速时间（Eq. 71）：$\tau$_acc ≈ 3.3×$10^{7}$ E_TeV · B⁻¹_100 · V⁻²_sh,8 s。
- [FACT] 同步辐射损失时间（Eq. 72）：$\tau$_syn = 4×$10^{10}$ B⁻²_100 · E⁻¹_TeV s。
- [FACT] 最大电子能量（Eq. 73）：E_e,max ≈ 34 B⁻¹/²_100 · V_sh,8 TeV。
- [FACT] 最大光子能量（Eq. 74）：E_$\gamma$,max ≈ 1.7 V²_sh,8 keV —— **与 B 无关**（Bohm 假设下）。
- [FACT] 特征宽度（Eq. 75）：√(D·$\tau$_syn) ≈ 3.7×$10^{-2}$ B⁻³/²_100 pc → 观测到的 ~$10^{-2}$ pc 窄边缘需要 B~几百 µG。
...
- [FACT] 若 D(E) 取 ISM 值（3×$10^{28}$ (E/10 GeV)^$\delta$ cm²/s），$\tau$_acc 远超自由膨胀期（Eq. 76）。
...
- [FACT] 共振 streaming 不稳定性增长速率（Zweibel 1979；Achterberg 1983，Eq. 79/87）：
  $\omega$_I(k) = ($\pi$/8)·$\Omega$*_p·(V_sh/v_A)·[n_CR(p>pres(k))/n_i]
- [FACT] 共振不稳定性适用条件（Eq. 86, 88）：n_CR/n_i ≪ v_A²/(V_sh c)，即 $\xi$_CR ≪ 8×$10^{-4}$ · (V_sh/5×$10^{8}$ cm/s)⁻³ —— **对 $\xi$_CR ~ 10% 完全不成立**。
- [FACT] **CR modified regime**（$\xi$_CR 大，Eq. 89）：$\omega$_I ≈ $\omega$_R = [ ($\pi$/8)$\Omega$*_p k V_sh n_CR(p>pres)/n_i ]^½，相位速度 v_$\phi$ ≫ v_A。
- [FACT] 此条件下 $F_{0}$ ≤ 1（Eq. 90）：$F_{0}$(k) = ($\pi$/6)^½ ($\xi$_CR/$\Lambda$)^½ (c/V_sh)^½，即"efficient CR acceleration ... reduces the growth of the waves and limits the value of the self-generated magnetic field to the same order of magnitude as the pre-existing magnetic field."
- [FACT] **Bell 非共振小尺度模**（Bell 2004, 2005）：$\xi$_CR 大时，右手极化模在 k r_L,0 > 1 出现非共振分支，增长率 ∝ k^½，峰值在 k* r_L,0 = 3 $\xi$_CR $\gamma$_min / $\Lambda$ · (V_sh/v_A)² · (V_sh/c) > 1（Eq. 92），比共振模快 (k* r_L,0)^½ 倍。
...
- [FACT] **大尺度非共振 firehose 模**（§4.2.4）：$\Gamma$_FH(k) ≈ $\xi$_CR^½ · V_sh² k / c（Eq. 93）；对于 k ≪ 1/r_L,max，$\Gamma$_FH·$\tau$_adv(p_max) ≪ $\xi$_CR^½ < 1，时间不够。
...
- [FACT] R_tot–R_sub 关系（Eq. 99）：R_tot^($\gamma$_g+1) = $M_{0}$² R_sub^$\gamma$_g / [2 · ($\gamma$_g+1 − R_sub($\gamma$_g−1))/(1+$\Lambda$_B)]，$\Lambda$_B = W [1 + R_sub(2/$\gamma$_g − 1)]，W = P_w,1/P_g,1。
...
| 60 | ∇(½$\rho$u³ + $\gamma$_g/($\gamma$_g−1)·uP_g) = −u∇P_c | 含 CR 能量守恒 |
| 62, 63 | E_c, P_c 的定义 | CR 能量/压力 |
| 66 | CR 能量输运方程 | 含扩散项、做功项 |
| 68 | R_sub($M_{1}$) | 子激波压缩比 |
| 69 | R_tot(R_sub, $M_{0}$) | 总压缩比 |
| 70 | $\xi$_CR(z) ≈ 1 − u(z)/$u_{0}$ | precursor 深度 |
| 71 | $\tau$_acc (Bohm) | 加速时间（磁化放大情形） |
...
| 81 | $F_{0}$(k) = ($\pi$/4)·$\xi$_CR·(V_sh/v_A)·(1/$\Lambda$) | 共振 streaming 功率谱 |
| 86, 88 | $\xi$_CR ≪ 8×$10^{-4}$(V_sh/5×$10^{8}$)⁻³ | 共振不稳定性适用条件 |
| 87 | $\omega$_I(k) = ($\pi$/8)$\Omega$*_p(V_sh/v_A)(n_CR/n_i) | 低 $\xi$_CR 共振增长率 |
| 89 | $\omega$_I ≈ $\omega$_R = [($\pi$/8)$\Omega$*_p k V_sh n_CR/n_i]^½ | 高 $\xi$_CR 增长率 |
| 90 | $F_{0}$(k) = ($\pi$/6)^½ ($\xi$_CR/$\Lambda$)^½ (c/V_sh)^½ | 高 $\xi$_CR 情况下的 $F_{0}$ |
| 92 | k* r_L,0 = 3 $\xi$_CR $\gamma$_min/$\Lambda$ · (V_sh/v_A)² · (V_sh/c) | Bell 非共振模峰值尺度 |
| 93 | $\Gamma$_FH(k) ≈ $\xi$_CR^½ · V_sh² k/c | 大尺度 firehose 增长率 |
| 95 | ∇($\rho$u² + P_g + P_c + P_w) = 0 | 含波动的动量守恒 |
| 99 | R_tot^($\gamma$+1) 与 R_sub 关系 | 磁反作用下的压缩比 |
| 100 | $\Lambda$_B = W[1 + R_sub(2/$\gamma$ − 1)] | 磁动力学反作用参量 |
...
- [CRITIQUE] 作者对共振不稳定性在 $\xi$_CR ~ 10% 时失效的论证（§4.2.1）是**决定性**的：如果这一论证正确，那么传统 "$\delta$B/B ~ 1 饱和" 的教科书结论在 SNR 强加速情形下不适用。
