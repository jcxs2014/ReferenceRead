...
## 2.7 深度分析与 [FACT]/[INTERPRETATION]/[CRITIQUE] 标注

**[FACT]** §2 的核心推导给出了一个简洁而强大的结果：$q = 3r/(r-1)$，对于理想气体强激波（$r=4$），$q=4$（对应微分谱 $dN/dE \propto E^{-2}$）。这个谱指数成为 DSA 理论的标志——它与 SNR 中观测到的宇宙线谱（$\propto E^{-2.1}$）接近，但后者还需考虑传播效应才能解释 $E^{-2.7}$ 的实测值。[FACT]

**[INTERPRETATION]** BO 的推导中最关键的几何洞察是：粒子在激波两侧的穿越是**非对称的**——上游粒子穿越激波进入下游的速率高于反过来穿越的速率（因为下游流速更慢），所以粒子净获能量。这个非对称性来自于压缩比 $r>1$，与扩散系数无关——这就是为什么 DSA 的谱指数如此 robust。[INTERPRETATION]

**[CRITIQUE]** BO 的推导假设了稳态激波结构——但真实 SNR 的激波是快速演化的（Sedov-Taylor 阶段，压缩比从大变小）。对于有限时间演化，幂律只在特定能量范围成立，且最大能量的粒子存在时间截断（"steepened by adiabatic expansion"）。原文§2.5的讨论是初步的，更系统的时变 DSA 需要 NLDSA 框架（如 Blasi 2002）。[CRITIQUE]

**[FACT]** §2 的四个长度尺度排序（$\delta \ll r_L \ll L \ll H$）直接决定了 DSA 的适用范围：① $\delta \ll r_L$ 确保激波是"无碰撞"的（CR 与热气体解耦）；② $r_L \ll L$ 确保扩散近似成立（粒子的随机行走步长 << 宏观尺度）；③ $L \ll H$ 确保可以忽略边界效应（把激波当作半无限平面）。[FACT]

**[INTERPRETATION]** 这三个不等式在 SNR 的演化早期（Eiley 阶段）和晚期（Sedov 阶段）可能同时成立，但在某些极端环境（如 pulsars、AGN jets）中，$rl \sim L$ 或 $L \sim H$ 的条件可能被违反——这意味着 DSA 在这些环境中的适用性需要重新检验，而不能简单套用 $q = 3r/(r-1)$。[INTERPRETATION]

**[CRITIQUE]** BO 的 $q = 3r/(r-1)$ 推导没有考虑磁场几何效应——原文假设散射是各向同性的（扩散张量各向同性），但真实 SNR 中的准平行/准垂直激波会产生显著各向异性。Caprioli & Spitkovsky (2014) 的 PIC 模拟明确显示：对于准垂直激波，DSA 效率显著低于 $q = 3r/(r-1)$ 的预言——BO 的公式在磁场方向不利的激波上是失效的。[CRITIQUE]
