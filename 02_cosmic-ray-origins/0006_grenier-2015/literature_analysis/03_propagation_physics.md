...
>
> 上一章：`02_direct_measurements.md`
>
> 下一章：`04_crwanderers.md`

# 3. Advances in Cosmic-Ray Propagation

## 3.1 本节核心内容

**[FACT]** 一次核子在 ISM 中通过散裂产生次级核子。次级源函数由主核子谱与散裂截面给出，因此次级/初级比是检验 CR 传播模型、约束再加速程度的关键手段。

**[FACT]** 散裂次级丰度大（Li、Be、B、sub-Fe）是 CR 与星际介质发生大量相互作用的最显著证据。

## 3.2 B/C 比作为传播探针

**[FACT]** 最有用的传播探针是 **B/C**：C 几乎全部为一次（primary），B 全部为次级（C、O、重核的散裂产物）。B/C 随动量的演化主要反映 D_xx(p) 的动量依赖性（因为次级与母核每核子能量相近）。

**[FACT]** B/C 随动量下降 → D_xx(p) 随 p 增大。将 D_xx(p) 表示为幂律，指数范围 **0.3–0.8**（取决于模型）。该指数解释了 SNR 逃逸谱（~2.2）与 ISM 观测谱（~2.7）的差异。
...
**[FACT]** 使用 USINE 代码 + 贝叶斯 MCMC 的系统参数研究（Maurin 2010; Putze 2011; Lavalle 2014）：
- D_xx(p) 的很宽动量依赖范围都可拟合 B/C（部分因对流包含在内）。
- 不同一次核子的谱差异**自然归因于传输效应**。
- 次级正电子排除 D_xx(p) 非常陡峭的变。

## 3.4 §3.0.2 Diffusive Reacceleration 与替代方案

**[FACT]** 扩散再加速 = 动量空间扩散，源于与运动散射体的相互作用；D_pp 与 D_xx 存在基本关系（Strong 2007）；Thornbury & Drury 2014 给出清晰推导及其与 Fermi 二阶机制的关系。

**[FACT]** 再加速被频繁包含在模型中，因为它：
- 无需 D_xx(p) 中的 ad-hoc 断点即可解释 B/C 的动量依赖
- 与 Kolmogorov 湍流的 **D_xx(p) ∝ p^(1/3)** 一致
...
**[FACT]** Mertsch & Sarkar (2014) 计算邻近遗迹中的次级正电子产生，并与 B 和反质子产生关联。

## 3.6 §3.1 Advances in Propagation Physics — 粒子轨迹模拟
...
2. **致密气体中增加的电离损失** → 净向内 CR streaming 通量 → 云边缘到云间介质连线上的 Alfvén 波阻止 CR 穿透 → 在几十 MeV 处压制通量数个量级（Cesarsky & Volk 1978; Skilling & Strong 1976）。
   - 但 Everett & Zweibel (2011) 预测在 GeV 能量处仅有 7.5% 的 CR 压力下降（即使是热扩散电离层 → 冷致密云极端过渡）。

**[FACT]** 亚秒差距核心的磁场结构导致 CR **排除（磁镜）**或**聚集（磁聚焦）**：
- 排除主导多数核心构型 → 电离 CR 密度减少 2–4 倍（Padovani & Galli 2011）。
- 收缩前恒星核心中心离子化率可降至 <10⁻¹⁸ s⁻¹（Padovani 2013）。
...
- **DRAGON**（Evoli & Yan 2014; Gaggero 2014）：扩展到各向异性、空间依赖扩散，与 MHD 湍动水平相关（dragonproject.org）。
...
- **Panel (b)**：B/C vs E_kin
  - 数据：AMS-02 (preliminary)、PAMELA (preliminary)

### 关键观察
- **[FACT]** 三个模型（KOL, KRA, PD）都能拟合 B/C 曲线。
- **[FACT]** 说明 3D 扩散处理 + 源分布的螺旋结构可解释 AMS-02 与 PAMELA 数据。
- **[FACT]** <几 GeV 处太阳调制对预测重要。

### 物理意义
- 说明参数简并（见 §3.3）：多种模型都能拟合 B/C，需要反质子、正电子等独立探针打破简并。
