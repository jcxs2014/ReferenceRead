---
title: The Nine Lives of Cosmic Rays in Galaxies
authors: Isabelle A. Grenier, John H. Black, Andrew W. Strong
year: '2015'
journal: Annual Review of Astronomy and Astrophysics, Volume 53, pp. 199–246
pages: '199-246'
doi: 10.1146/annurev-astro-082214-122457
arxiv: 未提供
keywords: γ rays, interstellar medium, interstellar chemistry, dust, superbubbles
abstract: 近年来宇宙线天体物理进展迅速，对其它天文分支的影响不断扩大。众多新实验通过大气中/太空中的直接测量以及 γ 射线、同步辐射的间接测量，拓宽了对宇宙线起源、传播和相互作用信息的获取范围。低能宇宙线对星际化学的影响是一个快速发展的方向，包括这些粒子如何在发生化学反应的云中传播。宇宙线通过其
  γ 射线产生，也成为探测星际气体含量的强力方式。宇宙线与星际介质相互作用的观测和建模取得了重大进展。 综述聚焦于能量 ≤1 TeV 的宇宙线，因为它们与星际作用主要相关；讨论"九种角色"以展现宇宙线与其星际栖息地之间多种多样的相互作用
category: 宇宙线起源
status: completed
sections:
  - 'Introduction / Scope of the Review'
  - 'Advances from Direct Measurements'
  - 'Advances in Cosmic-Ray Propagation'
  - 'Cosmic-Ray Wanderers in the Milky Way'
  - 'Cosmic-Ray / ISM Interaction Processes'
  - 'Cosmic-Ray Stimuli on the ISM'
  - 'Cosmic-Ray Tracers of the ISM'
  - 'Cosmic Rays in Starburst Environments'
  - 'A Few More Words'
read_date: '2026-08-12'
lastread: '2026-08-12'
tags:
- γ rays
- interstellar medium
- interstellar chemistry
- dust
- superbubbles
citations:
- '[[02_cosmic-ray-origins/0005_amato-2014/literature_analysis/00_overview|0005_amato-2014]]'
- '[[02_cosmic-ray-origins/0004_blasi-2013/literature_analysis/00_overview|0004_blasi-2013]]'
- '[[03_stellar-nucleosynthesis/0009_asplund-2009-solar-composition/literature_analysis/00_overview|0009_asplund-2009-solar-composition]]'
- '[[03_stellar-nucleosynthesis/0011_kewley-2001-starburst/literature_analysis/00_overview|0011_kewley-2001-starburst]]'
path: 02_cosmic-ray-origins/0006_grenier-2015/literature_analysis/00_overview.md
---
> 本章属于：The Nine Lives of Cosmic Rays in Galaxies（Grenier, Black & Strong 2015）
>
> 下一章：[[02_cosmic-ray-origins/0006_grenier-2015/literature_analysis/01_scope_and_nine_lives.md|01_scope_and_nine_lives.md]]

# 0. 文献基本信息

- **Title:** The Nine Lives of Cosmic Rays in Galaxies
- **Authors:** Isabelle A. Grenier, John H. Black, Andrew W. Strong
- **Collaboration:** 无
- **Journal:** Annual Review of Astronomy and Astrophysics, Volume 53, pp. 199–246
- **Publication Date:** 2015（First published online as a Review in Advance on 2015-06-18）
- **DOI:** 10.1146/annurev-astro-082214-122457
- **arXiv:** 未提供
- **Research Field:** 宇宙线天体物理 / 星际介质物理 / 星系反馈
- **Keywords:** $\gamma$ rays, interstellar medium, interstellar chemistry, dust, superbubbles
- **Affiliations:**
  1. Laboratoire AIM Paris-Saclay, CEA/Irfu, CNRS, Université Paris Diderot（Grenier）
  2. Onsala Space Observatory, Chalmers University of Technology（Black）
  3. Max-Planck-Institut für extraterrestrische Physik（Strong）

## 关于文件名与元数据的说明
> **[FACT]** 本 PDF 实际作者为 Grenier, Black & Strong (2015)，发表期刊为 Annu. Rev. Astron. Astrophys. 53:199–246。
> **[FACT]** 存储目录命名 "02_cosmic-ray-origins/0006_thackeray-2016/" 与文件元数据不符。
> **[CRITIQUE]** 本库中的目录 / 元数据标注（Thackeray & Hnatyk 2016）与 PDF 内真实信息存在明显矛盾，后续检索、引用请以 PDF 本身为准。本精读所有事实以 PDF 文本为准。

## 结构总览（文献树）

```
Introduction / Scope of the Review
├── 2. Advances from Direct Measurements
│   ├── 2.1 Propagation Principles
│   ├── 2.2 Composition of Primary Cosmic Rays
│   ├── 2.3 Spectral Puzzles (2.3.1 GeV 以上, 2.3.2 <2 GeV, 2.3.3 日球层内)
│   └── 2.4 Positron Excess at High Energies
├── 3. Advances in Cosmic-Ray Propagation
│   ├── 3.0.1 Parametric studies of CR propagation
│   ├── 3.0.2 Diffusive reacceleration and alternatives
│   ├── 3.0.3 Secondary production in CR sources
│   ├── 3.1 Advances in Propagation Physics
│   │   └── 3.1.1 CR penetration into molecular clouds
│   └── 3.2 Advances in Propagation Models
├── 4. Cosmic-Ray Wanderers in the Milky Way
│   ├── 4.1 CR in the Local ISM
│   ├── 4.2 CR in the Outer Galaxy
│   ├── 4.3 CR in the Inner Galaxy (4.3.1 未分辨 $\gamma$ 源, 4.3.2 Fermi Bubbles & GC)
│   └── 4.4 Global Properties of the High-Energy Galaxy
├── 5. Cosmic-Ray / ISM Interaction Processes
│   ├── 5.1 Atomic and Molecular Processes
│   ├── 5.2 CR-Induced UV Radiation
│   ├── 5.3 Interaction with Solid Particles
│   └── 5.4 Spallation and Synthesis of Light Elements
├── 6. Cosmic-Ray Stimuli on the ISM
│   ├── 6.1 Vicinity of CR Sources
│   ├── 6.2 Interstellar Ion Chemistry Driven by CRs
│   ├── 6.3 Molecular Ions: H3+
│   └── 6.4 Molecular Ions: OH+, H2O+, H3O+
├── 7. Cosmic-Ray Tracers of the ISM
│   ├── 7.1 Tracing Gas and Dust Properties
│   │   ├── 7.1.1 In the HI
│   │   ├── 7.1.2 In the dark neutral medium
│   │   ├── 7.1.3 About XCO
│   │   └── 7.1.4 On dust properties
│   └── 7.2 Tracing CR Leptons and the Magnetic Field
├── 8. Cosmic Rays in Starburst Environments
│   ├── 8.1 Effect on CR Composition
│   ├── 8.2 Cocoon of Young CRs in Cygnus X
│   ├── 8.3 Normal Galaxies
│   ├── 8.4 Starburst Galaxies
│   └── 8.5 $\gamma$ Rays in Extragalactic Space
└── 9. A Few More Words
```

## Figure / Table 清单

- **Table 1** — Recent measurements of CR composition and spectra（气球/卫星，2008–2014）
- **Figure 1** — Local spectra of CR protons and He near Earth / heliopause
- **Figure 2** — CR electron/positron spectra: AMS-02 + PAMELA + 邻近脉冲星模型
- **Figure 3** — Proton/He 谱 + B/C 比 vs 传播模型（KOL/KRA/PD）
- **Figure 4** — $\gamma$-ray emissivity q$\gamma_{\rm H}$ per H 的银河系半径分布 + 局部云对比
- **Figure 5** — 全天图：Fermi-LAT $\gamma$ 强度（1 GeV 以上） vs Planck+IRAS 尘埃光学深度
- **Figure 6** — 银河系内部 $\gamma$-ray 能谱（INTEGRAL+COMPTEL+Fermi-LAT）
- **Figure 7** — 银河系多波段光度谱（射电→$\gamma$，含 CR 功率分解）
- **Figure 8** — CR 电离引发的星际化学反应流图
- **Figure 9** — 银河系 XCO 径向分布 + DNM 的 AV/NH
- **Figure 10** — 附近云 CO-dark H2 质量分数
- **Figure 11** — Chamaeleon 云中尘埃发射性质的演化
- **Figure 12** — 高纬 synchrotron 谱（主/次电子 + 正电子）
- **Figure 13** — Cygnus X：8 $\mu$m 图 + Fermi-LAT $\gamma$-ray cocoon

---

## 全文精读导航

- 01_scope_and_nine_lives.md — §1 综述范围与"宇宙线的九种角色"
- 02_direct_measurements.md — §2 直接测量进展（含 Table 1, Fig 1–2）
- 03_propagation_physics.md — §3 宇宙线传播物理（含 Fig 3）
- 04_crwanderers.md — §4 宇宙线在银河系内的漫游（含 Fig 4–7）
- 05_crism_interactions.md — §5 宇宙线/星际介质相互作用过程（含 Fig 8）
- 06_cr_stimuli.md — §6 宇宙线对 ISM 的激发（H3+, OH+ 等）
- 07_tracers.md — §7 宇宙线作为 ISM 示踪剂（含 Fig 9–12）
- 08_starbursts.md — §8 星暴环境中的宇宙线（含 Fig 13）
- 09_conclusion.md — §9 结语 + 参考文献
- 99_final_summary.md — 总结、记忆点、可借鉴之处

## Abstract（中文精读）

**[FACT]** 近年来宇宙线天体物理进展迅速，对其它天文分支的影响不断扩大。众多新实验通过大气中/太空中的直接测量以及 $\gamma$ 射线、同步辐射的间接测量，拓宽了对宇宙线起源、传播和相互作用信息的获取范围。低能宇宙线对星际化学的影响是一个快速发展的方向，包括这些粒子如何在发生化学反应的云中传播。宇宙线通过其 $\gamma$ 射线产生，也成为探测星际气体含量的强力方式。宇宙线与星际介质相互作用的观测和建模取得了重大进展。

**[FACT]** 综述聚焦于能量 ≤1 TeV 的宇宙线，因为它们与星际作用主要相关；讨论"九种角色"以展现宇宙线与其星际栖息地之间多种多样的相互作用。

---

## 篇间导航

### 前序阅读

- **Strong, Moskalenko & Ptuskin (2007)** — 传播理论标准参考（Rev. Mod. Phys.）
- **Blasi (2013)** — SNR 作为 PeVatron 的核心证据（arXiv:1311.7346）

### 关联论文

- [[02_cosmic-ray-origins/0005_amato-2014/literature_analysis/00_overview|0005_amato-2014]] — Amato 2014 §6 涉及 CR-ISM 耦合
- [[02_cosmic-ray-origins/0004_blasi-2013/literature_analysis/00_overview|0004_blasi-2013]] — Blasi 2013 与 Grenier 2015 在 PeVatron 论证上互补
- [[03_stellar-nucleosynthesis/0009_asplund-2009-solar-composition/literature_analysis/00_overview|0009_asplund-2009-solar-composition]] — CR 元素丰度比较依赖太阳丰度标准
- [[03_stellar-nucleosynthesis/0011_kewley-2001-starburst/literature_analysis/00_overview|0011_kewley-2001-starburst]] — Grenier 2015 §8 综述星暴星系中 CR 角色