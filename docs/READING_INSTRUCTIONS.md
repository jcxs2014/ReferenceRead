...
**先判断原文是否具有显式子节层级**（判断方法见 §4.1），据此选择路径：

### 路径 A（原文有显式子节层级）——子节镜像【优先】
...
- **每个子节标题下第一行必须插入中文翻译段**（引用块 `> **译文**` 形式，见示例 `docs/子节镜像示例-ruszkowski-02.md`）
- 子节内覆盖"核心内容 / 公式 / 参数 / 图表 / 逻辑"（沿用 X.1–X.8 的要素精神，**不强制固定八段**，按子节实际内容取舍）

### 路径 B（原文平铺，无显式子节）——维持现状

原文无子节层级时**不强造子节**，分章内直接写内容（可用以下八段要素精神组织，但不强制编号）：
...
- **新精读**：一律按本 §4（路径 A 优先）
- **存量分章**：分批改造为路径 A（批次安排见 `docs/精读深度扩充设计备忘.md`），未改造前维持现状
- 长文献判定阈值：综述 ≥40 页 / 原始论文 ≥20 页（pages 字段已全量补齐）
...
- 核素/同位素：`¹²C`、`⁸Be`、`²⁰Ne`、`Zr⁹⁰`、`¹³N(p,γ)¹⁴O`（化学符号惯例）
...
4. 精读产出后自查：`grep -n "⁰¹²³⁴⁵⁶⁷⁸⁹⁻⁺" 文件` 应只命中核素/单位/标记行

**公式正确性铁律（2026-08-15 用户明确）**：修改/书写公式必须对照原文 PDF 确保正确性——
- 批量转换（老库 Unicode → LaTeX）：转换后必须从审计清单抽样对照原文 PDF，**数值零差异、语境正确、核素/单位无误转**（方法见 `docs/公式LaTeX化批量执行说明.md` §2）
