...
> 本节记录第2轮复验中发现的4项降级决策，供后续审查参考。

### C9 — telescope-array-2023

| 维度 | 内容 |
|---|---|
| **原验收** | §05 结论66行，标P2（"属合理篇幅"） |
| **WorkBuddy复验** | 66行对单事件论文（Amaterasu事件）属合理篇幅；内容含4条系统误差讨论（能量校准/Auger交叉校准/成分分离不确定性/anisotropy统计不足）+ 库内关系表 |
| **降级理由** | Amaterasu是单事件Letter，66行结论符合该体例；行数不是质量唯一判据 |
| **复验结论** | WorkBuddy接受降级P2；无需强制补深 |

### C10 — caprioli-2014

| 维度 | 内容 |
|---|---|
| **原验收** | §05/§07/§09各35/34/28行，仅1条CRITIQUE，标P2 |
| **WorkBuddy复验** | 全文497行接近研究型论文标准（≥500行），篇幅实质达标；CRITIQUE偏少（1条→3条后仍偏少）但非阻塞项 |
| **降级理由** | 篇幅实质达标，批判偏少属优化方向非强制修复 |
| **复验结论** | WorkBuddy接受降级P2；批判标注已从1条增至3条 |

### C11 — genolini-2021

| 维度 | 内容 |
|---|---|
| **原验收** | 03_statistical_method 74行，标P2 |
| **WorkBuddy复验** | 74行含实质性方法论反思：SLIM envelope方法优缺点讨论、Bayesian vs Frequentist对比（脚注）、系统性不确定性来源（模板误差/传播效应/能量校准）；方法论章节不宜过长，内容实质即可 |
| **降级理由** | 统计方法章节篇幅受论文定位约束（Letter 7页），内容实质比行数更重要；binary fulltext由外部系统管理 |
| **复验结论** | WorkBuddy接受降级P2；外部管理文件不纳入本轮修复范围 |

### C12 — weinrich-2020

| 维度 | 内容 |
|---|---|
| **原验收** | note commit称"ratio 29%（350L/1162L）" |
| **WorkBuddy复验** | 实测lit 557行/fulltext 1162行，ratio 48%；之前note中"350行/ratio 29%"为笔误（应为557L/ratio 48%） |
| **降级理由** | 7页A&A Letter（1162行fulltext属正常），557行lit接近50% ratio属合理；补深23行后（366L→389L）仍以results展开不足为主因，属非强制优化 |
| **复验结论** | WorkBuddy接受降级P2；数字已修正为557L/ratio 48% |
