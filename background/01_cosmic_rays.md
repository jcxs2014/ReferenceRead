...
**K-俘获同位素检验**：³⁷Ar、⁴⁴Ti、⁴⁹V、⁵¹Cr 等通过电子俘获快速衰变，丰度对能量变化敏感。⁵¹V/⁵²Cr 与再加速模型更符合，但 ⁴⁹Ti/Ti 结果相反——**直接证据尚不明确**，主要困难在核碎裂截面精度。
...
**Li、Be、B 丰度**：这些元素在恒星内部被破坏（低聚变阈值），主要由 CR 散裂产生。⁷Li/⁶Li 在星际介质中测得约 **2**（太阳系为 12.3），直接显示**最近散裂产生 Li** 的证据。

### 1.1.7 数值模型（GALPROP 等）

**GALPROP**（Strong & Moskalenko 1998, ApJ 509:212）：
- 在空间网格上数值求解传播方程 (1)，支持 2D（轴对称）或完整 3D；
- 从最重初级核（如 ⁶⁴Ni）开始，逐步向下计算碎裂源项，直至质子、次级电子/正电子和反质子；¹⁰B 通过 ¹⁰Be 衰变产生，需第二次迭代；
...
$$
\xi_{\rm CR}(z) \approx P_c(z)/(ρ_0 u_0^2) \approx 1 - u(z)/u_0
$$
...
**放射性时钟逻辑链**：
- ¹⁰Be/⁹Be 数据 → 局部扩散系数 $D_{xx} \approx (3-5) \times 10^{28}$ cm²/s（在 3 GV）；
...
**Grenier (2015) 的更新**：Fermi-LAT 数据显示 $q_{\gamma,H}$（每 H 核的 $\gamma$ 射线 emissivity）在银河系内**相当均匀**（变化 <30%），外臂仅降 20–40%。这确认**长存 CR 梯度问题**——外银 $q_{\gamma,H}$ 梯度与 ¹⁰Be/⁹Be 允许的最大晕大小 $\sim 10$ kpc 之间勉强一致。
...
**加速延迟**：
- ⁵⁹Ni（半衰期 $7.6 \times 10^4$ y）：存在；
- ⁵⁷Co（0.74 y）：存在；
- ⁵⁶Ni（6 d）：缺失（符合预期，加速前就衰变）。

ACE 数据表明：**合成与加速之间的延迟 $\ge 10^5$ 年**，与"超新星加速自身喷射物"的模型不一致，但支持"加速已有星际物质"（DFA 模型）。Grenier (2015) 在星暴环境中也引用此结论：⁵⁹Ni 缺失 → SN 合成到重核加速之间 $\ge 0.1$ Myr。

**再加速检验**：⁵¹V/⁵²Cr 与再加速模型更符合，但 ⁴⁹Ti/Ti 相反；V/Cr 比在轻微上更符合含再加速的模型——**结果不明确**，主要困难在核碎裂截面精度。
...
| 晕高度 $z_h$ | 4–6 kpc（¹⁰Be/⁹Be 约束） | Strong 2007 |
...
| 加速延迟（⁵⁹Ni） | $\ge 10^5$ yr | Strong 2007; Grenier 2015 |
