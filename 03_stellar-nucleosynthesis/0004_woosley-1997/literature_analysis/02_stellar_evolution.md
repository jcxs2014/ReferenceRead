...
- **12C** 第二激发态 γ 宽度：B2FH 时代不确定至少一个数量级（Hoyle 1954 预言的 7.65 MeV 态对 3α 过程至关重要）
...
- 右半部分 (log T_eff ≤ 4.7)：初始质量 0.2, 1, 5, 25 M☉ 的模型单星演化轨迹，叠加真实亮星位置
- 粗线表示核心核燃烧阶段（时标 ~ 核时标）
- 0.6 M☉ 与 0.85 M☉ 虚线轨迹：AGB 超风阶段模型，展示类星体星云 (PN) 中心星的演化
- 左半部分：LMXB 的最大光度，恒定半径线 ~ 10^25 cm（相当 1.4 M☉ NS 或 10 M☉ BH 的半径）
- 星形符号：USXRs、新星后中心星、PN 中心星

[FACT] 主序寿命 τ_MS ∝ M^(−2.25)：
- 1 M☉：~10^10 年（~1 Hubble 时间）
- 25 M☉：~7×10^6 年

[FACT] <0.8–1.0 M☉ 的恒星在主序寿命内不足以离开主序（因此 0.2 M☉ 轨迹极短）

[FACT] **He 核心燃烧阶段**：
- <2.3 M☉ 模型在此阶段 He 核质量几乎相同 → 光度几乎相同 (~50 L☉)，寿命 ~10^8 年
- He 燃烧时标 = 主序寿命的 ~25%（中质量）至 ~10%（大质量）

[FACT] **红巨星/超巨星序列**（Fig. 1 中最大号圆点）：
- (a) M ≤ 2.3 M☉：惰性电子简并 He 核 + H 燃烧壳层（RGB）
- (b) 2.3–20 M☉：核心 He 燃烧 + H 壳层燃烧的第一阶段
- (c) 具有 ν 冷却的 CO 核 (1–9 M☉) 或 ONe 核 (9–11 M☉) + 交替 H/He 壳层燃烧（AGB 星）

[FACT] **白矮星形成率**：~0.5–1 yr^(-1)，通过比较观测的 WD 数-光度分布与冷却模型得到；与 1–11 M☉ 恒星的形成率一致

[FACT] **质量损失 (Mass loss)**：
- Deutsch (1956) 观测到 α Her（M 超巨星）高质量损失率
...
[FACT] 当脉动周期 > 400 天时，质量损失率 ~10^(-5)–10^(-4) M☉/yr（比核燃烧处理物质速率大 10^2–10^3 倍）

[FACT] **WD 初始-最终质量关系**：WD 质量 ≈ 模型刚成为双壳层燃烧 AGB 星时的电子简并核质量 + 仅几个百分之一 M☉ 的核燃烧产物

[FACT] 银河系中观测到的 WD 质量分布在 ~0.55–0.65 M☉ 处峰值（Liebert & Bergeron 1995）
...
- 发生在 H 燃烧消耗约 10% 星体质量（M < 2.3 M☉）或 0.1 M☉(M/M☉)^1.4（大质量）之后
...
- [FACT] 但 <1.3 M☉ 巨星 Li 耗竭被低估多达两个数量级（Luck & Lambert 1982）——暗示**对流超射 (convective overshoot) 或差旋 (differential rotation) 比标准第一次挖掘更重要**

**(ii) He 闪 (Helium flash)**（<2.3 M☉）：
- He 核电子简并 → He 点火前核心达到 ~0.45–0.5 M☉ 时 He 闪
- 闪传播至中心解除简并 → 星体成为"clump" 星（Fig. 1 中 L~50L☉ 的重线段）

**(iii) 第二次挖掘 (Second dredge-up)**：
- 在 5–9 M☉ 模型中：He 燃烧壳层接近 H-He 不连续面时
- CO 核质量 >0.56–0.61 M☉ 后触发
- 鲜活的 4He 与 14N 被挖掘到表面

[FACT] **9–11 M☉ 大质量模型的第二次挖掘极为复杂**：CO 核部分简并 + C 燃烧闪 + He 燃烧闪，能量来源可来自 C 燃烧、He 燃烧或引力热能（gravothermal energy）
- 9 M☉ 模型：第一次 C 壳层闪 + 引力热能驱动挖掘（García-Berro et al. 1997）
- 10 M☉ 模型：C 燃烧晚期 + He 与 C 燃烧共同驱动（Ritossa et al. 1996）
- 10.5 M☉ 模型：三种能量源均起作用（Iben, Ritossa, García-Berro 1997）
...
[FACT] **闪初期**：14N 完全通过 14N(α,γ)18F(β+,n)18O(α,γ)22Ne 转化为 22Ne

[FACT] **两个 s 过程中子源**：

| 中子源 | 触发条件 | 适用模型 | 关键数值 |
|---|---|---|---|
| **22Ne(α,n)25Mg** | 对流区底部温度 > 3.5×10^8 K | CO 核质量 ≥ 0.9 M☉ 的 AGB 星 | s 过程同位素丰度比太阳大几百倍 |
| **13C(α,n)16O** | 13C 口袋 (pocket) 形成后，~1.5×10^8 K | CO 核质量 < 0.9 M☉ 的 AGB 星 | 主要 s 过程中子源 |

[FACT] **B2FH 的历史性误差**：B2FH 猜测 21Ne(α,n)24Mg 可能是恒星中子源，**但完全没有考虑 22Ne(α,n)25Mg**（Cameron 1961 之后才被认识）
...
- 这意味着 **12C(α,γ)16O / 3α 比率的不确定性对挖掘物质中 C 丰度的影响远小于对 C/O 比的影响**（重要结论）

[FACT] **3He 与 7Li 在中等质量星中的产生**：
- 3He 在主序星中心区域产生（Iben 1967b），比太阳低质量端（1–2 M☉）的 AGB 星保存并释放至 ISM——与类星体星云中高的 3He/H 比一致（Balsar et al. 1997）
- 7Li：高质 AGB 星中，3He + 4He → 7Be 在对流包层底部发生；若 7Be 能混合至较冷区域，7Be(e−,ν)7Li 破坏 → Li 超丰（Cameron 1955; Cameron & Fowler 1971）
- [FACT] Sackmann & Boothroyd (1992) 构建 3–7 M☉ 演化模型，发现初始质量 **4–6 M☉** 的模型可重现 LMC 超锂巨星观测（Smith & Lambert 1989, 1990; Plez et al. 1993; Smith et al. 1995）
...
[FACT] 大质量星（>11 M☉）通过强烈的辐射星风 (radiative wind) 进行质量损失（Cassinelli 1979）：
- 质量损失率随初始质量与光度增加，可达 ~几 ×10^(−5) M☉/yr
- 对于 ≥30 M☉ 的星，质量损失时标与核燃烧寿命相当或更短——模型**必须**考虑质量损失

[FACT] **Wolf-Rayet (WR) 星**：
- 大质量星最终失去 H 富集包层，暴露完全 H 燃烧物质（WN 类：He+N 谱线）与部分 He 燃烧物质（WC 类：He+C+O 谱线）
- Langer (1989a,b) 的研究结论：
  - 核心质量范围：5 M☉ < M_WR < 20 M☉（与初始质量无关）
  - 质量损失率：Ṁ_WR ≈ (0.6–1.0)×10^(−7) (M_WR/M☉)^2.5 M☉/yr
...
[FACT] **初始质量 >50 M☉ 的单星在超新星爆发前不成为巨星**——这对大质量双星演化有重要含义

**SN 爆炸后的演化与核合成**：

[FACT] 大质量星内部演化（13 M☉ 以上）：
- 依次安静燃烧：C → Ne → O → Si
- 最终形成铁峰同位素（统计平衡态）的核心
- Fe-Ni 核收缩加热 → 部分光致分解为 α 粒子和中子
- 核心塌缩至核物质密度 → **SN II 爆炸**

[FACT] **SN 1987A 的关键证据**：
- 初始质量 ~20 M☉ 的星爆炸时，**~0.1 M☉ 的 56Fe 被抛射至 ISM**（Arnett et al. 1989 光变曲线分析）
- 56Ni → β 衰变 → 56Co → β 衰变 → 56Fe
- 激发态核能级退激产生的 γ 射线帮助供能光变曲线
...
- **本文采用具体假设**：M_crit = 40 M☉，M_NS = 1.4 M☉，M_BH = 10 M☉
...
[FACT] **CE 效率参数**：α_CE = ΔE_remove / ΔE_bind
- 越小 → 轨道收缩越大
- 近几十年有争议（Iben & Livio 1993）
- 3D 光滑粒子流体动力学计算建议 α_CE ≈ 1（Rasio & Livio 1996; Yorke et al. 1995）

[FACT] 银河系盘星出生函数（Iben & Tutukov 1984）：
$$\frac{d^3 n}{d \log A_0 \, dM_{10} \, dq_0} \sim 0.2 \, M_{10}^{-2.5} \, \text{yr}^{-1}$$
- 积分 A₀ = 10^−1–10^6, M₁₀ = 0.8–100, q₀ = 0–1 → n ≈ 1 yr^(−1)（银河系主序逃逸率）
...
- Paczynski-Uus 关系（用于 PN 中心星）：L_max/L☉ = 59,000 (M_WD/M☉)^0.52
- 差异源于新星底层 WD 更冷、半径更小
...
- 两颗中等质量主序星 → 两次 CE 事件 → 一对总质量 > Chandrasekhar 质量 (~1.4 M☉) 的 WD → GWR 使它们在 <Hubble 时间内合并 → 爆炸
...
- **双星场景**（Webbink 1984; Iben & Tutukov 1984a,b, 1996a,b）：CO WD + He WD 合并 → 混合 → He 燃烧使 CO 核增长 → 若合并产物质量 >0.78 M☉ → 进入 R CrB 区域
...
- 理论出生率：~2×10^(−5) yr^(−1)（α_CE=1）至 ~2×10^(−6) yr^(−1)（α_CE=0.5）
- NS 吸积 ~0.01 M☉ → 自转周期降至毫秒范围 (<10^3×10^(-3) s) → 毫秒脉冲星 (MSP)
- 主序供星 → MSP + 供星被摧毁
- 亚巨星供星 → MSP + He WD 残余
...
4. **大质量星的初始质量–最终产物（NS/BH）映射仍未理论化**——需要假设（本文取 M_crit = 40 M☉）
5. **密近双星演化**产生了丰富的观测现象（CV, SN Ia, R CrB, LMXB, HMXB, MSP, 双 NS）
6. **SN 1987A 光变曲线分析**证明 SN II 是宇宙铁的主要来源（~0.1 M☉ 56Fe/颗）
...
| He 燃烧时标/主序寿命 | 10–25% |
| He 燃烧温度敏感性 | ∝ T^(40) |
| AGB 超风质量损失率 | 10^(−5)–10^(−4) M☉/yr |
| 触发超风的脉动周期 | > 400 d |
| WD 形成率 | ~0.5–1 yr^(−1) |
| WD 质量分布峰值 | 0.55–0.65 M☉ |
| 12C(α,γ)16O 速率确定性 | B2FH 后 40 年仍未确定 |
| CO 核质量 s 过程中子源阈值 | ≥ 0.9 M☉（22Ne 源） |
| 22Ne 源触发温度 | > 3.5×10^8 K |
| 13C 源触发温度 | ~ 1.5×10^8 K |
| 挖掘物质 12C 丰度 | 0.15–0.25（质量分数） |
| WR 星质量损失率 | ~10^(−7) M☉/yr |
| M_crit（NS/BH 分界）| 40 M☉（本文假设） |
| NS 平均引力质量 | 1.35 ± 0.27 M☉ |
| SN 1987A 56Fe 抛射量 | ~0.1 M☉ |
| CV 出生率 | ~10^(−3) yr^(−1) |
