...
[FACT] Gies & Lambert (1992) 用 McDonald 2.1 m 高质量 Reticon 光谱对 39 颗早 B 星做双通道（Kurucz-LTE / Becker & Butler non-LTE）CNO 丰度分析，发现非超巨星 B 星丰度 ≈ 太阳 ≈ Orion 星云丰度，仅少数非超巨星和全部 5 颗超巨星显示 CN-cycled 部分混合的 N 富集特征；**不能**确认 Lyubimkov 关于 N 随演化年龄系统性增大的主张，并证明其主张可用"恒星参数固定假设（log g=4, $\xi$=0）"产生的伪相关完美复现。
...
- **log $\epsilon$(He)** ≈ 11.00 ± 0.11（29 星）—— 与太阳一致
- **log $\epsilon$(C)** ≈ 8.15 — 略低于太阳（8.4），与 Orion Nebula 一致
- **log $\epsilon$(N)** ≈ 8.15 — 略低于太阳（8.4），与 Orion Nebula 一致
- **log $\epsilon$(O)** ≈ 8.80 — 与太阳（8.69）一致

[FACT] 8 颗 Orion 关联星直接对比 Baldwin et al. (1991)：$\Delta$ log $\epsilon$ ≈ 0（He −0.01、C −0.10、N −0.28、O +0.07）。

### 15.4.2 Lyubimkov 主张不成立

[FACT] 用本文自身数据 + Lyubimkov 的**固定假设**（log g=4, $\xi$=0）重演：
- 得到 N vs 年龄斜率 **0.035 dex per $10^{6}$ yr**——与 Lyubimkov 方向一致；
- **用正确的非 LTE + 温度修正丰度**：无系统相关；
- 效应量化：演化到 log g=3.5 → 假增 0.32 dex；$\xi$=5 → 假增 0.22 dex；组合 log g=3.5 + $\xi$=10 → 假增 **0.90 dex**。

### 15.4.3 少数 N 富集星 = 部分 CN-cycled 混合

[FACT] $\rho$ Leo (HD 91316) 最 N 富集：+0.76 (LTE) / +0.60 (NLTE) dex。
[FACT] Maeder & Meynet 预测对应变化：He +0.10、C −0.10、O −0.04 (non-LTE)。
[FACT] $\rho$ Leo 观测：He 略高、C 略低、O 近均值——**定量匹配 CN-cycle 部分混合**。
...
3. **对 Lyubimkov 的"自证性"反证**：用**自己**的观测数据 + Lyubimkov 的方法学假设（log g=4, $\xi$=0）复现其相关性，证明是伪相关——这比"用新数据反驳"强得多；
4. **CN-cycle 定量匹配**：对 $\rho$ Leo 用 Maeder & Meynet 演化轨预测与观测的 He/C/N/O 变化做定量匹配，为部分混合的观测证据；
...
11. **$\rho$ Leo (HD 91316)**：最 N 富集（+0.60 NLTE），其 He/C/O 变化与 Maeder & Meynet 预测定量匹配 → **CN-cycle 部分混合**。
12. **5 颗超巨星全部 N 富集** + 可能 He 富集，但 C/N 未达 CN-cycle 平衡 → 部分混合但未到红巨星。
13. **Orion 星云直接对比**（8 星）：$\Delta$ log $\epsilon$ ≈ 0 在测量不确定度内 → 年轻 B 星丰度 ≈ 电离气体丰度。
14. **Si II 丰度异常**：log $\epsilon$(Si II) = 6.86 显著低于 Si III/IV (~7.5)；用 non-LTE 也未改善。
...
1. **CN-cycle 平衡态的观测标尺**：$\rho$ Leo 等 N-rich 星的 He/C/N/O 丰度比给出了 CN-cycle 部分混合的观测参照；
...
| 基线 | 值 |
|------|-----|
| 早 B 星 log $\epsilon$(He) | 11.00 ± 0.11 |
| 早 B 星 log $\epsilon$(C) | 8.15 |
| 早 B 星 log $\epsilon$(N) | 8.15 |
| 早 B 星 log $\epsilon$(O) | 8.80 |
| 早 B 星 log $\epsilon$(Ne) | 8.41 ± 0.13 |
| 早 B 星 log $\epsilon$(Al) | ~6.5 |
| 早 B 星 log $\epsilon$(Si) (Si III) | 7.69 ± 0.40 |
| 温度尺度修正 | $\Delta$T/T = 0.034 |
| 超巨星 log g 范围 | 2.1–3.7 |

### 16.4 可以参考的分析方法

- 用**电离态一致性**（如 N I vs N II）检验温度/重力；
- 用**删选规则**（EW 阈值、>2$\sigma$ 离群值、电离态-温度一致性）剔除可疑数据；
...
| 微湍流 | log $\epsilon$ vs EW 斜率 | 加权平均 $\xi$ |
