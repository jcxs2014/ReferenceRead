...
[FACT] 最简单选择 $\Delta$T_eff = 常数，但作者改用 **$\Delta$T_eff = f · T_eff**（分数修正），因部分趋势在热端更陡。
...
- 纵轴：log $\epsilon$ − log $\epsilon$(太阳)
- 虚线：每个物种的最优 f 拟合
- 实线：统一 f = 0.034 的拟合

[FACT] **修正后丰度分布**（Figure 15）：
- 直方图展示修正后 log $\epsilon$ − log $\epsilon$(太阳)，实心线 = LTE，阴影区 = non-LTE
...
- **$\Delta$ log $\epsilon$ = log $\epsilon$(B 星) − log $\epsilon$(Orion)**：
  - He：**+0.06**
  - C：**−0.13**
  - N：**−0.13**
  - O：**+0.10**
...
- 均值：$\Delta$ log $\epsilon$ = **−0.01 (He)**、**−0.10 (C)**、**−0.28 (N)**、**+0.07 (O)**
- 即 **$\Delta$ log $\epsilon$ ≈ 0 在测量不确定度内**

### 7.2.2 其他元素

[FACT] **Ne**：基于一条弱线 Ne I $\lambda$6506.5，仅 11 颗星可用。修正后 LTE Ne 丰度 **log $\epsilon$(Ne) = 8.41 ± 0.13**（基于 11 星）。Baldwin et al. (1991) 的 Orion 星云 Ne 显著高 $\Delta$ log $\epsilon$ = −0.63，但 Ferland (1991) 质疑其可靠性（H II 区 ionization 结构不自洽），Rubin et al. (1991) 用更高温源得到与 B 星一致的 Ne → **本文不赋予此差异天体物理意义**。

[FACT] **Al III**：
- Dufton et al. (1990)：log $\epsilon$(Al) = 6.1 ± 0.4 与 6.2 ± 0.2（两个 OB 团）；
- Peters (1976)：$\gamma$ Peg 八条 Al III 线得 6.47；
- Sadakane, Takada & Jugaku (1983)：Al II/III UV 共振线得 6.5（六颗 B9 正常星）。

[FACT] **Fe**：Orion 星云的 Fe 低是由于 H II 区内 Fe 进入尘埃，不影响 B 星 Fe 丰度。

[FACT] **与太阳差异**：太阳与 Orion 星云丰度可能有 $\Delta$ log $\epsilon$ 差异，来源可能是**银河系气体混合不均匀**（Boesgaard 1989、Nissen 1988）——年轻疏散星 [Fe/H] 散布 ±0.1 dex 在同一 age。若太阳从富金属气体形成，则可解释太阳与 Orion 差异。

---

## 7.3 Evolutionary Changes in Abundance（§ 7.3）

### 7.3.1 最 N 富集星：$\rho$ Leo (HD 91316)

[FACT] **$\rho$ Leo (HD 91316)** 是最 N 富集星：
...
[FACT] $\rho$ Leo 的观测值：He 略高、C 略低、O 近均值（与 Maeder & Meynet 预测一致）。Walborn (1976) 已定性指出 $\rho$ Leo 是中等 N 增强。
...
[FACT] **对 Lyubimkov 的"事后解释"**：Lyubimkov 用了 Dufton & Hibbert (1981) 表，该表**只给出 log g = 4.0 且 $\xi$ = 0** 的预测。在演化星中：
- **log g 更低**（演化星膨胀，表面重力下降）→ 线变强
- **$\xi$ 更大**（演化星更强线，微湍流增大）→ 线变强
...
- log g → 3.5：**+0.32 dex**
- $\xi$ → 5：**+0.22 dex**
- log g → 3.5 **且** $\xi$ → 10：**+0.90 dex**

[FACT] **用本文自身数据重演**：对 16 颗 Lyubimkov 样本星（质量范围 8.5–13.0 $M_\odot$ 匹配 Lyubimkov 中质量组）：
- 用本文 N II $\lambda$4630 实测 EW + 本文 T_eff，**但** 假设 log g = 4、$\xi$ = 0 推导 N 丰度；
...
[FACT] Maeder (1987b) 认为 20–40 $M_\odot$ 星**只有到红巨星阶段**才发生大规模 CN-cycled 表面混合；蓝超巨星若完全富集，则代表其正在 HR 图上 blueward loop 演化。
...
|------|-----|
| 温度修正 | f = 0.034 ± 0.015 |
| 修正方法 | $\Delta$T_eff = f · T_eff |
| 与 Orion Nebula 差异（Baldwin 1991） | He +0.06, C −0.13, N −0.13, O +0.10, S +0.09 |
| 与 Orion 星云直接比较（8 星） | $\Delta$ log $\epsilon$ ≈ 0 |
| $\rho$ Leo N 富集 | +0.76 (LTE) / +0.60 (NLTE) |
| $\rho$ Leo 预期 He/C/O 变化 | He +0.10, C −0.10, O −0.04 (NLTE) |
| Lyubimkov 假象：N II $\lambda$4630 W$\lambda$ | log g=4, $\xi$=0：57 mÅ |
| 演化到 log g=3.5 | W$\lambda$=75 mÅ → 假增量 0.32 dex |
| 演化到 $\xi$=10 | W$\lambda$=88 mÅ → 假增量 0.90 dex（组合） |
| Lyubimkov 假相关斜率复现 | 0.035 dex per $10^{6}$ yr |
| 与 Kilian et al. 1991a 一致性 | T_eff 修正后良好一致 |
| Ne 均值（11 星） | log $\epsilon$(Ne) = 8.41 ± 0.13 |
...
3. $\rho$ Leo 案例的定量一致性（+0.60 N → +0.10 He、−0.10 C、−0.04 O）**几乎完美匹配 Maeder & Meynet 预测**，是 N-rich 星 = 部分混合 CN-cycled 物质的强观测证据；
