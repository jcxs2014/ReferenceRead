...
├── 03_stellar_parameters.md    § 3 有效温度与表面重力（Strömgren + H$\beta$）
...
1. INTRODUCTION
   ...
   - Lyubimkov (1984) 的核心方法：Kane et al. EW + Dufton & Hibbert non-LTE 表 + log g=4.0 + $\xi$=0
   - 关键数值：13–20 $M_\odot$ 星 log $\epsilon$(N) 从 7.6→8.6 在 <10^7 yr；d log $\epsilon$(N)/dt = 0.15/0.06/0.024 dex per 10^6 yr
...
4. PROJECTED ROTATIONAL VELOCITIES
   ...
   - 以 HD 35299 为基准；线性 limb-darkening $\epsilon$=0.28（Gray 1976）
...
7. RESULTS AND DISCUSSION
   7.1 Temperature Scale
       - C II/N II/O II 丰度随 T_eff 单调下降 → 温度偏小 → 采用 $\Delta$T = f·T_eff 修正
       - 式 (7)：$\Delta$ log $\epsilon$ = (∂W$\lambda$/∂T)/(∂W$\lambda$/∂ log $\epsilon$) · $\Delta$T_eff
       - 式 (8)：log $\epsilon$(T_eff) = ⟨log $\epsilon$⟩ + f · (∂W$\lambda$/∂T)(∂ log $\epsilon$/∂W$\lambda$)·$\Delta$T
       - 最佳拟合 f = 0.034 ± 0.015（表 10）
       - 修正后 log $\epsilon$ 分布与 solar system 比较（图 15）
       - 与 Orion Nebula 比较（表 12，8 颗 Orion 关联星：$\Delta$ log $\epsilon$ ≈ 0）
   7.2 Comparison with H II Regions and the Sun
       - Orion Nebula：5 项分析对比
       - Baldwin et al. 1991：$\Delta$ log $\epsilon$ = +0.06 (He), −0.13 (C), −0.13 (N), +0.10 (O), +0.09 (S)
       - Meyer (1989)：O 的 20%±5% 进入尘埃 → 星与星云丰度趋同
       - Ne、Fe 例外（Ne 因 ionization 结构问题，Fe 因尘埃）
   7.3 Evolutionary Changes in Abundance
       - $\rho$ Leo (HD 91316) 最 N 富集：+0.76 dex LTE / +0.60 dex NLTE
       - Maeder & Meynet (1988) 模型：N+0.60 → He+0.10、C−0.10、O−0.04 (non-LTE)
       - 图 16：C 与 N 的 anti-correlation
       - 图 17–20：He/C/N/O 在 HR 图中的分布
       - 图 21–22：Lyubimkov 效应重演 — 用 log g=4、$\xi$=0 得到 slope=0.035 dex per $10^{6}$ yr，正是忽略演化后 log g 下降、$\xi$ 增大所致
...
[FACT] Gies & Lambert (1992) 使用 McDonald 2.1 m coudé 光谱仪对 39 颗早 B 型星（O9–B3，含 5 颗超巨星）的 C II、N II、O II 弱线测量等值宽度，通过 Kurucz LTE 大气 + WIDTH6 与 Becker & Butler non-LTE 表双通道计算丰度，发现非超巨星 B 星的 He/C/N/O 丰度与 Orion 星云一致（近似太阳丰度），仅少量非超巨星与全部 5 颗超巨星显示 CN-cycled 的 N 富集特征；他们**不能**证实 Lyubimkov (1984) 关于 N 丰度随演化年龄系统性增大的主张，并证明该主张可用恒星演化过程中 log g 下降 + $\xi$ 增大所导致的"假象"完美复现。
...
2. **温度尺度**：用丰度-温度趋势反推 $\Delta$T = 3.4%·T_eff 的温度修正，成为后续同类研究的参考；
