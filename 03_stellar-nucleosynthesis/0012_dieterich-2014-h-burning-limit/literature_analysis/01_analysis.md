...
5. 模板 × 修正函数 → 新 SED；
6. 迭代直到所有波段残差 < 2%；
7. 归一化：先把模型通量归一到 $10^{-10}$ erg s⁻¹ cm⁻² 量级（避免数值溢出）；
8. 需要 3–20+ 次迭代；
...
[FACT] 关键公式：
> **L = 4$\pi$R² $\sigma_{\rm SB}$ T_eff⁴**
> $\sigma_{\rm SB}$ = $5.6704\times10^{-5}$ erg cm⁻² s⁻¹ K⁻⁴
...
1. **Stefan–Boltzmann（半径）**：L = 4$\pi$R² $\sigma_{\rm SB}$ T_eff⁴，$\sigma_{\rm SB}$ = $5.6704\times10^{-5}$ erg cm⁻² s⁻¹ K⁻⁴
2. **等效温度定义**：F = $\sigma_{\rm SB}$ T⁴（黑体通量等同）
...
7. **颜色–绝对星等**（示例 MV vs V−R）：
   MV = 0.21509(V−R)³ − 2.81698(V−R)² + 14.16273(V−R) − 1.45226，$\sigma$ = 0.53 mag，范围 1.61 ≤ (V−R) ≤ 3.64
