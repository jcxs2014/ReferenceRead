...
- (V−I) 上：存在趋势，拟合二次多项式：
  > **(V − IB) = −0.0364(V − IC)² + 1.4722(V − IC) − 1.3563**
...
[FACT] 算法流程：
1. 观测 9 波段（V, R, I, J, H, Ks, W1, W2, W3）→ 组合 **36 种颜色**，覆盖 ~0.4–16.7 μm；
...
5. 模板 × 修正函数 → 新 SED；
6. 迭代直到所有波段残差 < 2%；
7. 归一化：先把模型通量归一到 10⁻¹⁰ erg s⁻¹ cm⁻² 量级（避免数值溢出）；
8. 需要 3–20+ 次迭代；
...
- Cushing et al. 2008（模型大气拟合 0.6–14.5 μm 光谱）
- Rajpurohit et al. 2013（BT-Settl 匹配 0.52–1.0 μm 光谱）
...
[FACT] 示例公式（MV vs V−R）：
> **MV = 0.21509(V−R)³ − 2.81698(V−R)² + 14.16273(V−R) − 1.45226 (±0.53)**
> 适用范围：1.61 ≤ (V−R) ≤ 3.64

[FACT] Table 7 完整系数（6 种绝对星等 MV, MR, MI, MJ, MH, MK × 9 种颜色组合 = 54 组多项式）。1σ 不确定度范围 0.24–1.11 mag：
...
| 17 | 2MASS J0523−1403 | L2.5 | **半径极小**；有射电 + Hα 发射（Berger 2002 等），但本文 I 波段变率上界 ~11.7 mmag |
...
| 41 | Kelu-1AB | L2.0 | 已知 L2/L4 双星；锂检验（Li λ6708）→ 各 ≲0.06 M⊙，但 Li 检测"tenuous"；等光度分解则各 ~0.089 R⊙，进一步约束极小位置 |
...
1. **Stefan–Boltzmann（半径）**：L = 4πR² σ_SB T_eff⁴，σ_SB = 5.6704 × 10⁻⁵ erg cm⁻² s⁻¹ K⁻⁴
2. **等效温度定义**：F = σ_SB T⁴（黑体通量等同）
...
5. **迭代 SED 光度**：模板 × poly9(λ, 修正因子) → 迭代至各波段残差 < 2%
6. **黑体外推修正**：F_total = F_μm / f_bb(Teff)，其中 f_bb 是黑体在模板波长覆盖的辐射占比（典型 ~1.5% 修正）
7. **颜色–绝对星等**（示例 MV vs V−R）：
   MV = 0.21509(V−R)³ − 2.81698(V−R)² + 14.16273(V−R) − 1.45226，σ = 0.53 mag，范围 1.61 ≤ (V−R) ≤ 3.64
