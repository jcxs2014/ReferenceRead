...
- **inflow 时间尺度**：~t₀ = 5 × 10⁹ yr（量级）可复现银河系金属丰度梯度。
...
[CRITIQUE] "即时回收"近似在 α 元素 vs Fe 的相对丰度问题上失败，因为这两种元素的产生时标相差 ~10⁶ 倍。这一困难直到 1980s 才通过 "delayed SN Ia channel" 模型被解决（Thomson 1981, Tinsley 1979）。
