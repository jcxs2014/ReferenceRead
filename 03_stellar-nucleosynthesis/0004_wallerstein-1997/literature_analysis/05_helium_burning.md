...
[FACT] 氦燃烧终止于 16O，因为：
- 16O(α,γ)20Ne 速率在 $T_{9}$ < 0.2 时远低于 12C(α,γ)16O（Fowler et al. 1975）
...
[FACT] **两阶段过程**：
1. 两个 α 粒子在 8Be 基态（非束缚，半衰期 $t_{1}$/₂ = 0.97×10^(-16) s）中短暂共存
...
- η = $Z_{1}$$Z_{2}$e²/ℏv（Sommerfeld 库仑参数）
...
| 16O(α,γ)20Ne 相对速率 | << 12C(α,γ)16O ($T_{9}$ < 0.2) |