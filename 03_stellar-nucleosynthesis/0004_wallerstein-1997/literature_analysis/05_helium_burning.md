...
[FACT] 氦燃烧终止于 16O，因为：
- 16O(α,γ)20Ne 速率在 T₉ < 0.2 时远低于 12C(α,γ)16O（Fowler et al. 1975）
- 16O 阈值附近的能级具有错误的宇称/角动量 → 无法共振增强

[FACT] **12C(α,γ)16O 反应率是核天体物理当今最重要的未解决问题之一**：
- 决定 12C→16O 转化比例
- 决定大质量星抛射物与白矮星内部的 C/O 丰度比
- 决定大质量星后续重离子燃烧阶段的初始条件
- 许多更重元素（直至铁）的丰度都敏感依赖此速率（Weaver & Woosley 1993）

## 5.2 A. Triple-α capture

[FACT] **两阶段过程**：
1. 两个 α 粒子在 8Be 基态（非束缚，半衰期 t₁/₂ = 0.97×10^(-16) s）中短暂共存
2. 第三个 α 粒子在 8Be 衰变前被俘获，形成 12C** (Ex = 7.6542 MeV, J^π = 0⁺)
...
- Q = (M_12C** − 3M_α)c² = 379.5 ± 0.3 keV
- Γ_γ = (3.64 ± 0.50) meV
- Γ_pair = (60.5 ± 3.9) meV
- Γ_rad = Γ_γ + Γ_pair << Γ_α ≈ Γ → 速率仅依赖 Γ_rad

[FACT] **B2FH 时代低估此速率**：Γ_γ = 1 meV（偏低）部分被当时较小的 Q 值（372 keV，而非 379.5 keV）的指数因子补偿
...
- η = Z₁Z₂e²/ℏv（Sommerfeld 库仑参数）
- S_cap(E) = S_E1(E) + S_E2(E)（E1 + E2 多极分量）

[FACT] **Gamow 能量**：在氦燃烧温度 T₉ = 0.2–0.6 下，E₀ = 0.3–0.9 MeV

[FACT] 由于库仑势垒大，直接测量无法延伸至 E₀，**必须靠理论外推**
...
**Table II: E1 外推值 S_E1(E₀ = 0.3 MeV)**：

| 参考 | S_E1 (keV·b) | 方法 |
|---|---|---|
| Dyer & Barnes (1974) | 140^{−120}_{+140} (3-level R-matrix) | σ(90°) 测量 + E2 修正 |
| Redder et al. (1987) | 200^{−110}_{+270} (3-level R-matrix) | σ(θ) 测量，分离 E1/E2 |
| Kremer et al. (1988) | 10^{−10}_{+130} (3-level R-matrix + l=1 相移约束) | σ(90°) + γ 反冲符合 |
| **Ouellet et al. (1996)** | **79 ± 16** | **R/K-matrix + β 延迟 α 谱** |

[FACT] **16N β 延迟 α 谱**（Buchmann 1993, Zhao 1993/1995, France 1997）：
- 谱的次极大反映亚阈值 1⁻ 态的存在
- 谱公式：
...
- 窄共振（Γ = 0.625 keV）在 Ex = 9.8445 MeV

[FACT] E2 强度近似中心在 Ex ≈ 15 MeV，展宽 ~15 MeV → E2 主导贡献来自**远距离能级和直接俘获**

**Table III: E2 外推值 S_E2(E₀ = 0.3 MeV)**：

| 参考 | S_E2 (keV·b) | 方法 |
|---|---|---|
| Redder et al. (1987) | 96^{−30}_{+24} (direct capture + 单能级 Breit-Wigner) | σ(θ) |
| Ouellet et al. (1996) | 36 ± 6 | 微观团簇模型 |
| Trautvetter et al. (1996) | 14.5^{−14}_{+96} (R-matrix, 含 Ex < 11.52 MeV 的 2⁻ 能级) | σ(θ) |
...
- S_E1(E₀ = 0.3 MeV) = (79 ± 16) keV·b
- S_E2(E₀ = 0.3 MeV) = (36 ± 6) keV·b
- **S_cap(E₀) = (120 ± 40) keV·b**（加上级联贡献 20 ± 9 keV·b）
...
[FACT] **40 年后 B2FH 的核心问题仍未完全解决**：12C(α,γ)16O 反应率的不确定度仍是核天体物理最大单一不确定度

**核心数据**：
- S_cap(0.3 MeV) = 120 ± 40 keV·b
- E1 部分：79 ± 16 keV·b（Ouellet 1996，含 β 延迟 α 谱约束）
- E2 部分：36 ± 6 keV·b

**未来方向**（1997 年视角）：
- 更多 β 延迟 α 谱数据（TRIUMF vs Yale/UConn 的矛盾）
...
| Q(3α) | 379.5 ± 0.3 keV |
| Γ_γ(7.65 MeV) | 3.64 ± 0.50 meV |
| Γ_pair(7.65 MeV) | 60.5 ± 3.9 meV |
| S_cap(0.3 MeV) 推荐值 | 120 ± 40 keV·b |
| S_E1(0.3 MeV) | 79 ± 16 keV·b |
| S_E2(0.3 MeV) | 36 ± 6 keV·b |
| Gamov 峰 (T₉=0.2–0.6) | E₀ = 0.3–0.9 MeV |
| 16O(α,γ)20Ne 相对速率 | << 12C(α,γ)16O (T₉ < 0.2) |
