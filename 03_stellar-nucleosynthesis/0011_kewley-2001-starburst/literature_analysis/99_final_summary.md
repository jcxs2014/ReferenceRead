...
1. **观测约束**：1–4 Ry 区间 EUV 必须较硬（log([O III]/Hβ) 高值）；
2. **模型判别**：PEGASE (Padova + PNN) 是唯一覆盖几乎所有观测点的模型；Geneva+Lejeune 被排除；
3. **He II λ4686 约束**：模板星暴 log(He II λ4686/Hβ) ≈ −1.6，与 Schmutz 大气预测（−1.7）**一致**，与 PEGASE/Lejeune 预测（−6）**相差 5 个量级**；
4. **SNR 贡献**：Hβ 16–20%，但 [O III]/Hβ 贡献仅 ~2%（可忽略），不足以解释诊断图差异；
...
- **Guseva, Izotov & Thuan 2000**：高空间分辨率 W-R 星系观测——本文模板平均光谱独立印证 He II λ4686；
- **Kennicutt 1998**：SFR 定标标准方法；
- **González Delgado & Leitherer 1999**：星暴年龄与 Hβ 吸收等值宽度关系，用于 §5 年龄上限；
...
- **诊断图网格分析**：系统性扫描 Z-χ 二维网格，找出"禁带"——直接定位需要额外物理过程（激波、AGN、金属不透明度）。
...
- 1–4 Ry（54–756 eV）区间是诊断图对 EUV 谱的**关键灵敏区间**——高于 He II 电离极限的光子（>4 Ry）对 [N II]/Hα 等比值贡献小；
- 诊断图上的"折回"（fold）是两参数网格（Z-χ）的**内在拓扑**——任何额外激发机制（激波、AGN、金属屏蔽）的引入都能让模型跳出此折回。

## 16.4 系统误差处理借鉴

- 用**两套独立代码**（不同演化轨、大气、IMF 默认值）交叉检验理论预测的鲁棒性；
- 用**观测锚点**（He II λ4686）直接约束大气模型选择，而非仅依赖诊断图拟合；
...
- He II λ4686 观测对 W-R 星大气的验证，直接关系到 W-R 星在化学演化中的**质量损失与核合成产物释放率**；
...
| 关键数值（SFR、χ 范围、Z 范围、He II/Hβ、SNR 数目） | ✓ |
| 论证链重建 | ✓ |
| 关键参考文献与作用 | ✓ |
| 隐含假设与信息缺失 | ✓ |
| 中文写作 | ✓ |
