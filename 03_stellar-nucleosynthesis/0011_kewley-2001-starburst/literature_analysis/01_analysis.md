> 本章属于：[[03_stellar-nucleosynthesis/0011_kewley-2001-starburst/literature_analysis/00_overview.md|Theoretical Modeling of Starburst Galaxies (Kewley, Dopita, Sutherland, Heisler & Trevena 2001, ApJ 556:121)]]
>
...
- 电离参数范围：$6\times10^{6}$ ≤ $\chi$ ≤ $6\times10^{7}$；
- 金属丰度覆盖 0.2 – 3 Z☉；
...
## 14.1 补充解读与批判（16 条 INTERPRETATION + 10 条 CRITIQUE）

### 补充 [INTERPRETATION]

[INTERPRETATION] 从 §1 的动机链看，Kewley et al. 2001 真正挑战的不是\"BPT 分类\"本身，而是**\"瞬时星暴\"假设能否撑起 BPT 分类**——诊断图对 1–4 Ry 区间的敏感，本质上是把\"连续 vs. 瞬时\"转化为\"观测可分辨的坐标位移\"，使分类方案第一次获得**基于电离辐射场物理的判据**。

[INTERPRETATION] 157 个星暴样本占 IRAS 温热星系 285 个中的 55%——这个高比例说明**星暴（而非 AGN）才是温热红外总星暴星系的光学发射主导成分**，直接支持了\"IRAS 温热 = 恒星形成主导\"这一经验命题，为 §8 用恒星种群合成建模的合理性提供样本层面的证据。

[INTERPRETATION] §3 对 PEGASE 与 STARBURST99 的系统对比（演化轨、大气、He、质量损失）揭示了一个普遍问题：**恒星种群合成代码的\"物理等价性\"在 UV/EUV 段是不成立的**——两套代码的 ZAMS 与热星大气相同，却在 W-R 阶段分道扬镳；这意味着\"用同一组模型做不同诊断\"在不同波段可能给出互相矛盾的答案。

[INTERPRETATION] Padova 相对 Geneva 在更大质量下采用混叠（overshooting）的设定，导致 PEGASE 的 W-R 阶段更早启动、更持续——这是 PEGASE 1–4 Ry 区间更硬的**演化轨层面的解释**，可与 §8 的 continuum metal blanketing（大气层面）一起构成\"EUV 硬度双重来源\"图景。

[INTERPRETATION] §3.4 的密度参数 $(\dot{M}_0/v_\infty)^2 R_*^{-3}$ 揭示了一个反直觉的物理量：更大的 EUV 光子被消耗用于维持扩展大气的电离，导致逃逸出的 EUV **更硬但更弱**——这一\"硬-弱关联\"是 Schmutz 大气在 He II 以上强但 1–4 Ry 区间弱的根本原因，也是诊断图需要区分\"硬度\"与\"强度\"的物理根源。

[INTERPRETATION] §4 的电离参数范围 log U = −3.5 – −2.0 覆盖了河外 H II 区典型值（Dopita et al. 2000），这使 MAPPINGS III 网格**可直接用于河外星系对比**，而无需额外标定——这是本文分类线（公式 5–7）能够推广到河外星系的关键前提。

[INTERPRETATION] MAPPINGS III 自洽处理尘埃（吸收、带电、光电加热）意味着尘埃**既是吸收体也是辐射场再加工者**——在温热 IRAS 星暴（尘埃覆盖因子的关键证据）中，这一自洽处理比纯气体模型更贴近观测，解释了为何作者坚持使用 MAPPINGS III 而非 CLOUDY。

[INTERPRETATION] §4.3 的初级/次级假设把 N 与 He 单独拎出，实质上把\"N/O 转折点\"作为 AGN 与星暴的**化学探针**——在连续星暴（Z 高、N 次级主导）下 N/O 应随 Z 单调上升，这解释了为何 [N II]/Hα 成为诊断图上最敏感的轴。

[INTERPRETATION] §5 发现瞬时模型在诊断图上产生\"禁带\"（无参数组合可到达），这给出了一个**判据性结论**：只要观测点在禁带内，就**必然**混有 W-R 星或 AGN 贡献，无法用简单瞬时星暴解释——这是把诊断图从\"分类工具\"提升为\"物理诊断\"的一步。

[INTERPRETATION] §5.2 中 STARBURST99+Lejeune（c 模型）被\"排除\"是因为 1–4 Ry 太软，这一结果与 §3.5 \"STARBURST99 专为星暴设计\"的表述看似矛盾，实则揭示了**大气模型对诊断图的影响远大于演化轨**——Schmutz 大气虽更贴近 W-R 物理，却不能单靠它解决 1–4 Ry 硬度。

[INTERPRETATION] §6 对 SNR 贡献的量化（Hβ ~16–20%；[O III]/Hβ 下限 ~0.0）给出了一个**反例性结论**：即便星暴星系 SN 率极高，激波机械能对诊断图的位移仍**不足以**把 STARBURST99+Schmutz 推到观测区域——这**排除**了激波作为 EUV 硬度\"万能解药\"的可能。

[INTERPRETATION] 600 km s⁻¹ 激波在 n = 350 cm⁻³ 下典型变辐射时 1 pc、~600 yr——这一\"小尺度、短寿命\"结论说明 SNR 对诊断图的贡献是**空间上高度局部**的，与星暴星系 kpc 尺度平均发射线的形成方式**在时间/空间上都不同步**，因此混入激波成分的假设需谨慎。

[INTERPRETATION] §7 中 He II λ4686/Hβ ≈ −1.6 与 Schmutz 模型预测 −1.7 一致、但与 Lejeune 仅 0.001 的预测 −6 相差 4 个 dex——**He II λ4686 的等值宽度是区分两种大气模型的极强探针**，任何声称 W-R 大气\"影响不大\"的模型都必须在这一点上过关。

[INTERPRETATION] 56 个高 S/N 星系平均得到的模板光谱在 Hβ 吸收 EW ≈ 3.6 Å 上给出 7 Myr 年龄上限——这一**连续星暴年龄锚点**与 §4.2 的 8 Myr 连续模型上限互相印证，是本文\"连续假设\"观测侧的关键证据。

[INTERPRETATION] §8 的 continuum metal blanketing 把部分 >4 Ry 光子重新分布到 1–4 Ry，这在物理上等价于**\"金属线的不透明度把硬辐射重新软化为次硬\"**——它不是\"额外光子源\"，而是\"光子能谱再加工\"，因此其效果天然受金属丰度限制，解释了为何 STARBURST99+Schmutz 在低 Z 时更差。

[INTERPRETATION] §9 的矩形双曲线形式（公式 5–7）在渐近行为上对应**\"[O III]/Hβ 随 [N II]/Hα 变化率反比\"**——即 [N II]/Hα 越接近 −0.47 时 [O III]/Hβ 允许的上限越高，这一数学结构**内嵌**了\"连续星暴 EUV 硬度与金属丰度耦合\"的物理信息。

[INTERPRETATION] 理论星暴线模糊分类率 6%（vs. VO87 的 16%）的改进，部分源于 PEGASE 提供的\"最硬 EUV 上限\"——若 PEGASE 本身被未来更硬的模型取代，理论线的\"最保守\"性质会改变，因此分类线的**稳健性**依赖于当前 PEGASE 硬度的**上限地位**是否成立。

### 补充 [CRITIQUE]

[CRITIQUE] §3.2 对 Padova vs Geneva 的\"混叠\"差异没有给出**对 EUV 硬度的定量分解**——只说\"更硬的 EUV\"，但未在 PEGASE 内部做 Padova-overshooting-on/off 对照实验，因此\"演化轨差异贡献\"与\"大气差异贡献\"是否可分离仍不明确。

[CRITIQUE] §4.2 的电离参数内边界定义（尺寸化 χ）在实际观测星系中**无法独立测量**——U 值只能由线比值反推，这与论文\"用观测约束 EUV 场形状\"的目标构成循环风险：如果观测点落在禁带，反推出的 U 值是无意义的。

[CRITIQUE] §4.3 中\"非太阳丰度下耗竭因子保持不变\"的假设在 Z < 0.2 Z☉ 时被明确承认无依据，但 PEGASE 网格覆盖 0.01–3 Z☉——这意味着**低丰度模型的尘埃吸收可能被严重高估**，进而影响 EUV 场与诊断图位置的可靠性。

[CRITIQUE] §6 采用 $n_e = 350$ cm⁻³ 来自 [S II] 比值，但作者未讨论不同星暴星系 [S II] 比值的分散——若实际密度跨度 100–1000 cm⁻³，则 SNR 冷却时标（$τ_{cool} \propto n$）与 SNR 数目（$N \propto 1/n$）会有 1 个 dex 的变动，公式 (3) 的 SNR 贡献评估的稳健性值得怀疑。

[CRITIQUE] §7.2 中\"选择偏差警告\"——用 56 个高 S/N、蓝截止 < 4620 Å 的星系做模板平均——**必然**偏向高电离参数、低金属丰度、较年轻的星暴，导致 He II λ4686 强度被系统性高估，可能误导 Schmutz 大气模型\"更优\"的结论。

[CRITIQUE] §8 提出 continuum metal blanketing 是\"可能解法之一\"，但未给出**定量测试**（如改变 Schmutz 大气中的 C/Fe 不透明度重新跑 MAPPINGS III）——这使得\"可能\"在论文框架内**无法证伪**，削弱了该主张的科学地位。

[CRITIQUE] §8 的反证逻辑（\"He II λ4686 观测相符 → continuum metal blanketing 解释力受质疑\"）存在一个漏洞：continuum metal blanketing 主要吸收 C 系 > 4 Ry 光子，而 He II λ4686 是复合线（< 4 Ry），**两者物理上并不直接耦合**——因此这个\"反证\"力度比作者承认的要弱。

[CRITIQUE] §9 给出的三个分类公式（5–7）基于 PEGASE 网格、$n_e = 350$ cm⁻³、标准 IMF——若其中任一假设变动，公式系数都会漂移；但 ±0.1 dex 的虚线范围**仅来自\"化学丰度、耗竭因子、IMF 斜率\"的模糊估计**，缺乏来自 MAPPINGS III 参数扫描的严格误差传播。

[CRITIQUE] 全文使用 Salpeter IMF 但未讨论 Kroupa / Chabrier 等**次太阳末端截断** IMF 的影响——由于 > 20 M☉ 的 W-R 星对 EUV 起决定性作用，IMF 上端斜率变化（例如指数从 −2.35 变到 −1.7）会在 PEGASE 与 STARBURST99 间造成显著差异，而本文未量化。

[CRITIQUE] §1 提及 Kewley et al. 2000 与 2001 的观测但**未系统引用**同期独立工作（如 Leitherer 2000 STARBURST99 更新、Renzini 2000 PEGASE 2.0.3），也未讨论 2001 年后出现的 Pannella et al. 2005 等工作对 W-R EUV 的再估计——这使得论文的\"最新性\"局限于 2001 年之前。