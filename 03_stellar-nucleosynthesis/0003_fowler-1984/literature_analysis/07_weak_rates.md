...
[INTERPRETATION] 中子星形成要求质子→中子（n/p 转化），与 §V 中 He 燃烧通过 13C(α,n)16O 产生中子的「重元素合成」形成对称：一个是弱作用主导的质子中子化，一个是强作用主导的中子注入——但两者在 §VII 的核心坍缩中会汇合：核变富中子触发电子俘获。
...
[INTERPRETATION] Tz 核（如 54Fe）的 GT 矩阵元可通过 (n,p) 反应（实验可控）来约束，而 T+ 核则用 (p,n) 反应——这一同位旋对称性策略是 §7.3 中 87 个离散 GT 元从有限实验外推至 226 个核的理论支点。
...
[INTERPRETATION] 从离散态主导（准静态）到 GT 共振主导（坍缩），标志电子化学势 μ_e 已跨过 Fe 峰核的多条离散能级，连续态相空间开始主导——这一转变恰好对应 §VI Si 燃烧向 §VII 核心坍缩的物理过渡。
...
[INTERPRETATION] FFN 的「有效 ft 值」方法把复杂 T,ρ 依赖的弱相互作用速率压缩为近似 T,ρ 无关的表——这正是 §VIII 超新星模拟能使用固定速率表的前提；§VII.6 讨论的中子壳阻塞与 §VII.7 Bethe 等人 1982 的核心坍缩工作共同奠定了 2017 年 GW170817 多信使天文学的理论基础。
