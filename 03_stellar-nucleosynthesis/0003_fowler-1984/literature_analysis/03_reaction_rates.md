...
[INTERPRETATION] A≲30 的上限对应**轻核区**——正是 He/C/O 燃烧阶段的主导核；更重的核（如 Fe 组 A~56）必须用 Hauser-Feshbach 统计模型处理，Fowler 后续 §V–§VI 中 $^{12}{\rm C}+^{12}{\rm C}$、$^{16}{\rm O}+^{16}{\rm O}$ 的 S 因子外推正落在 A=24–32 的边缘区。

[INTERPRETATION] 三反应物程序（如 3$\alpha$ → $^{12}{\rm C}$）将反应写成两步热平衡的准平衡态，隐含假设 $^{8}{\rm Be}$ 的半衰期（~$10^{-16}$ s）远短于反应时标；这与 §V 中 Hoyle 共振对 3$\alpha$ 速率的放大共同解释了为何碳的宇宙丰度远高于无 Hoyle 共振时的预期。

[FACT] 文中明确指出：$\gamma$ 使用了 **Bose-Einstein 统计**；但**简并态电子、中子、质子**的 Fermi-Dirac 统计推广与 **$\alpha$ 的 Bose-Einstein 统计推广未纳入**。

[INTERPRETATION] 这一局限在 §VI 的准静态 Si 燃烧阶段（$T_9$ = 3–5，$\rho$ = $10^7$–$10^9$ g cm$^{-3}$）与 §VII 的核心坍缩阶段变得显著——彼处电子/中子强烈简并，Pauli 阻塞直接改变 $\nu_e$ 俘获率；Bethe-Yahil-Brown (1982) 与 FFN (1980–84) 的表中“有效 ft 值”方法正是对这一缺陷的部分补救。
...
[INTERPRETATION] S 因子的发明把恒星内 10⁻¹⁰ barn 量级、人无法直接进入的能量窗口，转化为可用实验室测量的"外推问题"——这本质上是"用可控实验回答不可达问题"的方法论胜利。
...
[INTERPRETATION] 把 Gamow 穿透因子从实验中剥离，相当于把实验数据从"被量子隧穿效应压制 20–40 个数量级"的混沌中解出来——这解释了为何实验室能量 ~1–2 MeV 的数据能回答恒星 Gamow 峰 ~0.3 MeV 的问题。
...
[INTERPRETATION] 有效能量 $E_{0}$ 同时取决于 $T_{9}$ 与 $Z^{2}A$：恒星温度升高一点（$T_{9}^{2/3}$ 效应）就能把有效能量推到更高、更可控的实验窗口——这正是"测高温下的截面→推断低温恒星速率"的数学依据。
