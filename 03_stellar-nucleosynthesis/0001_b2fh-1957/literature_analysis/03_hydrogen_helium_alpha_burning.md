...
```
R = n_0 · n_1 · <$\sigma$v> = $3.63\times10^{-12}$ · ($X_{0}$·$X_{1}$/√($A_{0}$·$A_{1}$)) · ($\sigma$v)   反应数·cm⁻³·s⁻¹
```

以 $S_{0}$=S($E_{0}$) 表达的恒星温度下的反应率：
```
R = $7.20\times10^{-11}$ · $T_{9}^{\rm -2}$/³ · exp(-3$\tau$) · (A·$Z_{1}^{2}$·$Z_{0}^{2}$)^(-1/3) · f_c · $S_{0}$   反应数·cm⁻³·s⁻¹
```
...
```
1/$\tau_\gamma$(0) = $4.34\times10^{-11}$ · $X_{1}$ · $\rho$ · $S_{0}$ · f_c · (A·$Z_{1}^{2}$·$Z_{0}^{2}$)^(-1/3) · $T_{9}^{\rm -2}$/³ · exp(-$\tau$)   s⁻¹
```
...
```
$\sigma$ = $\pi$$\lambda$̄² · g · ($\Gamma_{0}$·$\Gamma_{2}$)/((E-$E_{0}$)² + $\Gamma^{2}$/4)
...
```
$\sigma_{\rm res}$ = $\pi$$\lambda$̄² · g · 4·$\Gamma_{1}$·$\Gamma_{2}$/$\Gamma^{2}$
```
...
```
$\epsilon_{\rm CN}$ = (0.786 ± 0.16) · (X_C · X_H) · $10^{28}$ erg·s⁻¹·g⁻¹
```
...
**N¹⁴ 循环时间：**
```
$\tau_\gamma$(N¹⁴) = 0.55 · (X_CN/X_CN) · $10^{10}$ yr
...
- Tc⁹⁹ 的存在（Merrill 1952）证明 S 型星正在合成并混合到表面：时间 < Tc⁹⁹ 半衰期（$2\times10^{6}$ yr）。
