...
把前述反应率整合进**大规模反应网络**，对**三种典型天体物理条件**计算产物演化，得到 rp-process 的**端点核素**随 ($T_{9}$, ρ, t) 的变化。
...
## 8.3 章节 5.1: Low Temperatures and Densities ($T_{9}$ = 0.3, ρ = $10^{3}$, t = 10 s)
...
- 初始 ²⁰Ne 主要加工为 **²²Mg**，因长半衰期（$T_{1}$/₂ = **3.86 s**）**被禁闭**。
- 已有 ²⁴Mg 和 ²⁸Si 加工为 **²⁶Si ($T_{1}$/₂ = 2.23 s)** 和 **³⁰S ($T_{1}$/₂ = 1.18 s)**。
- 这些衰变导致 **³¹S ($T_{1}$/₂ = 2.57 s) 富集**。
...
## 8.4 章节 5.2: Intermediate Temperatures and Densities ($T_{9}$ = 0.4, ρ = $10^{4}$, t = 1000 s)
...
- **²⁰Ne** 在过程第 1 秒内耗尽 → 转换为 **²²Mg**（$T_{1}$/₂ = 3.86 s）。
- 因 ²²Mg 可被 (p,γ) 破坏，**有效半衰期更短，$T_{1}$/₂,eff = 2.4 s**。
- 物质在等待点核 **³⁴Ar 和 ⁴⁰Ca** 中转换；丰度在 **~10 s 后达到生产-衰变平衡**。
- 后续因生产流停止，净丰度下降。
- **⁴⁰Ca 显著增强**（因弱衰变反应 ⁴⁰Ca(p,γ)⁴¹Sc）。
- 进一步加工完全转换 ⁴⁰Ca 为 **⁵²Fe**。
- 因 **⁵²Fe 长 β 衰变半衰期 ($T_{1}$/₂ = 8.28 h)** 与弱 ⁵²Fe(p,γ)⁵³Co 衰变率 → **⁵²Fe 是该条件下 rp-process 的端点**。

## 8.5 章节 5.3: High Temperatures and Densities ($T_{9}$ = 1.5, ρ = $10^{6}$, t = 10 s)
...
**[FACT]** 反应继续至 **⁶⁴Ge ($T_{1}$/₂ = 63.7 s)**——在爆发期间**本质稳定**。
...
**[FACT]** 近期测量表明 **⁷³Rb 粒子未束缚** (110) → **rp-process 终止于 ⁷²Kr ($T_{1}$/₂ = 17.2 s)**。
...
| 情形 | $T_{9}$ | ρ (g/cm³) | t (s) | 端点核 | 主要过程 |
|------|-----|-----------|--------|-------|---------|
| (i) | 0.3 | $10^{3}$ | 10 (或 100) | **³²S** (或 ⁴⁰Ca) | 弱 rp-process |
| (ii) | 0.4 | $10^{4}$ | 1000 | **⁵²Fe** | rp-process 完整段 |
| (iii) | 1.5 | $10^{6}$ | 10 | **⁷²Kr** | rp-process + np-process 全开 |

**[INTERPRETATION]** 端点核随 ($T_{9}$, ρ, t) **单调向更重质量移动**——这直观反映了**温度与密度对质子俘获速率的指数增强** + **np-process 在高温下跨越 β 等待点**的双重作用。
...
1. **[FACT]** **np-process**（§5.3）是 Woosley (73) 提出但**本文首次在 $T_{9}$=1.5 网络中显式包含**的过程——它是**rp-process 在 X 射线暴条件下能跨越 β 等待点**的关键机制。
