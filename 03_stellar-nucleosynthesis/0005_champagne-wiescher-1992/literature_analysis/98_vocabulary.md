...
| furthermore | adv. | 递进 | 进一步 | "The rate is uncertain ...; **furthermore**, the J^π assignment is disputed." |
...
| subsequently | adv. | 时序 | 随后 | "Initial CNO → 34Ar **subsequently** → 38Ca via (α,p)." |
...
| given that | conj. | 条件因果 | 鉴于 | "given that Γ_γ ≪ Γ_p, the resonance strength is dominated by Γ_γ." |
...
| S factor | S 因子 | 剥离库仑穿透后反应截面随能量缓慢变化的部分，σ = S(E)/E · exp(−√(E_G/E)) | §2.2 |
| Breit-Wigner resonance | 布赖特-维格纳共振 | 单共振截面标准形式，由入射/出射道分宽度与总宽度决定 | §2.2 |
| ωγ (omega-gamma) | 共振强度 | ω·Γ_a·Γ_b/Γ，用于窄共振 Maxwell 积分后的反应率 | §2.2 |
...
| Thomas-Ehrman shift | 托马斯-厄尔曼位移 | 类比核中质子未束缚态向束缚态的能量移动，用于类比态推 (p,γ) 共振 | §4.2 |
| mirror nucleus / analog state | 镜像核 / 类比态 | 质子-中子交换对称的核，用于预言类比态 J^π 与共振参数 | §4.2 |
| branching ratio b = (p,α)/(p,γ) | 分支比 | 循环反应中 (p,α) 通道的相对强度，循环阻抗的核心量 | §4.5.4 |
...
| stellar β-decaday rate | 恒星 β 衰变率 | 因光致电离与热布居改变后的 β 衰变速率 | §3.6.2 |
...
**修饰**：because of … 引导原因状语（Boltzmann 激发态布居 + 恒星 β 衰变率）；proceeding through a resonance 作后置定语修饰 reactions
