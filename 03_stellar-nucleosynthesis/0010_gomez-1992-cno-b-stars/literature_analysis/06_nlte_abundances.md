...
[FACT] 这些表给出 Wλ 作为 T_eff、log g、ξ、丰度的函数，基于：
- blanketed **LTE 大气模型**
- 但 C II、N II、O II 原子的**详细 non-LTE 能级布居**

[FACT] 丰度计算方法与 § 5 相同：对给定 (T_eff, log g) 找每个 line 在三个 ξ 下的丰度，再选 log ε vs EW 斜率为零的 ξ。
...
[FACT] 表 9 对 39 颗星给出 non-LTE 平均的 log ε(He)、log ε(C)、log ε(N)、log ε(O)，及 σ、Δ 修正。

[FACT] 平均由以下线组成：
- 所有可用 C II 线；
- N II λλ4630–5045 之间（不含 λλ5002, 5025）；
- O II λλ4638–4705 之间。

[FACT] **内部一致性**：不同线给出的丰度基本一致，**除**：
- **C II λλ6578、6582**：丰度比其他 C II 线低 **> 0.3 dex**（异常，作者未纳入平均）

[FACT] **关键 non-LTE 均值**（从表 9 提取的非超巨星代表）：
- log ε(He) ≈ 10.82 – 11.16（中值 ≈ 10.95）
- log ε(C) ≈ 7.83 – 8.58（中值 ≈ 8.15）
- log ε(N) ≈ 7.83 – 8.69（中值 ≈ 8.15）
- log ε(O) ≈ 8.63 – 9.01（中值 ≈ 8.80）
...
[FACT] 该模型提供 3 条观测到的 He I 线（λλ4713, 5015, 5047）在**太阳丰度**下的 LTE 与 non-LTE 预测 EW，以及 ±0.3 dex 丰度偏差下的非 LTE 预测。
...
$W_\lambda(\text{NLTE})$ 与 β 由 Auer & Mihalas 表按 T_eff、log g 插值得到。
...
[FACT] **关键发现**：
- **Si II 异常**：LTE 平均 log ε(Si) = 6.86 ± 0.19，**显著低于** Si III 与 Si IV；用 Becker & Butler (1990) non-LTE 也未改善；
- **Si III**（Lennon et al. 1986 non-LTE）：
  - LTE：log ε(Si) = **7.63 ± 0.26**
  - non-LTE（Table 11）：log ε(Si) = **7.69 ± 0.40**（删两热星 HD 34078, 36512 后）
  - LTE 与 non-LTE 结果**几乎相同**
- **Si IV**（3 颗星）：
  - LTE：log ε(Si) = 7.51 ± 0.36
  - non-LTE（Becker & Butler 1990）：log ε(Si) = 7.47 ± 0.08
  - 与 Si III 一致

[FACT] 作者**建议**：因 Si II 异常未解，**Si III 与 Si IV 的 Si 丰度更可靠**。

[FACT] Dufton et al. (1990) 对 h & ι Per 与 Cep OB3 的 log ε(Si) = 7.5 ± 0.2（1–2 条 Si III/IV 线），与本文 Si III 结果一致。
...
- log ε(He) 明显**更高**（He 富集更甚）；
- log ε(C) 显著变化；
- log ε(N) 也有变化。

[FACT] 示例：HD 51309
- He I：log ε = 11.46 (σ=0.14, n=3)
- C II：log ε = 8.51 (σ=0.07, n=5)
- C III：log ε = 8.63 (σ=0.15, n=12)
- N II：log ε = 8.63 (σ=0.15, n=12)
- N III：log ε = 9.21 (σ=0.10, n=8)
- O II：log ε = 9.21 (σ=0.10, n=8)
- Si II：log ε = 6.79 (σ=0.04, n=2)
...
| He I 表 | WIDTH6 | Auer & Mihalas 1973a（幂律 β≈0.5） |
| Si III | Lennon et al. 1986 non-LTE | 同上 |
| Si IV | — | Becker & Butler 1990 |
| non-LTE 均值 log ε(C) | — | ≈ 8.15 |
| non-LTE 均值 log ε(N) | — | ≈ 8.15 |
| non-LTE 均值 log ε(O) | — | ≈ 8.80 |
| non-LTE 均值 log ε(He) | — | ≈ 10.95（非超巨星） |
| C II λ6578, 6582 异常 | — | 低 > 0.3 dex |
| Si II 异常 | log ε=6.86 | non-LTE 未改善 |
...
3. Si II 的 0.8 dex 异常（log ε=6.86 vs Si III/IV 的 ~7.5）**未被解释**——可能源于 Si II 线的严重 non-LTE 效应、错误 log gf、或尘埃效应（但大气谱无尘埃），这是后续研究需要解决的问题；
