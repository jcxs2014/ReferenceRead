...
- [FACT] Scott et al. (2006) 用弱 CO 分子线 + 自洽 O 丰度确认低 C 值，并给出同位素比 **$^{12}{\rm C}$/$^{13}{\rm C}$ = 86.8 ± 3.8**。
...
- [FACT] Caffau et al. 用 co5bold 3D 模型对 N I 线的分析得到**高 0.05 dex** 的值，主要是**谱线选择不同**：
  - Sauval 剔除混杂线 → $\sigma$ = 0.04 dex
  - Caffau 保留更多线 → $\sigma$ = **0.12 dex**（明显偏高）
...
- [FACT] Scott et al. (2006) 用 CO 线得 **$^{16}{\rm O}$/$^{18}{\rm O}$ = 479 ± 29**（接近地球值 498.7，但误差大）。
- [CRITIQUE] Ayres, Plymate & Keller (2006) 得明显低值 $^{16}{\rm O}$/$^{18}{\rm O}$ = 440 ± 20，可能因 1D 模型未正确处理不同同位素异构体的温度敏感性。
