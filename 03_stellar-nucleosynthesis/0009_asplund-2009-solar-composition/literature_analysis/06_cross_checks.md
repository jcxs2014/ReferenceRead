...
- [FACT] 优势：**质谱精度极高**（含同位素）。
- [FACT] 代价：**挥发元素 H, C, N, O, 惰性气体严重亏损**（恰好是丰度最高的元素）→ 陨石丰度必须以 **Si = 10⁶** 归一。

### 6.1.2 归一化

- [FACT] 陨石原数据：N_Si = 10⁶
- [FACT] 换算到天文标度：**log ε_X = 1.51 + log N_X**（用本文光球 Si 归一，log ε_Si = 7.51）
...
| 方案 | 来源 | 状态 |
|---|---|---|
| 不透明度 +10–20%（R = 0.4–0.7 R⊙, T = 2–5×10⁶ K） | Bahcall et al. 2005 | 需要大幅修正；Opacity Project 比 OPAL 大仅 ~3%，不太可能 |
...
- [FACT] Pena-Garay & Serenelli (2008) 用 SuperKamiokande I+II、SNO、Borexino 数据分析 GS98 vs AGS05 模型：
  - 现有数据给出的 **⁷Be、⁸B 中微子通量介于两模型预测之间**
  - 作者推测太阳中微子会**偏向 AGSS09**（新丰度）。
- [FACT] **更直接检验**：测量 ¹³N、¹⁵O β-衰变中微子（分别来自 ¹²C + p、¹⁴N + p）→ Borexino 未来几年 + SNO+（2011 后）有望给出明确答案。
