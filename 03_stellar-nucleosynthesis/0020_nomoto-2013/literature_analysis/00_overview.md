---
title: "Nucleosynthesis in Stars and the Chemical Enrichment of Galaxies"
authors: "K. Nomoto, C. Kobayashi, N. Tominaga"
year: "2013"
journal: "Annu. Rev. Astron. Astrophys. 51, 457–509"
pages: "457-509"
doi: "10.1146/annurev-astro-082812-140956"
keywords: "first star, galactic archaeology, gamma-ray burst, hypernova, metal-poor star, supernova"
category: "恒星核合成"
status: completed
read_date: "2026-08-16"
lastread: "2026-08-16"
tags:
  - core-collapse supernova
  - EMP stars
  - galactic chemical evolution
  - Pop III
  - hypernova
citations:
- '[[03_stellar-nucleosynthesis/0001_b2fh-1957/literature_analysis/00_overview|0001_b2fh-1957]]'
- '[[03_stellar-nucleosynthesis/0003_fowler-1984/literature_analysis/00_overview|0003_fowler-1984]]'
- '[[03_stellar-nucleosynthesis/0004_wallerstein-1997/literature_analysis/00_overview|0004_wallerstein-1997]]'
- '[[03_stellar-nucleosynthesis/0014_cameron-1968/literature_analysis/00_overview|0014_cameron-1968]]'
- '[[03_stellar-nucleosynthesis/0016_cowan-2021/literature_analysis/00_overview|0016_cowan-2021]]'
- '[[03_stellar-nucleosynthesis/0017_kaeppeler-2011/literature_analysis/00_overview|0017_kaeppeler-2011]]'
- '[[03_stellar-nucleosynthesis/0023_eichler-1989/literature_analysis/00_overview|0023_eichler-1989]]'
path: 03_stellar-nucleosynthesis/0020_nomoto-2013/literature_analysis/00_overview.md
---
> 本文件为文献精读档案的 00_overview.md（总览）
>
> 前序阅读:
> - `../0001_b2fh-1957/` (B²FH 1957 恒星核合成总纲 [FACT])
> - `../0014_cameron-1968/` (Cameron 1968 太阳丰度表 [FACT])
> - `../0003_fowler-1984/` (Fowler 1984 核合成综述 [FACT])
> - `../0004_wallerstein-1997/` (Wallerstein 1997 RMP 核合成综述 [FACT])
>
> 后续阅读:
> - `../0016_cowan-2021/` (Cowan 2021 RMP r/s 过程综述 [FACT])
> - `../0017_kaeppeler-2011/` (Käppeler 2011 s 过程综述 [FACT])
> - `../0023_eichler-1989/` (Eichler 1989 γ 射线暴 [FACT])

# 0. 文献基本信息

| 字段 | 内容 |
|---|---|
| **Title** | Nucleosynthesis in Stars and the Chemical Enrichment of Galaxies |
| **Authors** | K. Nomoto¹, C. Kobayashi², N. Tominaga³ |
| **Affiliations** | ¹Kavli IPMU/University of Tokyo; ²University of Hertfordshire; ³Konan University |
| **Journal** | Annu. Rev. Astron. Astrophys. **51**, 457–509 (2013) |
| **DOI** | 10.1146/annurev-astro-082812-140956 |
| **Year** | 2013 |
| **Pages** | 457–509 (共 53 页正文, PDF 54 页含目录页) |
| **Keywords** | first star, galactic archaeology, gamma-ray burst, hypernova, metal-poor star, supernova |
| **Category** | 恒星核合成 |

> **元数据核查**：任务上下文的 frontmatter (category=恒星核合成, year=2013, pages=457-509) 与 PDF 第 1 页一致（Nomoto, Kobayashi, Tominaga 2013, Annu. Rev. A&A 51:457）；目录名 `0020_nomoto-2013` 亦一致——**无勘误**。

## Frontmatter

- [FACT] **Review in Advance** 首版发布：2013 年 7 月 3 日
- [FACT] **Abstract 核心论题**：自 BBN 之后，重元素通过首批恒星 (Pop III) 的形成、演化与爆发而合成；首批超新星对后续恒星和星系的形成具有强动力、热、化学反馈。极贫金属 (EMP) 星的元素丰度模式携带了首批超新星产额的"指纹"，但其中相对太阳模式的偏离仍是未解之谜。
- [FACT] **6 个关键词**：first star / galactic archaeology / gamma-ray burst / hypernova / metal-poor star / supernova
- [FACT] **Yields Table 2013**：本文随文附带的产额表（Kobayashi et al. 2011），覆盖 Pop III SN / 大质量核心坍缩 SN / 中低质量 AGB / SN Ia，是全文的定量落点

## 章节结构（原文 §1–§10）

```
§1  Introduction
§2  Evolution and Nucleosynthesis of Asymptotic Giant Branch (AGB) Stars
    2.1  Yields Table
    2.2  C+O White Dwarfs versus Type 1.5 Supernovae
    2.3  Super AGB Stars, O+Ne+Mg White Dwarfs, and Electron-Capture Supernovae
        2.3.1  Formation of O+Ne+Mg white dwarfs
        2.3.2  Electron-capture supernovae
§3  Explosive Nucleosynthesis in Core-Collapse Supernovae
    3.1  Neutron-Proton Ratio Near the Mass Cut
    3.2  Mixing and Fallback Model
    3.3  Postshock Temperature and Explosive Nuclear Burning
        3.3.1  Complete Si burning
        3.3.2  Incomplete Si burning
        3.3.3  Fe-peak elements and neutron excess
        3.3.4  Explosive O-Ne-C burning
    3.4  Explosion Energy
§4  Massive Stars
    4.1  General Mass Dependence of Stellar Structure and Evolution
    4.2  Yields Tables for Core-Collapse Supernovae
    4.3  10–13 M☉ Stars and Faint Supernovae
    4.4  13–25 M☉ Stars and Normal Type II Supernovae
        4.4.1  SN 1987A
        4.4.2  SN 1993J and SN 1994I
        4.4.3  Yields
    4.5  Hypernovae, Faint Supernovae, and 25–140 M☉ Stars
        4.5.1  Hypernovae
        4.5.2  Faint supernovae
        4.5.3  Pulsational instability in precollapse stars
§5  Very Massive Stars
    5.1  Pair-Instability Supernovae of 140–300 M☉ Stars
    5.2  Stars with M ≥ 300 M☉ and Intermediate-Mass Black Holes
§6  Type Ia Supernovae: Progenitors and Nucleosynthesis
    6.1  Chandra versus Sub-Chandra Mass Models
    6.2  Single- versus Double-Degenerate Scenarios
§7  Chemical Evolution of Galaxies
    7.1  Solar Neighborhood
        7.1.1  α Elements
        7.1.2  Odd-Z elements
        7.1.3  Iron-peak elements
        7.1.4  Manganese
        7.1.5  Zinc
        7.1.6  Carbon
        7.1.7  Nitrogen
        7.1.8  Fluorine
    7.2  Galactic Bulge
    7.3  Galactic Thick Disk
    7.4  Galactic Halo
    7.5  Globular Clusters
    7.6  Dwarf Spheroidal Galaxies
    7.7  Damped Lyman-α Systems
    7.8  Elliptical Galaxies
§8  Extremely Metal-Poor Stars and Abundance Profiling
    8.1  Very Metal-Poor Stars
        8.1.1  Comparison with normal CC-SN yields
        8.1.2  Comparison with PISN yields
    8.2  Extremely Metal-Poor Stars
        8.2.1  Normal supernovae versus hypernovae
        8.2.2  Trends with metallicity
        8.2.3  Mixing and fallback
    8.3  Carbon-Enhanced Metal-Poor Stars
        8.3.1  Models for carbon enhancement
        8.3.2  Comparison of carbon-enhancement models
    8.4  Nitrogen-Enhanced Metal-Poor Stars
    8.5  Ultra Metal-Poor and Hyper Metal-Poor Stars
        8.5.1  Hyper metal-poor stars
        8.5.2  Ultra metal-poor stars
§9  Galactic Archaeology Surveys
§10 Future Outlook
```

## 图表索引

- **Figures**：Fig. 1 (stellar fate mass diagram), Fig. 2 (yield vs. mass), Fig. 3–4 (mixing/fallback), Fig. 5–6 (postshock temperature regimes), Fig. 7–8 (AGB yields), Fig. 9–11 (CC-SN yields vs. solar), Fig. 12 (VMP vs. CC-SN), Fig. 13 (EMP vs. hypernova), Fig. 14–15 (CEMP), Fig. 16–17 (GCE), Fig. 18–19 (DAMPs/ellipticals)
- **Tables**：Yields Table 2013（附录 A，Pop III + AGB + CC-SN + SN Ia），以及文中嵌入的产额对比表

## 精读重点（本任务）

- **核心坍缩超新星 (CC-SN) 产额表**（§3–§4）——mixing-fallback 参数化、Z=0 与 Z=0.004 产额
- **EMP 星约束**（§8）——Pop III 前身星质量/能量反推、CEMP-s/r 分类
- **化学演化模型 (GCE)**（§7）——太阳邻域、矮星系、椭圆星系对产额的自洽约束

## 相关论文

> 见本分类 `03_stellar-nucleosynthesis/` 内的其他精读：
> - B²FH 1957（§0001）——核合成总纲与产额表框架
> - Cameron 1968（§0014）——太阳丰度基线（[X/H]、[α/Fe] 的参考零点）
> - Fowler 1984（§0003）——核反应率综述
> - Wallerstein 1997 RMP（§0004）——上一代核合成综述
> - Käppeler 2011（§0017）——s 过程专论
> - Cowan 2021（§0016）——r/s 过程最新综述
> - Eichler 1989（§0023）——长暴 γ 射线与 Pop III 的联系
