---
title: Solar System Abundances and Condensation Temperatures of the Elements
authors: Katharina Lodders
year: '2003'
journal: The Astrophysical Journal (ApJ)
pages: '1220-1247'
doi: 未提供
arxiv: 未提供
keywords: Solar system abundances; condensation temperatures; photospheric abundances; meteoritic (CI chondrite) abundances;
  elemental abundances; solar composition; nebular condensation; mass fractions X, Y, Z; Z/X ratio; settling/diffusion
category: 恒星核合成
sections:
  - '§1 Introduction'
  - '§2 Abundance Selection'
  - '§3 Condensation Methods'
  - '§4 Major and Trace Condensation'
  - '§5 Ices and Volatiles'
  - '§6 Condensate Mass Fractions'
status: completed
read_date: '2026-08-12'
lastread: '2026-08-12'
tags:
- Solar system abundances
- condensation temperatures
- photospheric abundances
- meteoritic (CI chondrite) abundances
- elemental abundances
- solar composition
- nebular condensation
- mass fractions X
- Y
- Z
- Z/X ratio
- settling/diffusion
citations:
- '[[03_stellar-nucleosynthesis/0009_asplund-2009-solar-composition/literature_analysis/00_overview|0009_asplund-2009-solar-composition]]'
path: 03_stellar-nucleosynthesis/0008_lodders-2003/literature_analysis/00_overview.md
---
> 本章属于：Solar System Abundances and Condensation Temperatures of the Elements (Lodders 2003, ApJ 591:1220)
>
> 下一章：[[03_stellar-nucleosynthesis/0008_lodders-2003/literature_analysis/01_introduction.md|01_introduction.md]]

# 0. 文献基本信息

- **Title:** Solar System Abundances and Condensation Temperatures of the Elements
- **Authors:** Katharina Lodders
- **Collaboration:** 未提供（华盛顿大学行星化学实验室独立工作）
- **Journal / Conference:** The Astrophysical Journal (ApJ)
- **Publication Date:** 2003 July 10 (Received 2003 Jan 22; Accepted 2003 Mar 21)
- **Volume / Pages:** Vol. 591, pp. 1220–1247（共 28 页）
- **DOI:** 未提供
- **arXiv:** 未提供
- **Research Field:** 太阳物理学 · 天体化学 · 行星科学 · 陨石学（Astrochemistry / Meteors, Meteoroids / Solar System: Formation / Sun: Abundances / Sun: Photosphere）
- **Keywords:** Solar system abundances; condensation temperatures; photospheric abundances; meteoritic (CI chondrite) abundances; elemental abundances; solar composition; nebular condensation; mass fractions X, Y, Z; Z/X ratio; settling/diffusion
- **Funding:** NASA grant NAG5-10553
- **Cited Impact:** 本文是被引次数超过 6000 次的标志性综述/基准工作，定义了至今仍广泛使用的 Lodders (2003) 太阳成分基准；后续 ASH (2009)、Asplund et al. (2009, 2021) 与 Lodders & Fegley (2022) 都在其基础上工作。

---

# 0.1 全文结构树（Paper Structure）

- **Abstract**（摘要）
- **§1 Introduction**（引言，p. 1220–1221）
  - §1.1 冷凝温度在行星科学 / 天体化学 / 陨石学中的应用史
  - §1.2 先前冷凝温度表的问题（混合来源、混合"冷凝温度"与"50% 冷凝温度"）
  - §1.3 触发本文的直接原因：Allende Prieto et al. (2001, 2002) 大幅下调 C、O 丰度 → 需要自洽重算冷凝温度
  - §1.4 论文结构导览
- **§2 Elemental Abundances**（元素丰度，p. 1221–1235）
  - §2.1 Solar Photospheric Abundances
    - 表 1：Recommended Elemental Abundances in the Solar Photosphere
  - §2.2 CI Chondritic Abundances
    - 五块 CI 陨石样本：Alais / Ivuna / Orgueil / Revelstoke / Tonk
    - 表 3：Elemental Abundances in CI Chondrites
    - 天文对数标尺与宇宙化学标尺的耦合：A(El) = log[N(El)] + 1.540（式 2）
  - §2.3 Selection of Photospheric Abundances
    - §2.3.1 在太阳中相对于 CI 富集的：H、C、N、O、Noble Gases
      - §2.3.1.1 Helium Abundance（含表 4、表 5、式 3–5、Boothroyd & Sackmann 2003 拟合）
      - §2.3.1.2 Neon Abundance
      - §2.3.1.3 Argon Abundance
      - §2.3.1.4 Krypton and Xenon Abundances
      - §2.3.1.5 Noble Gases in CI Chondrites
    - §2.3.2 在太阳中相对于 CI 亏损的：Li、Be、B
      - §2.3.2.1 Beryllium Abundance
      - §2.3.2.2 Boron Abundance
    - §2.3.3 Phosphorus Abundance
    - §2.3.4 Sulfur Abundance
    - §2.3.5 Bromine and Iodine Abundances
    - §2.3.6 Mercury Abundance
  - §2.4 Solar System Abundances of the Elements（**表 2：Recommended Elemental Abundances of the Proto-Sun**）
    - 关键关系式（式 5）：A(El)$_{0}$ = A(El) + 0.074
  - §2.5 Abundances of the Isotopes（**表 6：Abundances of the Isotopes in the Solar System**，含 4.55 Gyr 前放射性/衰变产物丰度）
- **§3 Condensation Temperatures of the Elements**（p. 1236–1245）
  - §3.1 Computational Methods and Nomenclature（CONDOR 代码，式 7–15；p = $10^{-4}$ bar）
  - §3.2 Major Element Condensates（表 7）
  - §3.3 Minor and Trace Element Condensation（表 8、表 9）
    - §3.3.1 Ultrarefractory Trace Element Condensation：W, Re, Os, Ir, Mo, Pt, Rh, Ru, Zr, Hf（fremdlinge）
    - §3.3.2 Lithophile → Ca Titanates + Hibonite：Ba, Sr, Sc, Y, V, Nb, Ta, Th, U, REE
    - §3.3.3 Melilite：Be
    - §3.3.4 Olivine + Pyroxene：Cd, Li, Mn, Zn
    - §3.3.5 Feldspar：B, Cs, Ga, K, Na, Rb
    - §3.3.6 Fe Alloy：Ag, As, Au, Bi, Co, Cr, Cu, Ge, Ni, P, Pb, Pd, Sb, Sn, Te
    - §3.3.7 Troilite：Cd, Hg, In, Se, Tl
    - §3.3.8 Halogens：F, Cl, Br, I
  - §3.4 Highly Volatile Element Condensation：C, N, O, Noble Gases（表 10，含动力学效应）
    - §3.4.1 Oxygen Condensation
    - §3.4.2 Carbon Condensation（式 16–18）
    - §3.4.3 Nitrogen Condensation（式 19）
    - §3.3.4 Noble Gas Condensation（编号有误，应为 §3.4.4）
- **§4 Conclusions**（p. 1245–1247）（含 **表 11：Condensate Distribution**）
- **References**（约 95 条）
- **Acknowledgments**（感谢 Palme、Cameron、Gallino、Fegley、Schaefer）

---

# 0.2 全文核心结论（Overview）

[FACT] 本文给出两套自洽的太阳成分表：
- **太阳大气（photospheric）丰度**（表 1）：当前太阳大气，受 He 与重元素沉降（settling/diffusion）影响；
- **原始太阳 = 太阳系（protosolar = solar system）丰度**（表 2）：4.55 Gyr 前太阳形成时的成分。

[FACT] 通过 Boothroyd & Sackmann (2003) 标准太阳模型（SSM）的标度关系，从观测的大气 Z/X = 0.0177 推导：
- 当前质量分数：**X = 0.7491 ± 0.0030, Y = 0.2377 ± 0.0030, Z = 0.0133 ± 0.0014**，Z/X = 0.0177
- 原始（protosolar）质量分数：**$X_{0}$ = 0.7110 ± 0.0040, $Y_{0}$ = 0.2741 ± 0.0120, $Z_{0}$ = 0.0149 ± 0.0015**，$X_{0}$/$Z_{0}$ = 0.0210

[FACT] 原始与大气丰度的转换关系为 A(El)$_{0}$ = A(El) + 0.074 dex（除 H、He 外），即大气金属度约为原始太阳的 **84.29%**（约 16% 重元素沉降，约 18% He 沉降）。

[FACT] 使用自洽的原始太阳丰度与CONDOR代码，在总压 $10^{-4}$ bar 下重算全部 83 种天然元素的**初始冷凝温度**与**50% 冷凝温度**，按挥发性分为：
- 超难熔 >1650 K：Os, Re, W, Al, Hf, Sc, Th, Y, Zr, Gd, Tb, Dy, Ho, Er, Tm, Lu
- 高难熔 1650–1500 K：Ir, Mo, Ru, Ca, Nb, Ta, Ti, U, La, Pr, Nd, Sm
- 难熔 1500–1360 K：Pt, Rh, Ba, Be, Ce, Sr, V, Yb
- 普通（common）1360–1290 K：Mg, Si, Fe, Co, Cr, Ni, Pd, Eu
- 中等挥发 1290–704 K：Ag, As, Au, Bi, Cu, Ga, Ge, P, Pb, Sb, Te, Cs, B, K, Li, Na, Mn, Rb, Zn, Cl, F
- 挥发 704–371 K：Cd, In, S, Se, Tl, Sn, Br, I
- 高挥发 <371 K：C, N, O, Noble Gases, Hg

[FACT] 新大气 C、N、O 的向下修订（Allende Prieto et al. 2001, 2002）使总冷凝质量从 Anders & Grevesse (1989) 的 ~1.9% 降至 Lodders (2003) 太阳系的 **1.5%**、大气的 **1.3%**，冰/岩石比从 ~2 降到 ~1。

[FACT] 冰的冷凝温度受动力学效应强烈影响：$H_{2}$O 平衡冷凝 182 K（Model A/B）；$CH_{4}$·7$H_{2}$O 78 K → $CH_{4}$ 41 K（平衡）或石墨 626 K（动力学 Model A）；$NH_{3}$·$H_{2}$O 131 K 或 $N_{2}$·7$H_{2}$O 58 K。

---

# 0.3 与相关工作的关系

- **Anders & Grevesse (1989)**：被 Lodders (2003) 取代的旧基准；Z/X = 0.0267，C、N、O 显著偏高；冷凝序列定性相同但温度约低 10 K。
- **Grevesse & Sauval (1998, 2002)**：1998 版在 Lodders 之前，Z/X = 0.0245；2002 版（Z/X = 0.0208）在 Lodders 之后。Lodders 明确采用 Allende Prieto 新 C、O 值 → Z/X = 0.0177。
- **Lodders (2003) 之后**：
  - Lodders & Fegley (2005) 与 Lodders (2009a) 将本表拓展至恒星初始质量函数；
  - Asplund et al. (2009) 3D 非 LTE 大幅下调 O（8.69→8.66）引发新一轮争议；
  - Lodders (2010) 给出更新的太阳系统丰度；
  - Lodders & Fegley (2022) 《Astronomical Chemistry: A Solar System Primer》为本文的教科书化版本。

[INTERPRETATION] 本文在太阳系化学基准史上的地位是"承前启后"——它是把 Allende Prieto 的 C、O 向下修订正式引入行星科学冷凝计算的首次系统尝试，为后续外太阳系化学、巨行星与卫星形成、彗星化学奠定了新的成分基准。

---

## 篇间导航

### 前序阅读

- **Anders & Grevesse (1989)** — 14 年前版本（GCA 53, 197）
- **Grevesse & Sauval (1998)** — 5 年前版本（Space Sci. Rev. 85, 161）

### 关联论文

- [[03_stellar-nucleosynthesis/0009_asplund-2009-solar-composition/literature_analysis/00_overview|0009_asplund-2009-solar-composition]] — 6 年后 AGSS09 修订

### 后续阅读

- [[03_stellar-nucleosynthesis/0009_asplund-2009-solar-composition/literature_analysis/00_overview|0009_asplund-2009-solar-composition]] — 6 年后 AGSS09