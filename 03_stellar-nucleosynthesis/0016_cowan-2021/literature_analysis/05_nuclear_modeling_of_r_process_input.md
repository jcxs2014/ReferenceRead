...
> 下一章：[[03_stellar-nucleosynthesis/0016_cowan-2021/literature_analysis/06_astrophysical_sites_and_their_ejecta.md|06_astrophysical_sites_and_their_ejecta]]
...
- **§V.B $\beta$-decay 半衰期**——pn-QRPA、FRDM+QRPA
...
> **[FACT]** $\beta$-decay $T_{1/2}$ 预测不确定度：稳定线附近 ~10%；远离稳定线 ~30-50%。**P_n** 预测通常 ~50% 系统差。
...
> **[FACT]** r 过程中的裂变途径：
> 1. **$\beta$-delayed fission**：β-decay 后 Z 过高 → 自发裂变
...
| FRDM+QRPA | FRDM mass + QRPA | $\beta$-decay $T_{1/2}$ | ~10-50% |
| TALYS 1.9+ | 核质量 + 能级 | n-capture $\sigma$ | ~30% |
...
### Figure 9 — $\beta$-decay 半衰期模型预测 vs 实验

**1. 图的目的**：对比 Möller 2019 等模型的 $\beta$-decay $T_{1/2}$ 预测与实验值。
...
- $\beta$-decay $T_{1/2}$ 不确定度 50% → 最终丰度 0.3-0.5 dex
