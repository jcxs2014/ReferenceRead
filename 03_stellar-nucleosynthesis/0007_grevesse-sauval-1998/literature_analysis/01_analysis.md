...
**测量方法**：
- 大量原子/双原子分子指示线（CN、CH、CO、OH、C₂、N₂、NO…）
...
Ne、Ar 为**惰性气体**——不出现在光球光谱，也在陨石中流失 → 只能依赖：日冕光谱、太阳风、SEP、γ 射线光谱。
...
- **坐标轴**：
  ...
  - 误差棒 = 太阳丰度不确定度
- **关键观察**：[FACT]
  - 绝大多数点分布在 y = 0 附近（|Δ| < 0.2 dex）
  - 一个显著异常点在 Z ≈ 49（In），太阳 ≈ 1.66 vs 陨石 0.82 → Δ ≈ +0.84（被括号标注为低精度结果）
...
| 1956 | Suess & Urey | 关键数据，为 B²FH 1957 / Cameron 1957 奠定核合成理论基石 |
...
**重要背景**：
- Burbidge et al. 1957 (B²FH) — 核合成理论基石
...
为何需要 SSC：
  ...
  太阳丰度 = 太阳系其他天体丰度的校准靶
↓
丰度来源（光谱、粒子收集、γ、日震、月球）→ 光球为主（元素最全、混合最好、数据最久）
↓
关键进步：
