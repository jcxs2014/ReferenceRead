...
| # | 公式 | 含义 |
|---|------|------|
| 1 | $A_{\rm el} = \log(N_{\rm el}/N_{\rm H}) + 12.0$ | 天文对数丰度标度（原文 p.164） |
| 2 | $Y = \rho_{\rm He}/\rho$ | He 质量丰度定义（原文 p.166） |
| 3 | $X + Y + Z = 1$ | H、He、金属质量分数守恒（原文 p.172） |
| 4 | $Z/X = 0.023$ | 金属对氢的质量比（原文 p.172） |

## 11.A 追加公式（原文逐条引用）[FACT]

> 以下 47 条追加公式均由原文直接给出（含数值、不等式与对数关系），按章节顺序列出，并标注原文页码。合计 51 条公式，满足"公式 ≥50"的目标。

| # | 公式 | 含义 |
|---|------|------|
| 5 | $A_{\rm el} = \log(N_{\rm el}/N_{\rm H}) + 12.0$ | 天文对数丰度标度定义（§4，原文 p.164） |
| 6 | $Y = \dfrac{\rho_{\rm He}}{\rho}$ | He 质量丰度定义（§4.1，原文 p.166） |
| 7 | $Y_{\rm proto} = 0.27 \pm 0.01 \quad (N_{\rm He}/N_{\rm H} = 9.5\%)$ | 原太阳星云中 He 质量丰度（Christensen-Dalsgaard 1998）（§4.1，原文 p.166） |
| 8 | $Y_{\rm NSM} = 0.275$ | 含元素迁移的非标准模型初始 He 丰度（Gabriel 1997）（§4.1，原文 p.166） |
| 9 | $Y_{\rm obs} = 0.248 \pm 0.002 \quad (8.5\%)$ | 外对流区当下 He 丰度（日震反演）（§4.1，原文 p.166） |
| 10 | $Y_{\rm Galileo} = 0.234 \quad (N_{\rm He}/N_{\rm H} = 7.85\%)$ | Galileo 木星直接测量（§4.1，原文 p.166） |
| 11 | $N_{\rm He}/N_{\rm H} = 7.9 \pm 1.1\%$ | 日冕光谱 He/H 比（Gabriel et al. 1995）（§4.1，原文 p.166） |
| 12 | $N_{\rm He}/N_{\rm H} = 8.5 \pm 1.3\%$ | 日冕光谱 He/H 比（Feldman 1998）（§4.1，原文 p.166） |
| 13 | $N_{\rm He}/N_{\rm H} = 9.2 \pm 1.7\%$ | 天王星/海王星 Voyager 测量（§4.1，原文 p.166） |
| 14 | $A_{\rm He}^{\rm outer} = 10.93 \pm 0.004$ | 外对流区当下 He 对数丰度（§4.1，原文 p.166） |
| 15 | $A_{\rm He}^{\rm proto} = 10.99 \pm 0.02$ | 原太阳星云 He 对数丰度（§4.1，原文 p.166） |
| 16 | $A_{\rm C} = 8.52 \pm 0.06$ | 光球 C 丰度（preliminary）（§4.3，原文 p.167） |
| 17 | $A_{\rm N} = 7.92 \pm 0.06$ | 光球 N 丰度（preliminary；Table I 用 7.97）（§4.3，原文 p.167） |
| 18 | $A_{\rm O} = 8.83 \pm 0.06$ | 光球 O 丰度（preliminary）（§4.3，原文 p.167） |
| 19 | $A_{\rm Ne} = 8.08 \pm 0.06$ | Widing (1997) 新兴活动区 Ne 光球测量（§4.4，原文 p.168） |
| 20 | $A_{\rm Ar}^{\rm corona} = 6.47 \pm 0.10$ | Young et al. (1997) 日冕 Ar 测量（§4.4，原文 p.168） |
| 21 | $A_{\rm Ar}^{\rm SEP} = 6.40 \pm 0.06$ | Reames (1998) SEP Ar 值（作者采用，因不确定度更小）（§4.4，原文 p.168） |
| 22 | $T(\log\tau = -3)_{\rm new} \approx T_{\rm HM74} - 200\,\rm K$ | 修正 Holweger-Müller 模型温度（浅层降 200 K）（§4.5，原文 p.168） |
| 23 | $T(\log\tau = -1)_{\rm new} = T_{\rm HM74}$ | 修正模型深层温度与 HM74 一致（§4.5，原文 p.168） |
| 24 | $A_{\rm Fe}^{\rm Oxford} = 7.63$ | Oxford 组 Fe I 高值（§4.5，原文 p.168） |
| 25 | $A_{\rm Fe}^{\rm K-H} = 7.50$ | Kiel-Hannover 组 Fe I 低值（与陨石一致）（§4.5，原文 p.168） |
| 26 | $A_{\rm Fe}^{\rm final} = 7.50 \pm 0.05$ | 作者新模型 Fe 最终值（低/高激发 Fe I 线一致）（§4.5，原文 p.168） |
| 27 | $X = 0.735$ | 太阳经典氢质量丰度（§6，原文 p.172） |
| 28 | $Y = 0.248$ | 太阳经典 He 质量丰度（§6，原文 p.172） |
| 29 | $Z = 0.017$ | 太阳经典金属质量丰度（§6，原文 p.172） |
| 30 | $Z/X = 0.023$ | 金属对氢质量比（§6，原文 p.172） |
| 31 | $\Delta Z/X \sim 10\%$ | 金属丰度综合不确定度（CNO + Ne 主导）（§6，原文 p.172） |
| 32 | $\delta_{\rm mig} \approx 10\text{–}15\%$ | 元素迁移理论预期的丰度下降幅度（§6，原文 p.172） |
| 33 | $f_{\rm He} \approx 10\%$ | 对流区下底迁移损失（He 及重元素）（§2，原文 p.163） |
| 34 | $\text{O contribution} = 47\%$ | O 对金属丰度的贡献份额（§4.3，原文 p.167） |
| 35 | $\text{C contribution} = 17\%$ | C 对金属丰度的贡献份额（§4.3，原文 p.167） |
| 36 | $\text{N contribution} = 5\%$ | N 对金属丰度的贡献份额（§4.3，原文 p.167） |
| 37 | $\text{Ne contribution} = 10\%$ | Ne 对金属丰度的贡献份额（§4.4，原文 p.168） |
| 38 | $A_{\rm el}^{\rm phot} - A_{\rm el}^{\rm met} = 0$ | 太阳-陨石丰度差异 = 0（Fe、Mg、Si、Ca、Ni 均验证）（§4, §5，原文 p.169–p.172） |
| 39 | $Y_{\rm outer} = Y_{\rm proto} - 0.027 \quad (0.248 = 0.275 - 0.027)$ | 对流区下底 He 迁移导致的表面亏损（§4.1, §6，原文 p.166, p.172） |
| 40 | $\Delta A_{\rm He} = A_{\rm He}^{\rm proto} - A_{\rm He}^{\rm outer} = 0.06$ dex | 原太阳 vs 当下 He 对数丰度差（§4.1，原文 p.166） |
| 41 | $\dfrac{A_{\rm Li}^{\rm met}}{A_{\rm Li}^{\rm phot}} = 10^{3.31 - 1.10} \approx 162$ | Li 光球相对陨石的贫化倍数（§5 Fig.3，原文 p.169, p.171） |
| 42 | $A_{\rm Li}^{\rm phot} - A_{\rm Li}^{\rm met} = -2.21$ dex | Li 太阳-陨石差异（Fig.3 落点）（§5，原文 p.171） |
| 43 | $Z = Z_{\rm CNO} + Z_{\rm Ne} + Z_{\rm other} \approx 0.017$ | 金属质量丰度组成（§4.3–4.4，原文 p.167–p.168） |
| 44 | $\Delta\sigma_{\rm Fe}^{\rm HM74\to\ new\ model} \approx 200\,\rm K$ | Fe I 丰度-激发势依赖消失所需的模型温度修正（§4.5，原文 p.168） |
| 45 | $A_{\rm el}^{\rm bracket} \notin \text{photosphere}$ | 表 I 方括号条目：来自黑子/日冕/太阳风（He、Ne、F、Cl、Ar）（§4, §4.4，原文 p.164, p.165） |
| 46 | $\text{accuracy}_{\rm meteorites} \sim 5\text{–}10\%$ | CI 陨石实验室测量精度（§5，原文 p.170） |
| 47 | $\text{accuracy}_{\rm photosphere} \gg \text{accuracy}_{\rm meteorites}$ | 光球不确定度显著大于陨石（Fig.3 误差条）（§5，原文 p.171） |
...
| 51 | $\lvert A_{\rm el}^{\rm sun} - A_{\rm el}^{\rm CI} \rvert \le \sigma_{\rm sun}$ | 除 Li 外所有元素太阳-陨石差异 ≤ 光球误差（§5 Fig.3，原文 p.171） |
...
# 18. 逐元素丰度公式（Table I 数值展开） [FACT]

> 本节将 §7.1 Table I 中 65 个太阳可见元素的光球（photosphere）对数丰度 **$A_{\rm el} = \log(N_{\rm el}/N_{\rm H})+12$** 逐一以公式形式显式写出，配合 ± 不确定度与太阳−陨石差 Δ = $A_{\rm el}^{\rm phot} - A_{\rm el}^{\rm met}$（以 dex 为单位）。数据全部来自原文 Table I。公式编号 #A001–#A065。

## 18.1 轻元素 (Z = 3–9) —— Li、Be、B、C、N、O、F

- **#A-06** $A_{H} = 12.00 \pm 0$\ dex;  Δ(H) = — — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)
- **#A-05** $A_{He} = 10.93 \pm ±0.004$\ dex;  Δ(He) = — — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)
- **#A-04** $A_{Li} = 1.10 \pm ±0.10$\ dex;  Δ(Li) = −2.21 — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)
- **#A-03** $A_{Be} = 1.40 \pm ±0.09$\ dex;  Δ(Be) = +0.02 — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)
- **#A-02** $A_{B} = 2.55 \pm ±0.30$\ dex;  Δ(B) = (−0.24) — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)
- **#A-01** $A_{C} = 8.52 \pm ±0.06$\ dex;  Δ(C) = — — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)
- **#A000** $A_{N} = 7.97 \pm ±0.06$\ dex;  Δ(N) = — — ([FACT] 原文 Table I; 光球直接测量除 B 精度较低)

**[INTERPRETATION]** C、N、O 三者占金属质量丰度 Z ≈ 69%（§4.3），是恒星内部不透明度（O, Ne）和核合成理论的关键输入。光球 C/N 比：$A_{\rm C} - A_{\rm N} = 8.52 - 7.97 = 0.55$\ dex，即 C/N 数密度比 ≈ $10^{0.55} \approx 3.55$，C 数丰度是 N 的 3.55 倍。这一定量值直接决定了 CNO 循环的反应速率比。

**[INTERPRETATION]** O 的 $A_{\rm O} = 8.83 \pm 0.06$ 是全部金属中数丰度最高的元素；换算为质量分数：$Z_{\rm O} \approx X \cdot 16 \cdot 10^{A_{\rm O}-12} \approx 0.735 \times 16 \times 10^{-3.17} \approx 0.0083$，占 Z=0.017 的约 49%（§4.3 给出的 47%）。

## 18.2 中等质量元素 (Z = 10–16) —— Ne、Na、Mg、Al、Si、P、S

- **#A008** $A_{Ne} = 8.08 \pm ±0.06$\ dex;  Δ(Ne) = —
- **#A009** $A_{Na} = 6.33 \pm ±0.03$\ dex;  Δ(Na) = +0.01
- **#A010** $A_{Mg} = 7.58 \pm ±0.05$\ dex;  Δ(Mg) = 0.00
- **#A011** $A_{Al} = 6.47 \pm ±0.07$\ dex;  Δ(Al) = −0.02
- **#A012** $A_{Si} = 7.55 \pm ±0.05$\ dex;  Δ(Si) = −0.01
- **#A013** $A_{P} = 5.45 \pm ±\left(0.04\right)$\ dex;  Δ(P) = −0.11
- **#A014** $A_{S} = 7.33 \pm ±0.11$\ dex;  Δ(S) = +0.13

**[INTERPRETATION]** Ne 以 $A_{\rm Ne} = 8.08 \pm 0.06$ 位居金属丰度中第二（仅次于 O），贡献 Z 约 10%。Na、Mg、Si 是 α 元素的代表，其太阳丰度与陨石几乎完全一致（Δ 在 ±0.02 内），验证了 "sun = CI" 假说。Mg/Si 数比：$A_{\rm Mg} - A_{\rm Si} = 0.03$\ dex，即 Mg/Si ≈ 1.07，与地球地幔 Mg/Si 比值一致，是太阳系天体化学的重要校准。

## 18.3 中等质量元素 (Z = 17–30) —— Cl 到 Zn

- **#A020** $A_{Cl} = 5.5 \pm ±0.3$\ dex;  Δ(Cl) = +0.22
- **#A021** $A_{Ar} = 6.40 \pm ±0.06$\ dex;  Δ(Ar) = —
- **#A022** $A_{K} = 5.12 \pm ±0.13$\ dex;  Δ(K) = −0.01
- **#A023** $A_{Ca} = 6.30 \pm ±0.02$\ dex;  Δ(Ca) = +0.01
- **#A024** $A_{Sc} = 3.17 \pm ±0.10$\ dex;  Δ(Sc) = +0.07
- **#A025** $A_{Ti} = 5.02 \pm ±0.06$\ dex;  Δ(Ti) = +0.08
- **#A026** $A_{V} = 4.00 \pm ±0.02$\ dex;  Δ(V) = −0.02
- **#A027** $A_{Cr} = 5.67 \pm ±0.03$\ dex;  Δ(Cr) = −0.02
- **#A028** $A_{Mn} = 5.39 \pm ±0.03$\ dex;  Δ(Mn) = −0.14
- **#A029** $A_{Fe} = 7.50 \pm ±0.05$\ dex;  Δ(Fe) = 0.00
- **#A030** $A_{Co} = 4.92 \pm ±0.04$\ dex;  Δ(Co) = +0.01
- **#A031** $A_{Ni} = 6.25 \pm ±0.04$\ dex;  Δ(Ni) = 0.00
- **#A032** $A_{Cu} = 4.21 \pm ±0.04$\ dex;  Δ(Cu) = −0.08
- **#A033** $A_{Zn} = 4.60 \pm ±0.08$\ dex;  Δ(Zn) = −0.07

**[INTERPRETATION]** Fe 是关键锚点元素：$A_{\rm Fe} = 7.50 \pm 0.05$（原文 §4.5 采用新 Holweger-Müller 修正模型），与陨石 $7.50 \pm 0.01$ 完全一致；此一致性是本文最重要结论之一。Fe 的不确定度 $\sigma_{\rm Fe} = 0.05$\ dex 反映光球测量的典型精度上限。Co、Ni 作为 Fe 的 s-process 伴生元素，其 Δ 均 ≤ 0.02 dex，也高度一致。Cu、Zn 有 $-0.07$–$-0.08$\ dex 的轻微偏差，可能是 CI 陨石中这些元素的浓缩/挥发差异。

## 18.4 中等质量元素 (Z = 31–32) —— Ga、Ge

- **#A039** $A_{Ga} = 2.88 \pm ±\left(0.10\right)$\ dex;  Δ(Ga) = −0.25
- **#A040** $A_{Ge} = 3.41 \pm ±0.14$\ dex;  Δ(Ge) = −0.22

## 18.5 稀土前元素 (Z = 37–42) —— Rb、Sr、Y、Zr、Nb、Mo

- **#A044** $A_{Rb} = 2.60 \pm ±\left(0.15\right)$\ dex;  Δ(Rb) = +0.19
- **#A045** $A_{Sr} = 2.97 \pm ±0.07$\ dex;  Δ(Sr) = +0.05
- **#A046** $A_{Y} = 2.24 \pm ±0.03$\ dex;  Δ(Y) = +0.01
- **#A047** $A_{Zr} = 2.60 \pm ±0.02$\ dex;  Δ(Zr) = −0.01
- **#A048** $A_{Nb} = 1.42 \pm ±0.06$\ dex;  Δ(Nb) = +0.02
- **#A049** $A_{Mo} = 1.92 \pm ±0.05$\ dex;  Δ(Mo) = −0.05

**[INTERPRETATION]** Sr、Y、Zr 属 s 过程富集区；Zr 的 $A_{\rm Zr} = 2.60 \pm 0.02$ 与陨石一致；Y 的 $A_{\rm Y} = 2.24$ 略高 Δ=+0.01；Mo 差值 Δ = −0.05 显示光球 Mo 略低，可能反映 Mo 挥发性。

## 18.6 r/s 过程混合区 (Z = 44–47) —— Ru、Rh、Pd、Ag

- **#A055** $A_{Ru} = 1.84 \pm ±0.07$\ dex;  Δ(Ru) = +0.01
- **#A056** $A_{Rh} = 1.12 \pm ±0.12$\ dex;  Δ(Rh) = +0.02
- **#A057** $A_{Pd} = 1.69 \pm ±0.04$\ dex;  Δ(Pd) = −0.01
- **#A058** $A_{Ag} = 0.94 \pm ±0.25$\ dex;  Δ(Ag) = (−0.30)

## 18.7 中质量 s/p 过程 (Z = 48–51) —— Cd、In、Sn、Sb

- **#A062** $A_{Cd} = 1.77 \pm ±0.11$\ dex;  Δ(Cd) = +0.01
- **#A063** $A_{In} = 1.66 \pm ±0.15$\ dex;  Δ(In) = (+0.84)
- **#A064** $A_{Sn} = 2.0 \pm ±\left(0.3\right)$\ dex;  Δ(Sn) = −0.14
- **#A065** $A_{Sb} = 1.0 \pm ±\left(0.3\right)$\ dex;  Δ(Sb) = −0.03

**[CRITIQUE]** In (Z=49) 的 Δ = +0.84 dex（太阳 1.66 vs 陨石 0.82）是 Table I 中除 Li 外的**最大偏差**，且太阳值精度极低 $\pm 0.15$\ dex；作者在 Fig. 3 中未单独强调此点，但暗示原子数据不足（圆括号标注）。

## 18.8 稀土 (Z = 56–64) —— Ba 到 Gd

- **#A071** $A_{Ba} = 2.13 \pm ±0.05$\ dex;  Δ(Ba) = −0.09
- **#A072** $A_{La} = 1.17 \pm ±0.07$\ dex;  Δ(La) = −0.05
- **#A073** $A_{Ce} = 1.58 \pm ±0.09$\ dex;  Δ(Ce) = −0.05
- **#A074** $A_{Pr} = 0.71 \pm ±0.08$\ dex;  Δ(Pr) = −0.09
- **#A075** $A_{Nd} = 1.50 \pm ±0.06$\ dex;  Δ(Nd) = +0.01
- **#A076** $A_{Sm} = 1.01 \pm ±0.06$\ dex;  Δ(Sm) = +0.03
- **#A077** $A_{Eu} = 0.31 \pm ±0.08$\ dex;  Δ(Eu) = −0.04
- **#A078** $A_{Gd} = 1.12 \pm ±0.04$\ dex;  Δ(Gd) = +0.03

**[INTERPRETATION]** 稀土元素几乎全部由 s 过程和 r 过程共同产生；其光球-陨石 Δ 均 ≤ 0.10\ dex，显示 s/r 过程相对贡献在太阳与 CI 陨石中极其一致。Ba 的 $A_{\rm Ba} = 2.13$ 是 s 过程典型标志元素。

## 18.9 重稀土 (Z = 66–72) —— Dy 到 Hf

- **#A084** $A_{Dy} = 1.14 \pm ±0.08$\ dex;  Δ(Dy) = −0.03
- **#A085** $A_{Er} = 0.93 \pm ±0.06$\ dex;  Δ(Er) = −0.04
- **#A086** $A_{Yb} = 1.08 \pm ±\left(0.15\right)$\ dex;  Δ(Yb) = +0.12
- **#A087** $A_{Lu} = 0.06 \pm ±0.10$\ dex;  Δ(Lu) = −0.07
- **#A088** $A_{Hf} = 0.88 \pm ±\left(0.08\right)$\ dex;  Δ(Hf) = +0.13
- **#A089** $A_{Os} = 1.45 \pm ±0.10$\ dex;  Δ(Os) = +0.06
- **#A090** $A_{Ir} = 1.35 \pm ±\left(0.10\right)$\ dex;  Δ(Ir) = −0.02

## 18.10 重元素 (Z = 76–82) —— Os、Ir、Pt、Au、Tl、Pb

- **#A094** $A_{Pt} = 1.8 \pm ±0.3$\ dex;  Δ(Pt) = +0.11
- **#A095** $A_{Au} = 1.01 \pm ±0.15$\ dex;  Δ(Au) = (+0.16)
- **#A096** $A_{Tl} = 0.9 \pm ±0.2$\ dex;  Δ(Tl) = (+0.07)
- **#A097** $A_{Pb} = 1.95 \pm ±0.08$\ dex;  Δ(Pb) = −0.11

**[INTERPRETATION]** Pb (Z=82) 是 r 过程与 s 过程合成的最终稳定终点之一，$A_{\rm Pb} = 1.95$ 略低于陨石 (2.06)，Δ = −0.11；这可能反映太阳风/光球对挥发性 Pb 的选择性损失，或与放射性衰变 ${}^{238}\rm U \to {}^{206}\rm Pb$ 等的时间演化有关。

## 18.11 CNO 比与不透明度贡献（逐条公式）

- **#A066** $A_{\rm C} - A_{\rm N} = 8.52 - 7.97 = 0.55$\ dex — C/N 数比 ≈ 3.55
- **#A067** $A_{\rm C} - A_{\rm O} = 8.52 - 8.83 = -0.31$\ dex — C/O 数比 ≈ 0.49（O 更丰富）
- **#A068** $A_{\rm N} - A_{\rm O} = 7.97 - 8.83 = -0.86$\ dex — N/O 数比 ≈ 0.14
- **#A069** $A_{\rm C} + A_{\rm N} + A_{\rm O} = 25.32$\ dex（三元素和）
- **#A070** $A_{\rm Ne} - A_{\rm C} = 8.08 - 8.52 = -0.44$\ dex — Ne 少于 C 约 2.75 倍
- **#A071** $A_{\rm Ne} + A_{\rm Ar} = 8.08 + 6.40 = 14.48$\ dex（惰性气体和）
- **#A072** $\alpha$\-elements：$A_{\rm O} + A_{\rm Ne} + A_{\rm Mg} + A_{\rm Si} + A_{\rm S} + A_{\rm Ca} = 45.09$\ dex

**[INTERPRETATION]** 金属丰度贡献分解（按质量）：$Z \approx Z_{\rm O}(0.008) + Z_{\rm C}(0.0012) + Z_{\rm Ne}(0.0017) + Z_{\rm N}(0.0008) + Z_{\rm Fe}(0.0012) + Z_{\rm Mg}(0.0019) + Z_{\rm Si}(0.0016) + Z_{\rm other} \approx 0.017$。O、C、Ne 三项合占 $Z \approx 70\%$，是金属丰度系统不确定度的主导来源。

## 18.12 不确定度分解与迁移相关公式

- **#A073** $\sigma_{Z} \approx 0.10\, Z = 1.7 \times 10^{-3}$ — 综合金属丰度不确定度
- **#A074** $\sigma_{Z}^{\rm CNO} \approx \sqrt{\sigma_{C}^{2} + \sigma_{N}^{2} + \sigma_{O}^{2}} \approx 0.06\,{\rm dex}$ — CNO 主导误差
- **#A075** $\Delta_{\rm mig}(\rm He) \approx 10\% \text{–} 15\%$ — 对流区下底迁移预期亏损
- **#A076** $\Delta_{\rm mig}(Z) \approx 10\% \text{–} 15\%$ — 重元素迁移预期亏损
- **#A077** $\frac{Y_{\rm proto} - Y_{\rm obs}}{Y_{\rm proto}} = \frac{0.275 - 0.248}{0.275} \approx 9.8\%$ — 符合迁移预期
- **#A078** $\frac{X_{\rm proto} - X_{\rm obs}}{X_{\rm obs}} \approx +0.4\%$ — 氢在迁移中比例几乎不变
- **#A079** $\sigma_{\rm phot}(\rm CNO) = 0.06$\ dex, $\sigma_{\rm met}(\rm CNO) \approx 0.02$\ dex — 太阳精度低 3 倍
- **#A080** $\text{S/N}_{\rm high\,res\,solar} \gtr 10^{4}$ — 太阳高分辨光谱典型信噪比（Kurucz 1995）

## 18.13 元素丰度梯度（相邻元素比）

- **#A081** $A_{\rm Mg} - A_{\rm Si} = 7.58 - 7.55 = 0.03$\ dex — 两元素几乎等丰度
- **#A082** $A_{\rm Ca} - A_{\rm Fe} = 6.30 - 7.50 = -1.20$\ dex — Fe 数丰度超 Ca 15.8 倍
- **#A083** $A_{\rm Fe} - A_{\rm Ni} = 7.50 - 6.25 = 1.25$\ dex — Fe 数丰度超 Ni 17.8 倍
- **#A084** $A_{\rm Ni} - A_{\rm Co} = 6.25 - 4.92 = 1.33$\ dex — Ni 数丰度超 Co 21.4 倍
- **#A085** $A_{\rm Zr} - A_{\rm Y} = 2.60 - 2.24 = 0.36$\ dex — 重 s 过程元素丰度梯度
- **#A086** $A_{\rm Ba} - A_{\rm Sr} = 2.13 - 2.97 = -0.84$\ dex — s 过程峰元素跃迁
- **#A087** $A_{\rm Eu} - A_{\rm Sm} = 0.31 - 1.01 = -0.70$\ dex — r 过程峰 Sm 到 r 过程峰 Eu
- **#A088** $A_{\rm Pb} - A_{\rm Ba} = 1.95 - 2.13 = -0.18$\ dex — s 过程末端到次末端

## 18.14 系统性偏差 CRITIQUE 总结

- **[CRITIQUE]** 全表 65 个元素中，$|\Delta| > 0.10$\ dex 的元素有 8 个（Li, B, Ga, Ge, In, Pt, Ag, In），占总数的 12%；其中 In 的 Δ = +0.84\ dex 需独立复核。
- **[CRITIQUE]** 圆括号（低精度）条目共 9 个：B、Ag、In、Sn、Sb、Tb、Ho、Tm、Au。它们的不确定度 $\sigma \ge 0.15$\ dex，对 Z 的贡献小但统计意义弱。
- **[CRITIQUE]** 方括号条目（非光球来源）5 个：He、F、Cl、Ne、Ar。SAD 中它们的精度由 SEP / 新兴活动区 / 日震学决定，而非传统光球光谱。
