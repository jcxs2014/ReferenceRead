...
用于模拟 AGB 星中 $^{22}{\rm Ne}$($\alpha$,n)$^{25}{\rm Mg}$ 中子源的温度（23 keV 有效温度）；
...
1. **准恒星中子谱模拟**：实验室可用 $^{7}{\rm Li}$(p,n)$^{7}{\rm Be}$、$^{3}{\rm H}$(p,n)$^{3}{\rm He}$、$^{18}{\rm O}$(p,n)$^{18}{\rm F}$ 等反应模拟恒星中子能谱，MACS 可**直接在实验室测量**
...
|---|---|---|---|
| $^{7}{\rm Li}$(p,n)$^{7}{\rm Be}$ | 25 keV | $^{22}{\rm Ne}$($\alpha$,n)，AGB 23 keV | $10^9$ s$^{-1}$ |
| $^{3}{\rm H}$(p,n)$^{3}{\rm He}$ | 52 keV | 大质量星 C 壳层 90 keV（需外推） | $10^8$ s$^{-1}$ |
| $^{18}{\rm O}$(p,n)$^{18}{\rm F}$ | 5 keV | $^{12}{\rm C}$($\alpha$,n)，AGB 8 keV | $10^5$ s$^{-1}$ |

[FACT] **激活法 vs TOF 的互补性（Table II）**：

- Fe 组元素（共振主导的小截面）：激活法更准确
- 重核（平滑截面）：TOF 通常更准确
- 无普适规则——8$^{8}$Sr、$^{197}{\rm Au}$ 等反例
...
|---|---|---|
| $^{79}{\rm Se}$ | $3.2\times10^5$ yr（地球）/ 恒星条件剧减 | $^{80}$,$^{82}{\rm Kr}$ |
| $^{85}{\rm Kr}$ | 10.7 yr | $^{86}$,$^{87}{\rm Sr}$ |
| $^{123}{\rm Te}$ | $1.5\times10^5$ yr | $^{122}$,$^{124}{\rm Te}$ |
| $^{148}{\rm Pm}$ | 5.37 d | $^{148}$,$^{150}{\rm Sm}$ |
| $^{151}{\rm Sm}$ | 90 yr | $^{151}{\rm Eu}$ / $^{152}{\rm Sm}$ |
| $^{176}{\rm Lu}$ | 37.5 Gyr（基态） | $^{176}{\rm Hf}$ |
| $^{185}{\rm W}$ | 75.1 万 yr | $^{186}{\rm W}$ / $^{186}{\rm Re}$ |
...
|---|---|---|---|
| $^{80}$,$^{82}{\rm Kr}$ | 267 ± 14 / 90 ± 6 | 2.28 : 11.58 | 0.61 ± 0.05 |
| $^{122}$,$^{124}{\rm Te}$ | 295 ± 3 / 155 ± 2 | 2.55 : 4.74 | 1.06 ± 0.02 |
| $^{128}$,$^{130}{\rm Xe}$ | 262.5 ± 3.7 / 132.0 ± 2.1 | 1.92 : 4.08 | 0.96 ± 0.02 |
| $^{134}$,$^{136}{\rm Ba}$ | 176.0 ± 5.6 / 61.2 ± 2.0 | 2.417 : 7.854 | 0.94 ± 0.04 |
| $^{148}$,$^{150}{\rm Sm}$ | 241 ± 2 / 422 ± 4 | 11.24 : 7.38 | 0.88 ± 0.01 |
...
[FACT] **$^{187}{\rm Os}$ 关键案例**：$kT=30$ keV 时只有 30% 布居基态，70% 在激发态（其中 47% 在第一激发态 9.75 keV）；SEF 修正对 Re/Os 核宇宙年代学至关重要。
...
| 参数 | 数值 | 备注 |
|---|---|---|
| 主 s 过程有效温度 | $kT = 8–25$ keV | AGB 星 $^{12}{\rm C}$($\alpha$,n)$^{16}{\rm O}$ 源 |
...
| PHWT 系统误差 | < 2% | 最新水平 |
| $^{187}{\rm Os}$ 第一激发态 | 9.75 keV | 47% 布居 |
| $^{60}{\rm Fe}$ 截面 | 5.7 mb | 样品 1.4 μg |
| $^{147}{\rm Pm}$ 样品 | 28 ng | 半衰期 2.6 yr |
...
**1. 目的**：展示 $^{209}{\rm Bi}$ 第二共振的 R-矩阵拟合，揭示先前数据中的中子灵敏度假性贡献。
...
**5. 物理意义**：$^{209}{\rm Bi}$ 是 s 过程的最后一个 s-only 核素，其截面精度直接影响 r-residual 方法得到的 r 过程分布。
...
**3. 关键观察**：
- **多数核素 < 1**：新截面比旧截面小
- **$^{20}{\rm Ne}$ → $^{90}{\rm Zr}$ → $^{98}{\rm Mo}$ 区间**：差异最大
...
[CRITIQUE] **SEF 的精度风险**：原文说 SEF 在主 s 过程 <10%，但 $^{187}{\rm Os}$ 案例说明**个别核素的 SEF 可以远超平均值**——$^{187}{\rm Os}$ 在 $kT=30$ keV 时只有 30% 布居基态。如果统计模型对低能激发态的布居预测不准，SEF 修正可能引入 >10% 的系统误差。

[CRITIQUE] **C 壳层燃烧 $kT=90$ keV 时的外推问题**：实验室最接近的模拟谱是 $^{3}{\rm H}$(p,n)$^{3}{\rm He}$ 的 52 keV；从 52 keV 到 90 keV 的外推完全依赖统计模型计算，这是 **weak s 过程丰度预测的最大不确定性来源**。
...
[CRITIQUE] Ba$F_{2}$ 晶体的中子散射本底问题在 $^{209}{\rm Bi}$、$^{208}{\rm Pb}$、$^{90}{\rm Zr}$、$^{139}{\rm La}$ 等**中子魔法核**上表现最严重——它们的俘获-散射比极低。这意味着所有涉及中子魔法核的早期 TOF 数据都可能被高估，需要系统性地重新评估。

### 8.2 $^{13}{\rm C}$($\alpha$,n) 和 $^{22}{\rm Ne}$($\alpha$,n) 中子源反应
