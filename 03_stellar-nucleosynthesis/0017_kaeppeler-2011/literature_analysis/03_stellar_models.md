...
[FACT] §III 的核心论点是：**s 过程不是一个单一的过程，而是依赖于初始质量、金属丰度、¹³C 口袋强度、TDU 效率、质量损失率的复杂多参数过程。**
...
[FACT] 拟合得到的两个全局参数：
- $f \approx 0.04\%$：被辐照的 ⁵⁶Fe 分数
- $\rho_0 \approx 15$ n/种子：平均中子暴露

[FACT] **经典模型的巨大成功**：在分支点之外的 s-only 核素，均方偏差仅 3%（Käppeler, Gallino et al., 1990）。

[FACT] **经典模型的致命失败**——¹⁴²Nd（Arlandini et al., 1999）：
- ¹⁴²Nd 是 $N=82$ magic neutron number 附近的 s-only 核素
...
其中 ¹⁴N 来自前 H 燃烧阶段的 CNO 循环产物。

[FACT] **这一来源链是 weak s 过程"次级"性质的根本原因**：²²Ne 依赖 CNO 循环产物，因此 weak s 过程的产额随金属丰度降低而减少。
...
[FACT] **¹³C 口袋（13C pocket）——主 s 过程的关键**：

[FACT] ¹³C 口袋是 s 过程中**最核心的微物理结构**，其形成机制为：
...
[FACT] 早期模型中 ¹³C 口袋是**人为参数**（Straniero et al., 2003, 2006；Gallino et al., 1998；Arlandini et al., 1999 的"标准口袋"ST）

[FACT] 现代模型通过以下物理过程自然产生 ¹³C 口袋：
...
- **¹³C 口袋参数化**：从"标准口袋"（ST）出发，乘以不同系数（ST×2, ST, ST/2, ST/3, ..., ST/150）

[FACT] 图 10 展示 ¹³C 口袋形成过程（Straniero et al., 2009）：
- (a) TDU 刚发生，对流包络回落
- (b) ¹³C 在较热区域开始形成
- (c) ¹⁴N 也开始产生
- (d) ¹³C 和 ¹⁴N 口袋完全形成
...
- 低金属丰度（$\rm[Fe/H] < -2.5$）：¹³C 口袋效率变化可解释 CEMP-s 星的丰度散布
...
- 银河系化学演化中的 s 过程产额对**中子源反应率、¹³C 口袋、初始质量函数（IMF）**高度敏感
...
其中 ¹³C_ST 是 Gallino et al. (1998) 和 Arlandini et al. (1999) 的标准口袋。
...
| Core He 燃烧中子密度 | $n_n \sim 10^{10}$ cm$^{-3}$ | ²²Ne 源 |
...
**1. 目的**：分四步展示 ¹³C 口袋从 TDU 后到完全形成的演化过程。
...
**4. 物理意义**：¹³C 口袋是主 s 过程中子源的核心微物理结构，其形成机制直接决定 s 过程效率。
...
**4. 物理意义**：¹³C 口袋效率的变化（对应氢混合进 ¹³C 口袋的程度）是观测散布的主要解释。
...
|---|---|---|
| 初始质量 | 1.3 M☉ | |
| ¹³C 口袋 | ST×2 到 ST/150 | 覆盖观测散布 |
| 初始 r 增强 | [r/Fe]ᵢₙᵢ = 2 | 解释 CEMP-s=r 星 |
...
s 过程核合成发生在 He 燃烧层
  → 两类恒星：大质量星（weak s）和 AGB 星（main s）
    → 经典方法曾成功但存在不自洽
      → 新一代恒星模型解决经典方法的不自洽
        → AGB 星：¹³C 口袋 → TDU → 表面 s 过程增强
          → 参数化：质量、金属丰度、¹³C 口袋、TDU、质量损失率
            → 与观测对比（Ba 星、CEMP-s）
              → 银河系化学演化中的 s 过程贡献
```

[FACT] 作者在 §III 中反复强调的核心逻辑：**AGB 星是 s 过程的主工厂**——主 s 成分的丰度由 ¹³C 口袋决定，而 ¹³C 口袋的效率又由恒星质量和金属丰度决定。
...
[CRITIQUE] **¹³C 口袋仍然部分参数化**：尽管现代模型通过对流超射、旋转混合等物理过程产生了 ¹³C 口袋，但口袋的**精确深度和效率**仍不完全由 ab initio 物理决定。这是当前 AGB 模型最大的系统不确定性。
...
### 8.1 ¹³C 口袋的 ab initio 边界

[CRITIQUE] 虽然对流超射、重力波混合等物理过程使 ¹³C 口袋不再纯靠参数化，但口袋的深度仍与恒星演化代码中的**对流超射参数**有关。这意味着：
- 不同代码给出不同 ¹³C 口袋
- 与观测的\"吻合\"可能只是调参的结果
...
[CRITIQUE] 图 13 显示，1.3 M☉ AGB 模型 + [r/Fe]ᵢₙᵢ = 2 可解释 CEMP-s=r 星。但：
