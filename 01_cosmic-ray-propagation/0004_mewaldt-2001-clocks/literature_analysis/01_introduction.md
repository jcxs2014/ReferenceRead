...
- **[FACT]** 1970 年代 $^{10}$Be 的首次可靠测量发现大部分 $^{10}$Be 已衰变，由此给出**宇宙线储存时间 $\tau_{\rm esc} \sim 10$–$30$ My**（Simpson & Garcia-Munoz, 1988）；结合观测星际路径 $\sim 5$ g cm$^{-2}$，推出宇宙线所在介质**平均密度 $\sim 0.2$–$0.4$ H cm$^{-3}$**，远低于银盘平均 $\sim 1$ H cm$^{-3}$——晕存在的首个定量证据（原文 p.32，行 220–230）。
...
## 1.5 [FACT] ACE/CRIS 的性能与论文中的具体数值
Mewaldt 在 §1 末尾明确列出 CRIS 的性能指标（原文 p.27，Abstract 与 §1 末段）：
- **质量分辨率**：$m/\Delta m = 0.1$–$0.25$ amu（覆盖 $2 \le Z \le 30$）
- **几何因子（收集能力）**：$\sim 250$ cm$^2$ sr（原文 p.27，行 28–32）
- ACE 于 **1997 年** 发射（原文 p.28，§1 末段）

这些指标使得四种 β 衰变时钟（$^{10}$Be、$^{26}$Al、$^{36}$Cl、$^{54}$Mn）以及电子俘获时钟（$^{59}$Ni、$^{51}$Cr、$^{49}$V）的统计精度**首次**达到可同时给出统计+系统误差的水平（原文 p.27，Abstract 末句）。

## 1.6 [FACT] 四个尚未解答的"起源之谜"
Mewaldt 在 §1 第二段明确列出四条悬而未决的问题（原文 p.27，§1 第 2 段）：
1. **物质来源**：宇宙线是"新鲜"超新星抛射物、"老"星际物质，还是二者混合？
2. **加速模式**：一次性（one-shot，单个 SN 激波）还是分布式（一系列 SN 激波在其寿命中持续加速）？
3. **填充体积**：宇宙线是否填满银河系晕（halo）以及银盘（disk）？
4. **时间尺度**：核合成→加速、加速过程、储存寿命三段过程各自的时间尺度如何量测？

## 1.7 [FACT] 宇宙线能量收支的关键数字
Mewaldt 在 §1 首段给出两个关键数字（原文 p.27，§1 第 1 段）：
- 维持银河系宇宙线能量密度所需的功率：**$3 \times 10^{40}$ erg/s**
- 超新星是**唯一**能够提供这一功率的源
