...
利用长寿命放射性次级核素（¹⁰Be 等）作为"时钟"约束传播区域大小。
...
[FACT] Fig. 11 显示 ¹⁰Be/⁹Be 与模型的比较；ISOMAX ¹⁰Be 测量 (129) 至 2 GeV（因此衰变寿命更长）与拟合一致，但统计量不具强约束力。
...
| 物理量 | 值 |
|---|---|
| $D_{xx}$ | $(3-5)\times10^{28}$ cm²/s（在 3 GV） |
| $z_h$ | 4 kpc |
| 泄漏盒"逃逸时间" | $\approx 10^7$ yr |
| 实际从源到达晕边界的时间 | 比上述约 10 倍大 |
| 泄漏盒"气体密度" | $\approx 0.3$ cm⁻³ |
| 实际平均密度（4 kpc 晕） | $\approx 0.03$ cm⁻³（差 10 倍） |
...
[FACT] 实际情况更复杂：太阳约 $10^5$ 年前离开本地泡（在泡中停留数百万年），现在位于 CLIC（约 0.2 cm⁻³ HI 密度，35 pc 范围）(86, 87, 134)。
...
利用爆炸核合成产生的 K-俘获同位素（⁵⁹Ni, ⁵⁷Co, ⁵⁶Ni）判断加速是否在衰变之前发生。

#### 3.3.2 关键数据

| 核素 | 半衰期 | 观察 |
|---|---|---|
| ⁵⁹Ni | $7.6\times10^4$ y | 存在 |
| ⁵⁷Co | 0.74 y | 存在 |
| ⁵⁶Ni | 6 d | 缺失（符合预期，因为加速前就衰变） |

[FACT] 如果加速在衰变前发生，衰变被抑制（核被剥去电子）。⁵⁶Ni 缺失符合预期。

[FACT] (135) 使用 ACE 数据表明：合成与加速之间的延迟远长于 ⁵⁹Ni 的衰变时间，除非超新星中产生大量 ⁵⁹Co。
...
[FACT] (137)：⁵¹V/⁵²Cr 与再加速模型更符合，但 ⁴⁹Ti/(⁴⁶+⁴⁷+⁴⁸Ti) 给出相反结果。

[FACT] (74)：V/Cr 比在轻微上更符合含再加速的模型，涉及 Ti 的比值无结论。

[FACT] (137) 使用 ACE 的 ³⁷Ar, ⁴⁴Ti, ⁴⁹V, ⁵¹Cr, ⁵⁵Fe, ⁵⁷Co 数据，结果不明确。
...
[FACT] 10¹³ eV 处 Super-Kamiokande-I 探测器 (140) 给出了二维各向异性映射，统计显著性 $>5\sigma$，最大超额方向约 $\alpha = 75°$, $\delta = -5°$。
...
[FACT] 太阳系相对于本地星际介质的运动产生 $\sim 4\times10^{-4}$ 的常数项，最大强度指向银河中心方向，但**与 10¹²–10¹⁴ eV 数据（指向反银心方向）不符**。
...
[FACT] $\gamma$ 射线是确定 CO-H₂ 关系的重要独立方法。
...
| OSO-III | 1968 | 首次卫星观测 |
| SAS-2 | 1972 | 确认星际辐射 |
| COS-B | 1975–1982 | 发现 CR 分布不跟随 SNR，CO-H₂ 关系 |
...
[FACT] 可能的解决方案：H₂/CO 比随银心距增加，与银河系金属丰度梯度相关 (144)——可允许 CR 源跟随脉冲星示踪的 SNR 分布。

[FACT] Fig. 16 显示基于该模型的经度和纬度剖面，与 EGRET 数据满意一致。

[FACT] 但 H₂/CO 变化的幅度或其与金属丰度的关系尚不确定。
