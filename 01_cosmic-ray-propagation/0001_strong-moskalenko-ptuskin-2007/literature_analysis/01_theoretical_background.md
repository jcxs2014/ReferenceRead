...
[CRITIQUE] 作者明确声称"unified approach … has never been attempted"，但把 (32, 33) 列为"nearest approach"——这与 §2.12 中"自洽模型"的讨论在措辞上有微妙的张力：既然 (32, 33) 已经做了自洽耦合，为何 §1 又强调"从未尝试"？这可能是作者刻意区分"完整包含 CR 谱+次级+γ 射线的自洽耦合"（确实未完成）与"部分耦合"（(32, 33) 已做），但原文未明示这一区分。

[CRITIQUE] 在引言中作者把"宇宙线起源"问题明确排除（"we will, for the most part, sidestep this problem"），这一边界声明值得注意：后续 §5.4 中 $\gamma$ 射线推断 CR 分布偏离 SNR 经典分布——这实际上已经重新打开了起源问题。引言的"排除"与正文的"涉及"存在轻微不一致，反映作者对起源问题态度的转变。
...
- 超新星遗迹作为宇宙线源（文献可追溯至最近的 H.E.S.S. TeV $\gamma$-ray 结果 (34)）
- 太阳调制
- 星系团
- 外银河宇宙线
- 能量高于 $10^{15}$ eV 的宇宙线
...
> **分析 / Interpretation**：$a = 1/3$ 还是 $a = 1/2$ 不仅决定了 $D_{xx}(R)$ 的依赖关系，还决定了能否同时拟合次级/初级比和 CR 各向异性（见 §3.5）。

[CRITIQUE] §2.3.5 中作者对 Kolmogorov 与 Kraichnan 谱的判定引用 (55, 56)，仅停留在"理论预期"层面，未给出任何直接观测证据。在 2007 年时（至今亦是）没有对星际 MHD 湍流谱的直接观测能精确测量 $a$ 值——因此 $a$ 的选择实际上是"与观测相符的最佳拟合"而非"已被证实的理论"。这提醒读者：传播模型的谱指数选择是"由结果反推假设"，而非相反。

[CRITIQUE] 作者给出 $D_{xx}\approx 2\times10^{27}\beta R_{GV}^{1/3}$ 的理论估计，并强调其"与经验模型（含分布式再加速版本）一致"。这种"理论估计与经验拟合一致"的论断在 §2.5 再加速讨论中构成循环论证的嫌疑：经验模型已经假定包含再加速，再用这个 $D_{xx}$ 论证再加速是必要的——需要 K-俘获同位素等独立证据打破循环。
...
[FACT] 再加速的额外检验来自次级 K-俘获同位素：$^{37}{\rm Ar}$, $^{44}{\rm Ti}$, $^{49}{\rm V}$, $^{51}{\rm Cr}$ 等，在低能时通过电子俘获快速衰变。

[FACT] 电子轨道存在的概率强烈依赖于能量，因此这些同位素及其衰变产物的丰度是能量的强函数，对能量变化敏感。

[FACT] $^{49}{\rm V}$ 和 $^{51}{\rm Cr}$ 在 CR 中能量依赖衰变的首次测量 (73) 被用来检验分布再加速速率 (74)，但需要核产生截面的精细化。
...
[INTERPRETATION] 泄漏盒模型之所以在 §2.8 被保留讨论，是因为它在"低能区路径长度分布"问题上的简洁性：指数分布的截断直接对应观测到的次级/初级比在低能的上升——这是一个"用最简假设解释最锐利现象"的典型。

[INTERPRETATION] §2.9 的多源模型与 §3.9 的时间/空间非稳态效应是互补的：前者是解析方法对离散源的近似，后者指出这种近似在电子传播中系统性失效。作者对解析解的乐观（§2.11）与实际物理复杂性之间存在明显张力。

[INTERPRETATION] §2.12 自洽模型章节篇幅短（几段），但触及了本综述的一个核心盲点：如果 CR 是 ISM 动力学的一部分，那么"传播方程 (1)"就只是更一般 ISM 动力学方程的一个极限——这意味着所有基于 (1) 的结论都有隐含的前提假设，而该前提尚未被显式验证。
...
[CRITIQUE] GALPROP 被 NASA/AMS/ACE/Pamela 等多个合作组采纳，几乎形成"行业标准"垄断。这种主导地位在方法学上是一把双刃剑：一方面保证了不同研究的比较性，另一方面也意味着 GALPROP 的假设（如均匀源丰度、2D 轴对称、无 clumpy ISM）被整个领域不加批判地接受——缺乏独立代码的交叉验证。

[CRITIQUE] 作者自承 GALPROP "the computing resources required … are moderate by today's standards"，并明确承认"尚未找到一个模型同时满足所有数据"。这种坦承在 2007 年是诚实的，但到了 2026 年回头看，意味着 15 年过去后传播模型尚未在"全能区统一拟合"上取得决定性进展——这本身就说明宇宙线传播问题的复杂程度超出了作者当年的预期。
