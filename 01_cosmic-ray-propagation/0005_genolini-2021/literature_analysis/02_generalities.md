...
| 参数 | 含义 | 典型值 |
|---|---|---|
| $K_0$ | 扩散系数归一化 | $\sim 2 \times 10^{28}$ cm²/s |
| $R_1$ | 参考刚度 | 4 GV |
...
| 本地 DM 密度 $\rho_\odot$ | $\sim 0.4$ GeV/cm³ | §2 |
...
| 参数 | 值 | 出处 |
|---|---|---|
| $K_0$（扩散归一化）| $\sim 2\times10^{28}$ cm²/s | §II |
| $R_1$（参考刚度）| 4 GV | §II |
...
→ 扩散系数完整形式（含高低刚度断裂）
→ DM 湮灭源项（NFW 剖面）
→ 反质子通量标度：dp/dE ∝ L²/K（晕大小 + 扩散）
→ 正电子通量标度：de⁺/dE ∝ L²/(K^{1/2}·É^{1/2})（晕大小 + 扩散 + 能量损失）
→ L/K ~ const（B/C 约束）
