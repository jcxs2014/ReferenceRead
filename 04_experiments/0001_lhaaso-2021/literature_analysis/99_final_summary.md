...
2. **CR 抑制 10⁻⁵ @1 PeV** → background-free 探测模式
3. **似然比检验**（源+0.3° 高斯延展模板 vs. 背景-only 模型）
4. **SED 重建**：Δ(logE)=0.2 分 10 个 bin，向前展开 + 最小 χ²
...
1. **首次 background-free UHE γ 探测**（缪子 veto 效率达 10⁻⁵）
...
3. **KM2A 缪子 veto** 将 CR 抑制至 10⁻⁵ @1 PeV
4. **1.4 PeV 光子误机概率 0.028%**，N_μ/N_e = 1/941
5. **积分灵敏度 10⁻¹⁴ erg cm⁻² s⁻¹** 远超现有 γ 探测器
6. **三源 SED** 用对数抛物线拟合优于幂律（ΔAIC=2.5–5.7）
7. **12 源全部位于银河平面**，延展 ≥1°（≥10⁴ pc³ 尺度）
8. **100 TeV 通量范围 0.4–4 CU**（1 CU = 6.1×10⁻¹⁷ ph/TeV/cm²/s）
